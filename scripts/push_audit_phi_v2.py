...
async def push_tasks():
    ...
    for t in tasks:
        # Явно указываем phi3.5:3.8b в промпте и метаданных
        ...
        task_data = {
            ...
            'category': 'coding', # Смена категории на coding для выбора легких моделей
            'metadata': {
                'complex': True,
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE tasks SET metadata = metadata || $2::jsonb WHERE id = $1",
                t['id'],
