...
async fn async_main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let knowledge_engine = {
        let max_retries: u32 = std::env::var("GATEWAY_DB_MAX_RETRIES")
            .ok()
            ...
            match KnowledgeEngine::new(&database_url).await {
                ...
                Err(e) => {
                    if attempt < max_retries {
                        warn!(
                            "⚠️ KnowledgeEngine attempt {}/{} failed: {}. Retrying in {}s...",
                            attempt, max_retries, e, delay_secs
                        );
                        tokio::time::sleep(std::time::Duration::from_secs(delay_secs)).await;
                        delay_secs = (delay_secs * 2).min(32); // exponential backoff, cap at 32s
                    } else {
                        warn!(
                            "⚠️ KnowledgeEngine failed after {} attempts: {}. Knowledge features will be unavailable.",
                            max_retries, e
                        );
                    }
                }
