...
class ModelBase:
    ...
    def register(cls, *names: str) -> Callable[[AnyModel], AnyModel]:
        ...
        def func(modelcls: AnyModel) -> AnyModel:
            model_type = ModelType.MMPROJ if modelcls.model_arch == gguf.MODEL_ARCH.LLAMA else ModelType.TEXT
            for name in names:
...
class MmprojModel(ModelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.model_arch != gguf.MODEL_ARCH.LLAMA:
            raise TypeError("MmprojModel must be subclassed with model_arch = gguf.MODEL_ARCH.LLAMA")
        ...
        self.tensor_map = gguf.get_tensor_name_map(gguf.MODEL_ARCH.LLAMA, self.block_count)
...
@ModelBase.register("Qwen2Model", "Qwen2ForCausalLM", "Qwen2AudioForConditionalGeneration", "KORMoForCausalLM", "AudioFlamingo3ForConditionalGeneration", "LlamaForCausalLM", "Qwen2ForCausalLM")
...
class _LinearAttentionVReorderBase(Qwen3NextModel):
    model_arch = gguf.MODEL_ARCH.LLAMA  # overridden by subclasses
...
class BertModel(TextModel):
    ...
    def _xlmroberta_set_vocab(self) -> None:
        ...
        if isinstance(tokenizer, SentencePieceProcessor):
            ...
            if self.model_arch == gguf.MODEL_ARCH.LLAMAOE:
...
class NomicBertModel(BertModel):
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, **kwargs: Any):
        ...
        self.model_arch = gguf.MODEL_ARCH.LLAMAOE if self.is_moe else gguf.MODEL_ARCH.LLAMA
