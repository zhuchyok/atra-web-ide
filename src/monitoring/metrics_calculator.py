...
try:
    ...
    logger.warning(
        "Модуль extended_metrics не найден, расширенные метрики будут рассчитаны встроенным методом"
    )
    calculate_var = None
    calculate_cvar = None
...
class PerformanceMetricsCalculator:
    ...
    def _calculate_daily_returns(self, df: pd.DataFrame) -> pd.Series:
        ...
        if "exit_dt" in df_local.columns:
            ...
            df_local["exit_dt"] = pd.to_datetime(
                df_local["exit_time"], errors="coerce", utc=True, format="mixed"
            )
    ...
    def get_symbol_performance(
        self, symbol: str, limit_days: int = 90, trade_mode: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not trades:
                return self._empty_metrics(
                    start_date=start_date, user_id=None, trade_mode=trade_mode
                )
    ...
    def _calculate_metrics_from_df(
        ...
        if "entry_time" in df.columns:
            df["entry_dt"] = pd.to_datetime(
                df["entry_time"], errors="coerce", utc=True, format="mixed"
            )
        else:
            df["entry_dt"] = pd.NaT
        ...
        if len(daily_returns) > 0:
            if calculate_var is not None and calculate_cvar is not None:
                ...
                try:
                    ...
                    cvar_95 = (
                        abs(cvar_95_returns.mean()) * 100 if len(cvar_95_returns) > 0 else var_95
                    )
                    cvar_99 = (
                        abs(cvar_99_returns.mean()) * 100 if len(cvar_99_returns) > 0 else var_99
                    )
                except Exception:
                    pass
