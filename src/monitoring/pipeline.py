...
class PipelineMonitor:
    ...
    def log_stage(self, stage: str, symbol: str, passed: bool, details: str = ""):
        ...
        logger.debug("📊 [%s] %s: %s %s", symbol, stage, "✅ ПРОЙДЕН" if passed else "❌ ОТКЛОНЕН", details)
    ...
    def print_stats(self):
        ...
        logger.info("  ✅ Валидация: %.1f%% (%d/%d)", rates.get('validation_rate', 0) * 100,
                   self.stats['validation_passed'], self.stats['total_attempts'])
        logger.info("  🤖 ИИ-скор: %.1f%% (%d/%d)", rates.get('ai_score_rate', 0) * 100,
                   self.stats['ai_score_passed'], self.stats['validation_passed'])
        logger.info("  📈 Объем: %.1f%% (%d/%d)", rates.get('volume_rate', 0) * 100,
                   self.stats['volume_passed'], self.stats['ai_score_passed'])
        logger.info("  📊 Волатильность: %.1f%% (%d/%d)", rates.get('volatility_rate', 0) * 100,
                   self.stats['volatility_passed'], self.stats['volume_passed'])
        logger.info("  🎯 EMA паттерны: %.1f%% (%d/%d)", rates.get('ema_pattern_rate', 0) * 100,
                   self.stats['ema_pattern_passed'], self.stats['volatility_passed'])
        logger.info("  🚀 Финальные сигналы: %.1f%% (%d/%d)", rates.get('final_rate', 0) * 100,
                   self.stats['final_signals'], self.stats['ema_pattern_passed'])
        logger.info("  📈 ОБЩАЯ ПРОХОДИМОСТЬ: %.1f%% (%d/%d)", rates.get('overall_rate', 0) * 100,
                   self.stats['final_signals'], self.stats['total_attempts'])

        if patterns:
            logger.info("  🎨 РАСПРЕДЕЛЕНИЕ ПАТТЕРНОВ:")
            for pattern, rate in patterns.items():
                logger.info("    • %s: %.1f%% (%d)", pattern.replace('_', ' ').title(),
