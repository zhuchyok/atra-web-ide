...
class ProfitabilityMonitor:
            ...
            try:
                ...
                if "error" in time_analysis_result:
                    ...
                    if time_analysis_result.get("total_trades", 0) == 0:
                        logger.debug(
                            f"📊 [Максим] Анализ времени: {time_analysis_result.get('message', 'Нет данных')}"
                        )
                    else:
                        logger.warning(
                            f"⚠️ [Максим] Ошибка анализа времени: {time_analysis_result.get('error')}"
                        )
                else:
    ...
    def _log_metrics(self, metrics: Dict[str, Any], alerts: List[Dict[str, Any]]):
        ...
        logger.info(
            f"📊 Соотношение убытков к прибыли: {metrics.get('loss_profit_ratio', 0.0):.1f}:1"
        )
