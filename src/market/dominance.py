...
class BTCDominanceAnalyzer:
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        async with self.session.get(
                            eth_url, params=params, timeout=aiohttp.ClientTimeout(total=5)
                        ) as eth_response:
                    ...
                    # Кэшируем
                    _DOMINANCE_CACHE[cache_key] = {"data": dominance_data, "timestamp": time.time()}
    ...
    async def get_dominance_history(self, days: int = 7) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df = pd.DataFrame(
                {"timestamp": [current.timestamp], "btc_dominance": [current.btc_dominance]}
            )
    ...
    def calculate_dominance_trend(self, history_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if history_df is None or len(history_df) < 2:
            return {"trend": "neutral", "change_pct": 0.0, "days_rising": 0}
