    ...
    def set_symbol_info_cache(self, symbol: str, data: Dict[str, Any], timestamp: float):
        ...
        if self._use_dict_cache:
            ...
            self._symbol_info_cache.set(
                f"symbol_info:{symbol}", cache_entry, ttl=SYMBOL_INFO_CACHE_TTL
            )
...
async def get_dynamic_price_precision(
    symbol: str, cache_manager: Optional[StatelessCacheManagerWrapper] = None
):
    """
...
async def get_symbol_info(
    ...
    try:
        ...
        bybit_url = (
            f"https://api.bybit.com/v5/market/instruments-info?category=spot&symbol={symbol}"
        )
        price_tick = 0.0
        price_precision = 8
        min_price = 0.0
            ...
            if resp_b.status_code == 200:
                ...
                if symbols:
                    ...
                    if notional_f:
                        ...
                        min_notional = float(
                            notional_f.get("minNotional") or notional_f.get("minNotional", 0) or 0
                        )
        except (requests.exceptions.RequestException, ValueError, KeyError) as e:
