...
class PerformanceMonitor:
    ...
    def _collect_performance_metrics(self):
        ...
        try:
            ...
            logger.debug(
                f"Метрики производительности собраны: CPU={cpu_percent:.1f}%, "
    ...
    def _collect_application_metrics(self):
        ...
        try:
            ...
            logger.debug(
                f"Метрики приложения собраны: RPS={requests_per_second:.2f}, "
    ...
    def _check_alerts(self):
        ...
        try:
            ...
            if perf.memory_usage_mb > self.alert_thresholds["memory_usage_mb"]:
                self._trigger_alert(
                    "high_memory", f"Высокое использование памяти: {perf.memory_usage_mb:.1f}MB"
                )

            if perf.disk_usage_percent > self.alert_thresholds["disk_usage_percent"]:
                self._trigger_alert(
                    "high_disk", f"Высокое использование диска: {perf.disk_usage_percent:.1f}%"
                )

            # Проверка алертов приложения
            if app.error_rate > self.alert_thresholds["error_rate"]:
                self._trigger_alert(
                    "high_error_rate", f"Высокий уровень ошибок: {app.error_rate:.2%}"
                )

            if app.avg_response_time > self.alert_thresholds["avg_response_time"]:
                self._trigger_alert(
                    "slow_response", f"Медленный ответ: {app.avg_response_time:.2f}s"
                )
    ...
    def get_historical_metrics(self, hours: int = 1) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            perf_history = [
                m.__dict__ for m in self.performance_history if m.timestamp >= cutoff_time
            ]

            app_history = [
                m.__dict__ for m in self.application_history if m.timestamp >= cutoff_time
            ]

            return {"performance": perf_history, "application": app_history}
    ...
    def export_metrics(self, filename: str = None) -> str:
        ...
        try:
            ...
            export_data = {
                ...
                "historical_performance": [m.__dict__ for m in self.performance_history],
                "historical_application": [m.__dict__ for m in self.application_history],
            }
