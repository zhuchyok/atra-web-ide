...
try:
    from config import BLACKLISTED_SYMBOLS, STABLECOIN_SYMBOLS  # type: ignore
except ImportError:
    STABLECOIN_SYMBOLS = [
        "USDTUSDT",
        "USDCUSDT",
    ...
    try:
        ...
        if not use_new_entry_logic and current_price > ema_fast and ema_fast > ema_slow:
            ...
            try:
                ...
                confluence_passed, confluence_score, confluence_details = (
                    confluence_validator.validate_confluences(
                        symbol=symbol,
                        signal_type=signal_type,
                        df=df,
                ...
                if confluence_passed:
                    ...
                    )  # До 15% бонуса за хорошие конфлюэнции
                    quality_score += confluence_bonus
                    logger.info(
                        "✅ [CONFLUENCE BONUS] %s %s: %d конфлюэнций (score: %.2f, bonus: +%.2f%%)",
                        symbol,
                        signal_type,
                        confluence_details.get("total_confluences", 0),
                        confluence_score,
                        confluence_bonus * 100,
                    )
                else:
                    ...
                    logger.debug(
                        ...
                        signal_type,
                        confluence_details.get("total_confluences", 0),
                        confluence_details.get("required", 2),
                    )
            except ImportError as e:
            ...
            if backtest_min_confidence_long is not None:
                ...
                min_confidence_threshold = max(
                    0.50, base_confidence_threshold + market_adjustment * 0.7
                )
                ...
                try:
                    ...
                    if df_for_tp is not None and len(df_for_tp) > 0:
                        ...
                        if signal_type.upper() == "BUY":
                            ...
                            sl_price = (
                                signal_price * 1.03
                            )  # Увеличено с 1.02 до 1.03 (3% вместо 2%)
