...
try:
    from src.strategies.pair_filtering import get_filtered_top_usdt_pairs_fast as _get_pairs_fast  # type: ignore
except ImportError:
    try:
        from src.execution.exchange_api import get_filtered_top_usdt_pairs_fast as _get_pairs_fast  # type: ignore
    except ImportError:
...
try:
    from config import STABLECOIN_SYMBOLS, BLACKLISTED_SYMBOLS  # type: ignore
except ImportError:
    STABLECOIN_SYMBOLS = [
    ...
    BLACKLISTED_SYMBOLS = []  # Fallback - пустой список
...
async def get_bitget_futures_symbols() -> List[str]:
    ...
    if (_bitget_symbols_cache is not None and
        _bitget_symbols_cache_timestamp is not None and
        current_time - _bitget_symbols_cache_timestamp < BITGET_SYMBOLS_CACHE_TTL):
    ...
    try:
        ...
        for _, market in markets.items():
            ...
            if market.get('type') in ('swap', 'futures', 'linear') and market.get('settle') == 'USDT':
        ...
        if symbols:
            ...
            logger.info("✅ [LISTING] Список символов Bitget Futures обновлен (%d монет)", len(symbols))
            return symbols
...
try:
    ...
    logger.warning("⚠️ Новые фильтры (Dominance/InterestZone/Fibonacci/VolumeImbalance) недоступны: %s", e)

# Импорт Institutional Patterns фильтра (синхронный)
try:
    # pylint: disable=ungrouped-imports
    from src.filters.institutional_patterns_filter import check_institutional_patterns_filter  # type: ignore
...
try:
    from src.filters.smart_trend_filter import get_smart_trend_filter  # type: ignore # pylint: disable=ungrouped-imports
...
try:
    from src.analysis.pullback_entry import PullbackEntryLogic  # type: ignore # pylint: disable=ungrouped-imports
...
def _log_mtf_event(
    ...
    try:
        if _mtf_event_db is None:
            from src.database.db import Database  # type: ignore  # type: ignore # локальный импорт, чтобы избежать циклов
...
def _log_position_sizing_event(event: Dict[str, Any]) -> None:
    ...
    try:
        if _sizing_audit_db is None:
            from src.database.db import Database  # type: ignore  # type: ignore # локальный импорт для избежания циклов
...
async def _get_data_with_fallback(symbol: str, timeframe: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        from src.execution import exchange_api  # type: ignore # pylint: disable=import-outside-toplevel
        ...
        if timeframe == '4h':
            ...
            if df_2h and len(df_2h) >= 40:
                ...
                if len(df_4h) >= 20:
                    logger.debug("✅ %s: Используем 2h→4h агрегацию (%d свечей)", symbol, len(df_4h))
            ...
            if df_30m and len(df_30m) >= 60:
                ...
                if len(df_1h) >= 30:
                    logger.debug("✅ %s: Используем 30m→1h агрегацию (%d свечей)", symbol, len(df_1h))
...
async def _run_mtf_confirmation_with_logging(
            ...
            if HYBRID_MTF_CONFIG.get('enabled', True):
                ...
                if df_h4 is not None:
                    ...
                    _log_mtf_event(
                        ...
                        direction=direction,
                        confirmed=confirmed,
                        error_text=None if confirmed else f"Гибридная MTF: confidence={confidence:.2f} < min (details: {mtf_details.get('reason', 'N/A')})",
                        regime_data=regime_data,
                    )

                    if confirmed:
                        logger.debug("✅ %s: Гибридная MTF подтверждена (confidence=%.2f)", symbol, confidence)
                        pipeline_monitor.log_stage('mtf_confirmation', symbol, True,
                                                  f"Гибридная MTF: {confidence:.2f}")
                        return True, None
                    else:
                        logger.debug("🚫 %s: Гибридная MTF не подтверждена (confidence=%.2f)", symbol, confidence)
                    ...
                    logger.warning("⚠️ %s: Не удалось получить данные H4 для гибридной MTF, используем fallback", symbol)
        except Exception as e:
    ...
    try:
        ...
        if not mtf_confirmed:
            logger.debug("🚫 %s: MTF Confirmation не пройден для %s: %s", symbol, direction, mtf_error)
            pipeline_monitor.log_stage('mtf_confirmation', symbol, False, f"MTF не подтвержден: {mtf_error}")
...
async def calculate_anomaly_circles_with_fallback(symbol: str, signal_type: str) -> tuple:
    ...
    try:
        ...
        if not data.get("available"):
            ...
            logger.debug("⚠️ Функция calculate_anomaly_circles_with_fallback недоступна для %s (источник: %s)",
        ...
        if volume_24h < min_volume_threshold:
            logger.warning(
                "[Anomaly] %s: низкий объем торгов $%.0f < $1M — монета 'стоит'",
                symbol, volume_24h
            )
...
try:
    ...
    if lightgbm_predictor.load_models():
        ...
        logger.warning("⚠️ LightGBM предсказатель доступен, но модели не обучены (используйте train_lightgbm_models.py)")
except ImportError as e:
...
try:
    ...
    logger.warning("⚠️ Гибридный менеджер данных недоступен. "
                  "Будет использоваться прямой доступ к API.")
...
try:
    ...
    logger.error("❌ Ошибка импорта Telegram-интеграции: %s. "
                "Отправка в Telegram будет отключена.", e)
...
async def calculate_ai_signal_score(df: pd.DataFrame, ai_params: Dict[str, Any], symbol: Optional[str] = None, news_analysis: Optional[Dict[str, Any]] = None) -> float:
    """
    ...
    if ADAPTIVE_REGULATOR_AVAILABLE and get_adaptive_regulator:
        try:
            ...
            thresholds = await regulator.get_all_adaptive_thresholds(
                df=df,
                market_volatility=float(df['volatility'].iloc[-1]) if 'volatility' in df.columns else None,
    ...
    elif 'volume_ratio' in df.columns and vol_ratio_val > current_ai_params.get("soft_volume_ratio", 1.2):
        score += 5
...
async def get_cached_api_coins() -> List[str]:
    ...
    if (_api_coins_cache is not None and
        ...
        logger.info("✅ Используем кэшированный список монет из API (%d монет)", len(_api_coins_cache))
    ...
    try:
        ...
        try:
            ...
            if api_coins and len(api_coins) > 0:
                logger.info("✅ Получено %d монет из API (кэш обновляется раз в сутки)", len(api_coins))

                # Фильтрация стейблкоинов, чёрного списка и дублей
                filtered_symbols = [
                ...
                if filtered_symbols:
                    ...
                    if bitget_symbols:
                        ...
                        if len(filtered_symbols) < original_count:
                            logger.info("📉 [LISTING] Исключено %d монет, не торгуемых на Bitget Futures",
                                      original_count - len(filtered_symbols))
                    ...
                    for symbol in filtered_symbols:
                        ...
                        if symbol in optimized_symbols:
                            ...
                            if is_ready:
                                ...
                                logger.debug("✅ [%s] Используем оптимизированные параметры из intelligent_filter_system", symbol)
                            else:
                                # Параметры есть, но оптимизация не завершена
                                pending_symbols.append(symbol)
                                logger.info("⏳ [%s] Оптимизация в процессе, монета временно заблокирована", symbol)
                        else:
                            ...
                            if is_ready:
                                ...
                                pending_symbols.append(symbol)
                                logger.info("⏳ [%s] Оптимизация запущена, монета заблокирована до завершения", symbol)

                    if ready_symbols:
                        logger.info("✅ Готово %d монет для генерации сигналов (из %d API, %d в оптимизации)",
                                  len(ready_symbols), len(filtered_symbols), len(pending_symbols))
                        return ready_symbols
                    else:
                        logger.warning("⚠️ Нет готовых монет из API (все в процессе оптимизации), используем fallback на intelligent_filter_system")
        ...
        try:
            ...
            if intelligent_coins and len(intelligent_coins) > 0:
                logger.info("✅ Используем монеты из intelligent_filter_system (fallback): %d монет", len(intelligent_coins))
                filtered_symbols = [
                ...
                if filtered_symbols:
                    ...
                    logger.info("✅ Готово %d монет для генерации сигналов (из %d intelligent_filter_system)", len(ready_symbols), len(filtered_symbols))
                    return ready_symbols
        except Exception as e:
            ...
            if filtered_symbols:
                ...
                for symbol in filtered_symbols:
                    ...
                    if is_ready:
                        ...
                        ready_symbols.append(symbol)
                        logger.info("✅ [%s] Монета готова (базовые параметры)", symbol)
                logger.info("✅ Готово %d монет для генерации сигналов (из %d)", len(ready_symbols), len(filtered_symbols))
                return ready_symbols
...
async def process_symbol_signals(
    ...
    try:
        logger.info("🔍 [PROCESS] Начало обработки символа %s для %d пользователей", symbol, len(user_data_dict))
        # Проверяем каждого пользователя
        for user_id, user_data in user_data_dict.items():
            try:
                logger.info("🔍 [PROCESS] %s: Генерация сигнала для пользователя %s (mode=%s)",
                           symbol, user_id, user_data.get('trade_mode', 'unknown'))
                # Генерация сигнала с учетом режима
                logger.info("🔍 [BEFORE CALL] %s: Вызов _generate_signal_impl для пользователя %s", symbol, user_id)
                try:
                    signal_type, signal_price, ml_prediction = await _generate_signal_impl(
                        symbol, df, user_data, regime_data, regime_multipliers
                    )
                    logger.info("🔍 [AFTER CALL] %s: _generate_signal_impl вернул type=%s, price=%s",
                               symbol, signal_type, signal_price)
                except Exception as gen_exc:
                    logger.error("❌ [GEN ERROR] %s: Исключение в _generate_signal_impl: %s", symbol, gen_exc)
                    import traceback
                ...
                if signal_type and signal_price:
                    logger.info("✅ [SIGNAL GENERATED] %s: Сигнал %s @ %.8f сгенерирован для пользователя %s",
                              symbol, signal_type, signal_price, user_id)
                    # Отправляем сигнал с учетом режима (composite и quality будут дефолтными)
                    logger.info("📤 [SEND START] %s: Начало отправки сигнала %s для пользователя %s (источник: process_symbol_signals)",
                    ...
                    if success:
                        signals_sent += 1
                        logger.info("📤 [SEND SUCCESS] Сигнал %s для %s отправлен пользователю %s", signal_type, symbol, user_id)
                    else:
                        logger.warning("⚠️ [SEND FAILED] Сигнал %s для %s НЕ отправлен пользователю %s (send_signal вернул False)",
                                     signal_type, symbol, user_id)
                else:
                    logger.info("🚫 [NO SIGNAL] %s: generate_signal вернул None для пользователя %s", symbol, user_id)

            except Exception as e:
                logger.error("❌ [ERROR] Ошибка обработки сигнала для пользователя %s и символа %s: %s", user_id, symbol, e)
                import traceback
...
async def _generate_signal_impl(
    ...
    _ml_prediction = None
    logger.info("🔍 [GENERATE START] %s: Начало генерации сигнала для пользователя %s", symbol, user_id)
    try:
        ...
        if len(df) < 50:
            ...
            pipeline_monitor.log_stage('validation', symbol, False, f"Недостаточно баров: {len(df)}")
            return None, None, None
        ...
        if missing_columns:
            ...
            pipeline_monitor.log_stage('validation', symbol, False, f"Отсутствуют колонки: {missing_columns}")
            return None, None, None
        ...
        try:
            if get_symbol_news_analysis:
                ...
                if news_analysis:
                    logger.info("📰 [%s] Сентимент новостей: %s (score: %d)", 
                               symbol, news_analysis.get('sentiment'), news_analysis.get('score', 0))
        except Exception as e:
        ...
        score = await calculate_ai_signal_score(df, ai_params_data, symbol, news_analysis)  # Передаем news_analysis
        ...
        try:
            ...
            logger.debug("✅ [%s] Используем индивидуальные параметры: AI Score threshold = %.1f", symbol, ai_score_threshold)
        except Exception as e:
            logger.debug("⚠️ [%s] Ошибка загрузки индивидуальных параметров: %s, используем общие", symbol, e)
            # Используем наши оптимизированные пороги вместо ИИ-параметров
            # 🔧 УЛУЧШЕНО: Повышены пороги AI Score для улучшения качества сигналов
            # Старые значения: 15.0 (soft), 25.0 (strict)
            # Новые значения: 18.0 (soft), 28.0 (strict) - более строгие требования
            if filter_mode == "soft":
                ai_score_threshold = 18.0  # Повышен с 15.0 для лучшего качества
            else:
                ai_score_threshold = 28.0  # Повышен с 25.0 для лучшего качества
            logger.debug("📊 [THRESHOLD] %s: AI Score threshold = %.1f (mode=%s)",
                       symbol, ai_score_threshold, filter_mode)

        if score < ai_score_threshold:
            user_id = user_data.get("user_id", "unknown")
            logger.warning("🚫 [FILTER BLOCK] %s (user=%s): AI Score фильтр - Score %.1f < порог %.1f (mode=%s)",
        ...
        if COMPOSITE_ENGINE_AVAILABLE and composite_engine and regime_data:
            try:
                ...
                if composite_result['confidence'] > 0.7:
                    ...
                    logger.info("🎯 [%s] Composite бонус: +%.1f (confidence: %.2f)",
        ...
        if not check_ai_volume_filter(df, ai_params):
            user_id = user_data.get("user_id", "unknown")
            logger.warning("🚫 [FILTER BLOCK] %s (user=%s): Volume фильтр - Объем ниже порога", symbol, user_id)
        ...
        if not check_ai_volatility_filter(df, ai_params):
            user_id = user_data.get("user_id", "unknown")
            logger.warning("🚫 [FILTER BLOCK] %s (user=%s): Volatility фильтр - Волатильность вне диапазона", symbol, user_id)
        ...
        try:
            ...
            if anomaly_data_ok and circles_count and circles_count >= 5:
                ...
                pipeline_monitor.log_stage(
                    ...
                    f"Максимальный риск: {circles_display} кружков"
                )
                # 📊 Логируем отклонение в БД
                try:
                    from src.utils.filter_logger import log_filter_check_async  # type: ignore
            ...
            circles_display = circles_count if circles_count is not None else 0
            pipeline_monitor.log_stage('anomaly_filter', symbol, True, f"Риск приемлемый: {circles_display} кружков")

        except (ImportError, asyncio.TimeoutError, Exception) as e:
            error_type = type(e).__name__
            error_msg = str(e) if str(e).strip() else "Пустое сообщение ошибки"
            logger.debug("⚠️ Проверка аномалий для %s: %s - %s. Сигнал разрешен (fallback)",
        ...
        if use_new_entry_logic:
            ...
            if should_enter:
                ...
                logger.info("✅ [PULLBACK ENTRY] %s: Вход на откате к поддержке (Quality=%.2f, Regime=%s)",
                ...
                logger.info("🔍 [PULLBACK ENTRY] %s: Проверка трендов (умная логика на основе корреляции)...", symbol)
                ...
                new_filters_passed, new_filters_reason = await check_new_filters(symbol, signal_type, current_price, df, strict_mode=filter_mode == 'strict')
                if not new_filters_passed:
                    logger.warning("🚫 [PULLBACK ENTRY] %s: Новые фильтры заблокировали: %s", symbol, new_filters_reason)
                    return None, None, None
                logger.info("✅ [PULLBACK ENTRY] %s: Новые фильтры пройдены (%s)", symbol, new_filters_reason)
                ...
                logger.debug("⏭️ [PULLBACK ENTRY] %s: Не прошел проверку (%s), пробуем EMA кроссовер",
        ...
        if not use_new_entry_logic and current_price > ema_fast and ema_fast > ema_slow:
            ...
            if not trend_result:
                logger.warning("🚫 [LONG CLASSIC] %s: Тренд alignment не пройден - сигнал заблокирован, авто-исполнение не будет", symbol)
                return None, None, None
            ...
            if not new_filters_passed:
                ...
                return None, None, None
            logger.info("✅ [LONG CLASSIC] %s: Новые фильтры пройдены (%s)", symbol, new_filters_reason)
            ...
            logger.debug("⏭️ [LONG CLASSIC] %s: Direction confidence временно отключен (как в успешном бэктесте)", symbol)

            logger.info("🔍 [LONG CLASSIC] %s: Проверка RSI warning...", symbol)
            if not await check_rsi_warning(df, signal_type):
                logger.warning("🚫 [LONG CLASSIC] %s: RSI warning не пройден (RSI в опасной зоне)", symbol)
                return None, None, None
            ...
            # 🎯 КОНФЛЮЭНЦИИ (Confluences) - Павел (Trading Strategy): Мировая практика для повышения Win Rate
            # Минимум 2-3 подтверждений для входа (уровни, свечные паттерны, pullback, etc.)
            # 🔧 ВАЖНО: Конфлюэнции дают БОНУС, но НЕ блокируют сигналы (слишком строго было)
            try:
                from src.signals.confluence_validator import get_confluence_validator
                from src.patterns.candle_patterns import CandlePatternDetector
                
                confluence_validator = get_confluence_validator()
                candle_detector = CandlePatternDetector()
                
                confluence_passed, confluence_score, confluence_details = confluence_validator.validate_confluences(
                    symbol=symbol,
                    signal_type=signal_type,
                    df=df,
                    current_price=current_price,
                    filter_mode=filter_mode,
                    levels_detector=levels_detector if LEVELS_DETECTOR_AVAILABLE else None,
                    candle_detector=candle_detector,
                    regime_data=regime_data
                )
                
                # 🔧 ИЗМЕНЕНО: Конфлюэнции дают бонус, но НЕ блокируют сигналы
                # Если конфлюэнций достаточно - даем бонус к quality_score
                if confluence_passed:
                    confluence_bonus = confluence_score * 0.15  # До 15% бонуса за хорошие конфлюэнции
                    quality_score += confluence_bonus
                    logger.info(
                        "✅ [CONFLUENCE BONUS] %s %s: %d конфлюэнций (score: %.2f, bonus: +%.2f%%)",
                        symbol, signal_type, confluence_details.get('total_confluences', 0), 
                        confluence_score, confluence_bonus * 100
                    )
                else:
                    # Если конфлюэнций недостаточно - просто логируем, но НЕ блокируем
                    logger.debug(
                        "⚠️ [CONFLUENCE LOW] %s %s: Мало конфлюэнций (%d/%d), но продолжаем",
                        symbol, signal_type, confluence_details.get('total_confluences', 0), confluence_details.get('required', 2)
                    )
            except ImportError as e:
                logger.debug("⚠️ ConfluenceValidator недоступен: %s", e)
            except Exception as e:
                logger.warning("⚠️ Ошибка проверки конфлюэнций для %s: %s", symbol, e)
            
            # БОНУС: Проверка близости к статическим уровням (дополнительно к конфлюэнциям)
            ...
            if backtest_market_adjustment is not None:
                ...
                logger.debug("🔧 [BACKTEST] Используем переопределенный market_adjustment: %.3f", market_adjustment)
            else:
            ...
            # 🕒 АНАЛИЗ ВРЕМЕНИ - Максим (Data Analyst): Исключение худших периодов
            try:
                from src.analysis.time_analysis import get_time_analysis
                time_analysis = get_time_analysis()
                
                # Проверяем, можно ли торговать сейчас
                can_trade, time_reason = time_analysis.should_trade_now()
                if not can_trade:
                    logger.warning(
                        "🚫 [TIME FILTER] %s %s: Торговля заблокирована по времени: %s",
                        symbol, signal_type, time_reason
                    )
                    return None, None, None
                
                # Если период не худший, но близок к нему - снижаем пороги
                now = get_utc_now()
                ...
                if current_hour >= 17 or current_hour < 5:
                    ...
                    logger.info("🌙 [ADAPTIVE] Ночная адаптация для %s (час=%d UTC): -0.08", symbol, current_hour)
            except ImportError:
                ...
                logger.debug("Ошибка анализа времени: %s", e)
                ...
                if 'volatility' in df.columns and len(df) > 0:
                    ...
                    if current_volatility is not None and avg_volatility is not None and avg_volatility > 0:
                        ...
                        if current_volatility < avg_volatility * 0.7:
                            vol_adjustment = min(0.15, (avg_volatility - current_volatility) / avg_volatility * 0.30)  # 🔧 УВЕЛИЧЕНО: до 0.15
                            market_adjustment -= vol_adjustment
                            logger.info("📉 [ADAPTIVE] Низкая волатильность (%.2f%% < %.2f%%): снижаем пороги на %.3f",
            ...
            try:
                if 'volume' in df.columns and len(df) > 0:
                    ...
                    if current_volume is not None and avg_volume is not None and avg_volume > 0:
                        ...
                        if volume_ratio < 0.7:
                            vol_adjustment = min(0.12, (0.7 - volume_ratio) * 0.25)  # 🔧 УВЕЛИЧЕНО: до 0.12
                            market_adjustment -= vol_adjustment
                            logger.info("📊 [ADAPTIVE] Низкий объем (ratio=%.2f): снижаем пороги на %.3f",
                ...
                if ai_params and isinstance(ai_params, dict):
                    ...
                    if quality_params and isinstance(quality_params, dict):
                        ...
                        if adaptive_quality is not None:
                            ...
                            logger.debug("🤖 [AI ADAPTIVE] Используем AI-оптимизированный порог Quality: %.3f (mode=%s)",
            ...
            try:
                ...
                if recent_signals_count < 5:
                    ...
                    logger.info("📉 [ADAPTIVE] Адаптация по низкой активности для %s (<5 сигналов за час): -0.05", symbol)
            except Exception as e:
                ...
                market_adjustment -= 0.05
                logger.info("📉 [ADAPTIVE] Применяем снижение по умолчанию (ошибка подсчета): -0.05")
            ...
            if backtest_min_quality_long is not None:
                ...
                logger.debug("🔧 [BACKTEST] Используем переопределенный min_quality_threshold LONG: %.3f", min_quality_threshold)
            else:
                # 🔧 УЛУЧШЕНО: Повышен минимальный порог качества для LONG сигналов
                # Причина: Необходимо повысить качество сигналов и снизить количество убыточных сделок
                # Старое значение: 0.33 (33%) - слишком низкий порог
                # Новое значение: 0.50 (50%) - более строгие требования для лучшего качества
                # Оптимизировано на основе бэктеста (3 месяца, топ-20 монет, 15 потоков)
                # Результаты: Win Rate 68.81%, Profit Factor 1.29, Total Return +0.52%, Max Drawdown 0.37%
                # См. docs/FILTER_PARAMETERS_OPTIMIZATION_RESULTS.md
                min_quality_threshold = max(0.50, base_quality_threshold + market_adjustment)
            ...
            if backtest_min_confidence_long is not None:
                ...
                logger.debug("🔧 [BACKTEST] Используем переопределенный min_confidence_threshold LONG: %.3f", min_confidence_threshold)
            else:
                # 🔧 УЛУЧШЕНО: Повышен минимальный порог confidence для LONG сигналов
                # Старое значение: 0.40 (40%)
                # Новое значение: 0.50 (50%) - более строгие требования
                min_confidence_threshold = max(0.50, base_confidence_threshold + market_adjustment * 0.7)

            # 📊 ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ
            logger.info("🎯 [ADAPTIVE THRESHOLDS] %s: Quality=%.3f (base=%.3f, adjustment=%.3f), Confidence=%.3f "
            ...
            try:
                ...
                if dxy_trend == "BULLISH" and macro.get("strength") == "STRONG":
                    if signal_type == "LONG":
                        ...
                        logger.info("🌍 [MACRO] DXY Strong Bullish -> LONG quality threshold increased (+0.05)")
                ...
                # Если DXY медвежий (BEARISH), можно чуть ослабить для LONG
                    ...
                    if signal_type == "LONG":
                        ...
                        logger.info("🌍 [MACRO] DXY Bearish -> LONG quality threshold relaxed (-0.02)")
            except Exception as e:
                logger.debug("🌍 [MACRO] Ошибка получения данных DXY: %s", e)

            if quality_score < min_quality_threshold:
                user_id = user_data.get("user_id", "unknown")
                logger.warning("🚫 [FILTER BLOCK] %s (user=%s) LONG CLASSIC: Quality фильтр - Score %.3f < порог %.3f (base=%.3f, adjustment=%.3f)",
            ...
            if pattern_confidence < min_confidence_threshold:
                user_id = user_data.get("user_id", "unknown")
                logger.warning("🚫 [FILTER BLOCK] %s (user=%s) LONG CLASSIC: Confidence фильтр - Confidence %.3f < порог %.3f",
                             symbol, user_id, pattern_confidence, min_confidence_threshold)
                return None, None, None

            logger.info("✅ [QUALITY PASS] %s LONG CLASSIC: Quality %.3f >= %.3f, Confidence %.3f >= %.3f",
            ...
            if DEFENSE_SYSTEMS_AVAILABLE and volume_detector:
                try:
                    ...
                    if volume_quality < 0.8:  # Качество объема < 80%
                        logger.warning("🚫 [VOLUME BLOCK] %s LONG CLASSIC: Volume quality %.3f < 0.80 (манипуляции объемом)",
                                     symbol, volume_quality)
                        pipeline_monitor.log_stage('volume_quality', symbol, False, f"Volume quality {volume_quality:.3f} < 0.80")
                        return None, None, None
                    else:
                        logger.info("✅ [VOLUME PASS] %s LONG CLASSIC: Volume quality %.3f >= 0.80", symbol, volume_quality)
                except Exception as e:
            ...
            if FALSE_BREAKOUT_DETECTOR_AVAILABLE and false_breakout_detector:
                try:
                    ...
                    try:
                        ...
                        optimized_params = ml_optimizer.load_optimized_params()  # Используем публичный метод
                        ...
                        logger.debug("⚠️ [ML_FALSE_BREAKOUT] %s: ошибка ML оптимизации, используем стандартный порог: %s",
                                    symbol, ml_e)
                    ...
                    if breakout_analysis.get('is_false_breakout', False):
                        logger.warning("🚫 [BREAKOUT BLOCK] %s %s LONG CLASSIC: False breakout обнаружен (confidence: %.2f)",
                        ...
                        logger.info("✅ [BREAKOUT PASS] %s %s LONG CLASSIC: False breakout не обнаружен (confidence: %.2f)",
                                   symbol, signal_type, breakout_analysis.get('confidence', 1.0))
                except Exception as e:
                    logger.debug("⚠️ Ошибка false breakout detector для %s: %s (пропускаем проверку)", symbol, e)
            ...
            logger.info("🔍 [MTF CHECK] %s LONG CLASSIC: Проверка MTF confirmation (улучшенная логика)...", symbol)
            mtf_ok, mtf_error = await _run_mtf_confirmation_with_logging(symbol, signal_type, regime_data)
            logger.info("🔍 [MTF RESULT] %s LONG CLASSIC: MTF ok=%s, error=%s", symbol, mtf_ok, mtf_error)
            if not mtf_ok:
                user_id = user_data.get("user_id", "unknown")
                logger.warning("🚫 [MTF BLOCK] %s (user=%s) LONG CLASSIC: MTF confirmation не пройден: %s",
                             symbol, user_id, mtf_error or "MTF не подтвержден")
                pipeline_monitor.log_stage('mtf_confirmation', symbol, False, f"MTF блокировка: {mtf_error}")
                return None, None, None
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s LONG CLASSIC: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
            ...
            logger.info("✅ [SIGNAL GENERATED] %s LONG CLASSIC: Сигнал успешно сгенерирован! Quality=%.3f, Confidence=%.3f",
        ...
        # Альтернативный паттерн 1: EMA близко к кроссоверу + бычий бар + объем
        elif (ema_fast > ema_slow * 0.995 and  # EMA близко к кроссоверу
              current_price > df['open'].iloc[-1] and  # Бычий бар
              current_volume > avg_volume * 1.2):  # Объем выше среднего
            signal_type = "BUY"
            signal_price = current_price
            ...
            logger.debug("✅ %s: Quality %.2f, Confidence %.2f", symbol, quality_score, pattern_confidence)
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s LONG Alt-1: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
                pipeline_monitor.log_stage('ml_filter', symbol, False, ml_reason)
                return None, None, None
        ...
        # Альтернативный паттерн 2: Цена выше EMA + восходящий тренд + RSI не перекуплен
        elif (current_price > ema_fast and  # Цена выше быстрой EMA
              ema_fast > float(ema_fast_series.iloc[-2]) and  # Восходящий тренд EMA
              'rsi' in df.columns and df['rsi'].iloc[-1] < 70):  # RSI не перекуплен
            signal_type = "BUY"
            signal_price = current_price
            ...
            if not await check_all_trend_alignments(symbol, signal_type, df):
                return None, None, None
            # 🆕 ПРОВЕРКА НОВЫХ ФИЛЬТРОВ
            new_filters_passed, new_filters_reason = await check_new_filters(symbol, signal_type, signal_price, df, strict_mode=filter_mode == 'strict')
            if not new_filters_passed:
                logger.warning("🚫 [ALT-2] %s: Новые фильтры заблокировали: %s", symbol, new_filters_reason)
                return None, None, None
            # 🔧 ВРЕМЕННО ОТКЛЮЧЕН ДЛЯ СООТВЕТСТВИЯ УСПЕШНОМУ БЭКТЕСТУ
            # if not calculate_direction_confidence(
            #     df,
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s LONG Alt-2: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
                pipeline_monitor.log_stage('ml_filter', symbol, False, ml_reason)
                return None, None, None
        ...
        # Альтернативный паттерн 3: Отскок от поддержки + объем
        elif (current_price > df['low'].iloc[-1] * 1.001 and  # Отскок от минимума
              current_volume > avg_volume * 1.5 and  # Высокий объем
              'bb_lower' in df.columns and current_price > df['bb_lower'].iloc[-1]):  # Выше нижней BB
            signal_type = "BUY"
            signal_price = current_price
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s LONG Alt-3: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
                pipeline_monitor.log_stage('ml_filter', symbol, False, ml_reason)
                return None, None, None
        ...
        if use_new_short_entry_logic:
            ...
            if should_enter:
                ...
                logger.info("✅ [PULLBACK ENTRY SHORT] %s: Вход на откате к сопротивлению (Quality=%.2f, Regime=%s)",
                           symbol, entry_details.get("quality_score", 0), entry_details.get("market_regime", "UNKNOWN"))
                pipeline_monitor.log_stage('pullback_entry', symbol, True, f"Quality={entry_details.get('quality_score', 0):.2f}")
                pipeline_monitor.log_pattern_type(pattern_type)

                # 🔥 КРИТИЧЕСКИЕ ПРОВЕРКИ НАПРАВЛЕНИЯ СИГНАЛА
                logger.info("🔍 [PULLBACK ENTRY SHORT] %s: Проверка трендов (умная логика)...", symbol)
                if not await check_all_trend_alignments(symbol, signal_type, df):
                    logger.warning("🚫 [PULLBACK ENTRY SHORT] %s: Тренд alignment не пройден", symbol)
                    return None, None, None
                ...
                new_filters_passed, new_filters_reason = await check_new_filters(symbol, signal_type, current_price, df, strict_mode=filter_mode == 'strict')
                if not new_filters_passed:
                    logger.warning("🚫 [PULLBACK ENTRY SHORT] %s: Новые фильтры заблокировали: %s", symbol, new_filters_reason)
                    return None, None, None
                logger.info("✅ [PULLBACK ENTRY SHORT] %s: Новые фильтры пройдены (%s)", symbol, new_filters_reason)
                ...
                # Если новая логика не прошла, пробуем старую (EMA кроссовер) как fallback
                logger.debug("⏭️ [PULLBACK ENTRY SHORT] %s: Не прошел проверку (%s), пробуем EMA кроссовер",
                             symbol, entry_details.get("reason", "Unknown"))
        ...
        if not use_new_short_entry_logic and current_price < ema_fast and ema_fast < ema_slow:
            ...
            logger.info("🔍 [SHORT CLASSIC] %s: Обнаружен классический медвежий EMA кроссовер (цена=%.8f, ema_fast=%.8f, ema_slow=%.8f, режим=%s)",
                       symbol, current_price, ema_fast, ema_slow, trade_mode)
            pipeline_monitor.log_stage('ema_pattern', symbol, True, f"Классический медвежий EMA (режим: {trade_mode})")

            # Проверяем режим торговли - SHORT только для FUTURES
            if trade_mode != 'futures':
                logger.warning("🚫 [SHORT CLASSIC BLOCK] %s: SHORT сигнал пропущен (режим: %s, требуется: futures)", symbol, trade_mode)
                pipeline_monitor.log_stage('trade_mode_check', symbol, False, f"Режим {trade_mode} не позволяет SHORT")
                return None, None, None

            logger.info("✅ [SHORT CLASSIC] %s: Режим проверен (futures), продолжаем генерацию сигнала", symbol)
            ...
            new_filters_passed, new_filters_reason = await check_new_filters(symbol, signal_type, current_price, df, strict_mode=filter_mode == 'strict')
            if not new_filters_passed:
                logger.warning("🚫 [SHORT CLASSIC] %s: Новые фильтры заблокировали: %s", symbol, new_filters_reason)
                return None, None, None
            logger.info("✅ [SHORT CLASSIC] %s: Новые фильтры пройдены (%s)", symbol, new_filters_reason)
            ...
            logger.debug("⏭️ [SHORT CLASSIC] %s: Direction confidence временно отключен (как в успешном бэктесте)", symbol)

            logger.info("🔍 [SHORT CLASSIC] %s: Проверка RSI warning...", symbol)
            if not await check_rsi_warning(df, signal_type):
                logger.warning("🚫 [SHORT CLASSIC] %s: RSI warning не пройден (RSI в опасной зоне)", symbol)
                return None, None, None
            ...
            if backtest_min_quality_short is not None:
                ...
                logger.debug("🔧 [BACKTEST] Используем переопределенный min_quality_for_short: %.3f", min_quality_for_short)
            else:
                # 🔧 УЛУЧШЕНО: Повышены требования к качеству SHORT сигналов
                # Причина: Win Rate SHORT позиций всего 13.7% (очень низкий)
                # Старое значение: 0.45 (45%)
                # Новое значение: 0.60 (60%) - более строгие требования
                # См. docs/TRADING_BOT_ROOT_CAUSE_FOUND.md
                min_quality_for_short = 0.60
            ...
            if backtest_min_confidence_short is not None:
                ...
                logger.debug("🔧 [BACKTEST] Используем переопределенный min_confidence_for_short: %.3f", min_confidence_for_short)
            else:
                # 🔧 УЛУЧШЕНО: Повышены требования к уверенности SHORT сигналов
                # Причина: Win Rate SHORT позиций всего 13.7% (очень низкий)
                # Старое значение: 0.40 (40%)
                # Новое значение: 0.55 (55%) - более строгие требования
                # См. docs/TRADING_BOT_ROOT_CAUSE_FOUND.md
                min_confidence_for_short = 0.55

            logger.info("📊 [QUALITY CHECK] %s SHORT CLASSIC: Quality=%.3f (min=%.2f), Confidence=%.3f (min=%.2f)",
            ...
            try:
                ...
                if dxy_trend == "BEARISH":
                    ...
                    logger.info("🌍 [MACRO] DXY Bearish -> SHORT quality threshold increased (+0.05)")
            ...
            if quality_score < min_quality_for_short:
                logger.warning("🚫 [QUALITY BLOCK] %s SHORT CLASSIC: Quality score %.3f < %.2f",
            ...
            if pattern_confidence < min_confidence_for_short:
                logger.warning("🚫 [CONFIDENCE BLOCK] %s SHORT CLASSIC: Pattern confidence %.3f < %.2f",
            ...
            logger.info("✅ [QUALITY PASS] %s SHORT CLASSIC: Quality %.3f >= %.2f, Confidence %.3f >= %.2f",
                       symbol, quality_score, min_quality_for_short, pattern_confidence, min_confidence_for_short)
            ...
            if DEFENSE_SYSTEMS_AVAILABLE and volume_detector:
                try:
                    ...
                    logger.info("📊 [VOLUME QUALITY] %s SHORT: Volume quality=%.3f (min=%.2f)", symbol, volume_quality, min_volume_quality_for_short)
                    if volume_quality < min_volume_quality_for_short:
                        logger.warning("🚫 [VOLUME BLOCK] %s SHORT: Volume quality %.3f < %.2f",
                        ...
                        return None, None, None
                    logger.info("✅ [VOLUME PASS] %s SHORT: Volume quality %.3f >= %.2f", symbol, volume_quality, min_volume_quality_for_short)
                except Exception as e:
                    logger.warning("⚠️ [VOLUME ERROR] Ошибка проверки объема для %s: %s (пропускаем)", symbol, e)
            ...
            logger.info("🔍 [MTF CHECK] %s SHORT CLASSIC: Проверка MTF confirmation (улучшенная логика)...", symbol)
            mtf_ok, mtf_error = await _run_mtf_confirmation_with_logging(symbol, signal_type, regime_data)
            logger.info("🔍 [MTF RESULT] %s SHORT CLASSIC: MTF ok=%s, error=%s", symbol, mtf_ok, mtf_error)
            if not mtf_ok:
                logger.warning("🚫 [MTF BLOCK] %s SHORT CLASSIC: MTF confirmation не пройден: %s", symbol, mtf_error)
                return None, None, None
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s SHORT CLASSIC: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
                pipeline_monitor.log_stage('ml_filter', symbol, False, ml_reason)
                return None, None, None
            ...
            logger.info("✅ [SIGNAL GENERATED] %s SHORT CLASSIC: Сигнал успешно сгенерирован! Quality=%.3f, Confidence=%.3f",
                       symbol, quality_score, pattern_confidence)
        ...
        # Альтернативный SHORT паттерн 1: EMA близко к кроссоверу + медвежий бар + объем (только FUTURES)
        elif (ema_fast < ema_slow * 1.005 and  # EMA близко к кроссоверу (медвежий)
              current_price < df['open'].iloc[-1] and  # Медвежий бар
              current_volume > avg_volume * 1.2):  # Объем выше среднего
            # 🔍 ЛОГИРОВАНИЕ ДО ПРОВЕРКИ РЕЖИМА
            logger.info("🔍 [SHORT Alt-1] %s: Обнаружен альтернативный паттерн 1 (режим=%s)", symbol, trade_mode)
            pipeline_monitor.log_stage('ema_pattern', symbol, True, f"SHORT Альтернативный паттерн 1 (режим: {trade_mode})")

            # Проверяем режим торговли - SHORT только для FUTURES
            if trade_mode != 'futures':
                logger.warning("🚫 [SHORT Alt-1 BLOCK] %s: SHORT сигнал пропущен (режим: %s, требуется: futures)", symbol, trade_mode)
                pipeline_monitor.log_stage('trade_mode_check', symbol, False, f"Режим {trade_mode} не позволяет SHORT")
                return None, None, None

            logger.info("✅ [SHORT Alt-1] %s: Режим проверен (futures), продолжаем генерацию сигнала", symbol)
            signal_type = "SELL"
            signal_price = current_price
            pattern_type = "alternative_short_1"
            logger.debug("✅ %s: SHORT Альтернативный паттерн 1 (EMA близко + медвежий бар + объем)", symbol)
            pipeline_monitor.log_stage('ema_pattern', symbol, True, "SHORT Альтернативный паттерн 1")
            ...
            logger.debug("✅ %s: SHORT Quality %.2f, Confidence %.2f", symbol, quality_score, pattern_confidence)
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s SHORT Alt-1: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
                pipeline_monitor.log_stage('ml_filter', symbol, False, ml_reason)
                return None, None, None
        ...
        # Альтернативный SHORT паттерн 2: Цена ниже EMA + нисходящий тренд + RSI не перепродан (только FUTURES)
        elif (current_price < ema_fast and  # Цена ниже быстрой EMA
              ema_fast < float(ema_fast_series.iloc[-2]) and  # Нисходящий тренд EMA
              'rsi' in df.columns and df['rsi'].iloc[-1] > 30):  # RSI не перепродан
            # 🔍 ЛОГИРОВАНИЕ ДО ПРОВЕРКИ РЕЖИМА
            logger.info("🔍 [SHORT Alt-2] %s: Обнаружен альтернативный паттерн 2 (цена=%.8f, ema_fast=%.8f, RSI=%.2f, режим=%s)",
                       symbol, current_price, ema_fast, df['rsi'].iloc[-1] if 'rsi' in df.columns else 0, trade_mode)
            pipeline_monitor.log_stage('ema_pattern', symbol, True, f"SHORT Альтернативный паттерн 2 (режим: {trade_mode})")

            # Проверяем режим торговли - SHORT только для FUTURES
            if trade_mode != 'futures':
                logger.warning("🚫 [SHORT Alt-2 BLOCK] %s: SHORT сигнал пропущен (режим: %s, требуется: futures)", symbol, trade_mode)
                pipeline_monitor.log_stage('trade_mode_check', symbol, False, f"Режим {trade_mode} не позволяет SHORT")
                return None, None, None

            logger.info("✅ [SHORT Alt-2] %s: Режим проверен (futures), продолжаем генерацию сигнала", symbol)
            signal_type = "SELL"
            signal_price = current_price
            pattern_type = "alternative_short_2"
            logger.debug("✅ %s: SHORT Альтернативный паттерн 2 (цена < EMA + тренд + RSI)", symbol)
            pipeline_monitor.log_stage('ema_pattern', symbol, True, "SHORT Альтернативный паттерн 2")
            ...
            new_filters_passed, new_filters_reason = await check_new_filters(symbol, signal_type, signal_price, df, strict_mode=filter_mode == 'strict')
            if not new_filters_passed:
                logger.warning("🚫 [SHORT Alt-2] %s: Новые фильтры заблокировали: %s", symbol, new_filters_reason)
                return None, None, None
            ...
            logger.debug("⏭️ [SHORT Alt-2] %s: Direction confidence временно отключен (как в успешном бэктесте)", symbol)

            logger.debug("🔍 [SHORT Alt-2] %s: Проверка RSI warning...", symbol)
            if not await check_rsi_warning(df, signal_type):
                logger.warning("🚫 [SHORT Alt-2] %s: RSI warning не пройден (RSI в опасной зоне)", symbol)
                return None, None, None
            ...
            logger.info("📊 [QUALITY CHECK] %s SHORT Alt-2: Quality=%.3f (min=%.2f), Confidence=%.3f (min=%.2f)",
                       symbol, quality_score, min_quality_for_short, pattern_confidence, min_confidence_for_short)

            if quality_score < min_quality_for_short:
                logger.warning("🚫 [QUALITY BLOCK] %s SHORT Alt-2: Quality score %.3f < %.2f",
                             symbol, quality_score, min_quality_for_short)
                pipeline_monitor.log_stage('quality_check', symbol, False, f"Quality {quality_score:.3f} < {min_quality_for_short}")
                return None, None, None

            if pattern_confidence < min_confidence_for_short:
                logger.warning("🚫 [CONFIDENCE BLOCK] %s SHORT Alt-2: Pattern confidence %.3f < %.2f",
                             symbol, pattern_confidence, min_confidence_for_short)
                pipeline_monitor.log_stage('confidence_check', symbol, False, f"Confidence {pattern_confidence:.3f} < {min_confidence_for_short}")
                return None, None, None

            logger.info("✅ [QUALITY PASS] %s SHORT Alt-2: Quality %.3f >= %.2f, Confidence %.3f >= %.2f",
                       symbol, quality_score, min_quality_for_short, pattern_confidence, min_confidence_for_short)
            ...
            if DEFENSE_SYSTEMS_AVAILABLE and volume_detector:
                try:
                    volume_quality = volume_detector.get_volume_quality(df)
                    logger.info("📊 [VOLUME QUALITY] %s SHORT: Volume quality=%.3f (min=0.80)", symbol, volume_quality)
                    if volume_quality < 0.80:
                        logger.warning("🚫 [VOLUME BLOCK] %s SHORT: Volume quality %.3f < 0.80", symbol, volume_quality)
                        pipeline_monitor.log_stage('volume_quality', symbol, False, f"Volume quality {volume_quality:.3f} < 0.80")
                        return None, None, None
                    logger.info("✅ [VOLUME PASS] %s SHORT: Volume quality %.3f >= 0.80", symbol, volume_quality)
                except Exception as e:
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s SHORT Alt-2: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
                pipeline_monitor.log_stage('ml_filter', symbol, False, ml_reason)
                return None, None, None
            ...
            logger.info("✅ [SIGNAL GENERATED] %s SHORT Alt-2: Сигнал успешно сгенерирован! Quality=%.3f, Confidence=%.3f",
                       symbol, quality_score, pattern_confidence)
            pipeline_monitor.log_stage('final_signal', symbol, True, "SHORT сигнал сгенерирован")
            return signal_type, signal_price, _ml_prediction

        # Альтернативный SHORT паттерн 3: Отскок от сопротивления + объем (только FUTURES)
        elif (current_price < df['high'].iloc[-1] * 0.999 and  # Отскок от максимума (вниз)
              current_volume > avg_volume * 1.5 and  # Высокий объем
              'bb_upper' in df.columns and current_price < df['bb_upper'].iloc[-1]):  # Ниже верхней BB
            # 🔍 ЛОГИРОВАНИЕ ДО ПРОВЕРКИ РЕЖИМА
            logger.info("🔍 [SHORT Alt-3] %s: Обнаружен альтернативный паттерн 3 (режим=%s)", symbol, trade_mode)
            pipeline_monitor.log_stage('ema_pattern', symbol, True, f"SHORT Альтернативный паттерн 3 (режим: {trade_mode})")

            # Проверяем режим торговли - SHORT только для FUTURES
            if trade_mode != 'futures':
                logger.warning("🚫 [SHORT Alt-3 BLOCK] %s: SHORT сигнал пропущен (режим: %s, требуется: futures)", symbol, trade_mode)
                pipeline_monitor.log_stage('trade_mode_check', symbol, False, f"Режим {trade_mode} не позволяет SHORT")
                return None, None, None

            logger.info("✅ [SHORT Alt-3] %s: Режим проверен (futures), продолжаем генерацию сигнала", symbol)
            signal_type = "SELL"
            signal_price = current_price
            pattern_type = "alternative_short_3"
            logger.debug("✅ %s: SHORT Альтернативный паттерн 3 (отскок от сопротивления + объем + BB)", symbol)
            pipeline_monitor.log_stage('ema_pattern', symbol, True, "SHORT Альтернативный паттерн 3")
            ...
            if not ml_passed:
                logger.warning("🚫 [ML BLOCK] %s SHORT Alt-3: ML фильтр заблокировал сигнал: %s", symbol, ml_reason)
                pipeline_monitor.log_stage('ml_filter', symbol, False, ml_reason)
                return None, None, None
...
async def send_signal(
    ...
    logger.info("📨 [SEND_SIGNAL START] %s %s @ %.8f для пользователя %s (mode=%s)",
    ...
    if agent_prompt:
        ...
        if get_context_engine is not None:
            try:
                ...
                logger.debug("⚠️ ContextEngine недоступен: %s (продолжаем без обогащенного контекста)", e)
                enriched_context = {}
        ...
        logger.debug("📝 [PROMPT] signal_live v%s загружен (%d символов, контекст: %s)",
    ...
    if ENTRY_TIMING_OPTIMIZER_AVAILABLE and entry_timing_optimizer and df is not None:
        try:
            ...
            if entry_timing_result['strategy'] == 'retracement':
                logger.info("📍 [ENTRY TIMING] %s: рекомендован откат до %.8f (было: %.8f)",
                ...
                logger.info("📍 [ENTRY TIMING] %s: рекомендовано ждать подтверждения (%d мин)",
    ...
    try:
        ...
        if USE_CORRELATION_RISK and CORRELATION_MANAGER_AVAILABLE and correlation_manager:
            try:
                ...
                if not risk_check['allowed']:
                    logger.warning("🚫 [SEND_SIGNAL BLOCK] %s %s: Корреляционный риск заблокирован - %s",
                    ...
                    logger.debug("✅ [CORRELATION] %s %s разрешен: %s", symbol, signal_type, risk_check.get('details', 'OK'))
                    if trace is not None:
                        trace.record(
                            step="observe",
        ...
        if signal_was_sent_earlier:
            logger.warning("🚫 [SEND_SIGNAL BLOCK] %s %s: Сигнал уже был отправлен ранее", symbol, signal_type)
            ...
            logger.info("✅ [SEND_SIGNAL] %s %s: Сигнал уже был отправлен ранее - считаем успешной отправкой для автоисполнения", symbol, signal_type)
            if trace is not None:
                trace.record(
                    step="observe",
        ...
        logger.info("🔍 [%s] Сигнал сгенерирован: %s %s %.4f",
        ...
        if TELEGRAM_INTEGRATION_AVAILABLE:
            try:
                ...
                try:
                    ...
                    if df_for_tp is not None and len(df_for_tp) > 0:
                        ...
                        if AI_SL_OPTIMIZER_AVAILABLE and AI_SL_OPTIMIZER:
                            try:
                                ...
                                try:
                                    ...
                                    logger.warning("⚠️ [%s] Динамический SL также недоступен: %s, используем 2%%", symbol, e2)
                                    sl_pct_positive = 2.0
                        else:
                            ...
                            try:
                                ...
                                logger.info("🛡️ [%s] Динамический SL: %.2f%%", symbol, sl_pct_positive)
                            except Exception as e:
                                logger.warning("⚠️ [%s] Ошибка расчета динамического SL: %s, используем 2%%", symbol, e)
                                sl_pct_positive = 2.0
                            ...
                            try:
                                ...
                                if fibonacci_calculator:
                                    ...
                                    if zone_details.get("method") == "fibonacci":
                                        ...
                                        logger.info("🎯 [%s] TP/SL от Фибоначчи: TP1=%.2f%%, TP2=%.2f%%, SL=%.2f%%",
                                    ...
                                    try:
                                        ...
                                        if zones:
                                            ...
                                            if zone_details.get("method") == "interest_zones":
                                                ...
                                                if abs(tp1_pct_zone - tp1_pct) > 0.1:  # Если разница значительная
                                                    tp1_pct = tp1_pct_zone
                                                    tp2_pct = tp2_pct_zone
                                                    sl_pct_positive = sl_pct_zone
                                                    logger.info("🎯 [%s] TP/SL от Interest Zones: TP1=%.2f%%, TP2=%.2f%%, SL=%.2f%%",
                                                               symbol, tp1_pct, tp2_pct, sl_pct_positive)
                                    except Exception as e:
                                        logger.debug("⚠️ [%s] Ошибка расчета TP/SL от Interest Zones: %s", symbol, e)

                            except Exception as e:
                                logger.warning("⚠️ [%s] Ошибка расчета динамических TP/SL от зон: %s, используем базовые", symbol, e)
                        ...
                        if signal_type.upper() == "BUY":
                            ...
                            # 🔧 УЛУЧШЕНО: Увеличен SL для SHORT позиций
                            # Причина: SL срабатывает слишком часто (64 сделки, -1247.61 USDT)
                            # Увеличиваем SL на 50% для SHORT (было sl_pct_optimized, стало * 1.5)
                            sl_pct_optimized_short = sl_pct_optimized * 1.5  # Увеличиваем на 50%
                            tp1_price = signal_price * (1 - tp1_pct_optimized / 100)
                            tp2_price = signal_price * (1 - tp2_pct_optimized / 100)
                            sl_price = signal_price * (1 + sl_pct_optimized_short / 100)
                            sl_pct = -sl_pct_optimized_short  # Отрицательный для отображения

                        logger.debug("🎯 [%s] Оптимизированные TP/SL: TP1=%.2f%%, TP2=%.2f%%, SL=%.2f%% (было: TP1=%.2f%%, TP2=%.2f%%, SL=%.2f%%)",
                        ...
                        if signal_type.upper() == "BUY":
                            ...
                            # 🔧 УЛУЧШЕНО: Увеличен SL для SHORT позиций (значения по умолчанию)
                            # Причина: SL срабатывает слишком часто
                            # Старое значение: 2% (1.02)
                            # Новое значение: 3% (1.03) - больше места для волатильности
                            tp1_price = signal_price * 0.9725
                            tp2_price = signal_price * 0.952
                            sl_price = signal_price * 1.03  # Увеличено с 1.02 до 1.03 (3% вместо 2%)
                        sl_pct = -3.0  # Обновлено с -2.0 до -3.0
                except Exception as e:
                    ...
                    if signal_type.upper() == "BUY":
                        ...
                        # 🔧 УЛУЧШЕНО: Увеличен SL для SHORT позиций (fallback значения)
                        # Причина: SL срабатывает слишком часто
                        # Старое значение: 2% (1.02)
                        # Новое значение: 3% (1.03) - больше места для волатильности
                        tp1_price = signal_price * 0.9725
                ...
                try:
                    ...
                    logger.warning("⚠️ [%s] ИИ-оптимизация недоступна: %s, используем базовые параметры", symbol, e)
                ...
                if RISK_MANAGER_AVAILABLE and risk_manager:
                    try:
                        ...
                        conf_score = composite_result.get('confidence', 0.5) if composite_result else 0.5
                        ml_conf = ml_prediction.get('success_probability') if ml_prediction else None
                        current_regime = regime_data.get('regime', 'NORMAL') if regime_data else 'NORMAL'
                        ...
                        if intelligent_size:
                            ...
                            sizing_audit["kelly_fraction"] = float(intelligent_size.get('kelly_fraction', 0))

                            logger.info(
                                "🧠 [%s] Intelligent Sizing (Kelly): сумма=%.2f USDT, риск=%.2f%% (conf=%.2f, ml=%s, regime=%s)",
                                symbol, entry_amount_usdt, risk_pct, conf_score, ml_conf, current_regime
                            )
                    except Exception as e:
                ...
                if CORRELATION_MANAGER_AVAILABLE and correlation_manager:
                    try:
                        ...
                        if correlation_multiplier < 1.0:
                            logger.info("📉 [PENALTY] %s: сумма %.2f → %.2f USDT (x%.2f) - %s",
                ...
                if ADAPTIVE_SIZER_AVAILABLE and adaptive_sizer:
                    try:
                        ...
                        logger.info("⚖️ [ADAPTIVE] %s: сумма %.2f → %.2f USDT (x%.2f) - %s",
                ...
                if PORTFOLIO_RISK_MANAGER_AVAILABLE and portfolio_risk_manager:
                    try:
                        ...
                        if risk_adjusted_size < entry_amount_usdt:
                            logger.info("📉 [PORTFOLIO RISK] %s: размер скорректирован %.2f → %.2f USDT (risk score: %.2f)",
                ...
                if entry_amount_usdt and entry_amount_usdt < min_allowed_order:
                    logger.warning("📉 [%s] Сигнал ОТМЕНЕН: сумма входа %.2f USDT меньше минимума %.2f USDT",
                                 symbol, entry_amount_usdt, min_allowed_order)
                    if trace is not None:
                        trace.finish(status="error", metadata={"reason": "micro_order", "amount": entry_amount_usdt})
                    return False
                    ...
                    try:
                        ...
                        if 'macd' in df.columns and 'macd_signal' in df.columns:
                            ...
                            if not pd.isna(macd) and not pd.isna(macd_signal):
                                technical_data["macd_status"] = "Бычий" if macd > macd_signal else "Медвежий"
                            else:
                    ...
                    if HYBRID_DATA_MANAGER_AVAILABLE and HYBRID_DATA_MANAGER:
                        ...
                        if btc_df is not None:
                            ...
                            if isinstance(btc_df, pd.DataFrame) and not btc_df.empty and len(btc_df) >= 50:
                                ...
                                btc_trend_status = btc_ema_fast > btc_ema_slow  # True = бычий, False = медвежий
                                logger.debug("✅ [BTC TREND] %s: Реальный тренд BTC = %s (EMA fast=%.2f, slow=%.2f)",
                                           symbol, "🟢 БЫЧИЙ" if btc_trend_status else "🔴 МЕДВЕЖИЙ", btc_ema_fast, btc_ema_slow)
                            else:
                                # Fallback: если данных недостаточно, используем значение из check_btc_alignment
                                btc_trend_status = None
                                logger.debug("⚠️ [BTC TREND] %s: Недостаточно данных BTC для определения тренда", symbol)
                        else:
                            btc_trend_status = None
                ...
                if ml_prediction and ml_prediction.get('success_probability'):
                    ...
                    logger.info("🤖 [CONFIDENCE] Используем уверенность из ML модели: %.2f%%", ai_confidence)
                else:
                    try:
                        if 'rsi' in technical_data and technical_data['rsi'] != 50.0:
                            ...
                            volume_confidence = 95 if "высокий" in technical_data['volume_status'].lower() else 80
                ...
                try:
                    ...
                    logger.debug("✅ %s: MTF накопление рассчитано: %s", symbol, mtf_accumulation_line)
                except Exception as mtf_calc_error:
                    logger.warning("⚠️ Ошибка расчета MTF накопления для %s: %s", symbol, mtf_calc_error)
                ...
                if entry_amount_usdt is not None and entry_amount_usdt < 5.0:
                    logger.warning("🚫 [SEND_SIGNAL BLOCK] %s: Сумма входа %.2f USDT меньше минимума (5.0 USDT)",
                                 symbol, entry_amount_usdt)
                    return False
                ...
                if signal_was_sent_earlier:
                    ...
                    logger.info("⏭️ [SEND_SIGNAL] %s %s: Пропускаем повторную отправку (уже был отправлен ранее), продолжаем для автоисполнения", symbol, signal_type)
                else:
                    ...
                    if ENHANCED_DELIVERY_AVAILABLE:
                        ...
                        if success:
                            ...
                            try:
                                ...
                                logger.debug("📡 [COORD] Событие SIGNAL_GENERATED опубликовано для %s", symbol)
                            except Exception as coord_exc:
                                logger.debug("⚠️ Ошибка координации: %s", coord_exc)
                        else:
                            logger.warning("⚠️ Не удалось отправить сигнал пользователю %s (enhanced)",
                ...
                if not signal_was_sent_earlier:
                    ...
                    logger.info("✅ [SEND_SIGNAL] %s %s: Ошибка отправки, но сигнал уже был отправлен ранее - сохраняем signal_sent_successfully=True", symbol, signal_type)
                if trace is not None:
                    trace.record(
                        step="act",
                        name="telegram_delivery",
            ...
            # 🔧 ИСПРАВЛЕНО: Не сбрасываем signal_sent_successfully, если сигнал уже был отправлен ранее
            if not signal_was_sent_earlier:
                signal_sent_successfully = False  # 🆕 КРИТИЧНО: Telegram отключен = сигнал НЕ отправлен
            else:
                logger.info("✅ [SEND_SIGNAL] %s %s: Telegram отключен, но сигнал уже был отправлен ранее - сохраняем signal_sent_successfully=True", symbol, signal_type)
            if trace is not None:
                trace.record(
                    step="act",
        ...
        logger.info("💾 [DB SAVE START] Начало сохранения сигнала %s %s в БД (price=%.8f, user=%s)",
        ...
        try:
            ...
            logger.info("💾 [DB SAVE DEBUG] user_id_str=%s, chat_id_int=%s, message_id_result=%s, signal_sent=%s",
            ...
            if SIGNAL_ACCEPTANCE_AVAILABLE and signal_acceptance_manager:
                try:
                    ...
                    if message_id_result and chat_id_int:
                        logger.info("💾 [DB SAVE] Сохранение в accepted_signals с message_id=%s, chat_id=%s",
                        ...
                        logger.info("✅ [DB SAVE SUCCESS] Сигнал %s сохранен в accepted_signals (status: pending, message_id=%s)",
                                   symbol, message_id_result)
                    elif chat_id_int:
                        # Сохраняем без message_id (fallback)
                        logger.warning("⚠️ [DB SAVE] Сохранение БЕЗ message_id (message_id=%s, chat_id=%s) - используем fallback",
                                      message_id_result, chat_id_int)
                        ...
                        logger.info("✅ [DB SAVE SUCCESS] Сигнал %s сохранен в accepted_signals БЕЗ message_id (fallback)",
                                   symbol)
                    else:
                        logger.error("❌ [DB SAVE FAILED] Невозможно сохранить %s: chat_id_int=%s, message_id=%s, user_id_str=%s",
                                    symbol, chat_id_int, message_id_result, user_id_str)
                        logger.error("❌ [DB SAVE FAILED] Детали: user_data.user_id=%s, signal_sent=%s",
                                    user_data.get("user_id"), signal_sent_successfully)

                except Exception as e:
                    logger.error("❌ [DB SAVE ERROR] Ошибка сохранения %s в accepted_signals: %s", symbol, e, exc_info=True)
                    import traceback
                    logger.error("❌ [DB SAVE ERROR] Traceback: %s", traceback.format_exc())
            else:
                logger.warning("⚠️ [DB SAVE] Signal acceptance manager недоступен (SIGNAL_ACCEPTANCE_AVAILABLE=%s)",
            ...
            try:
                ...
                logger.info("💾 [DB SAVE] Данные для signals_log: symbol=%s, entry=%.8f, user_id=%s, quality_score=%s",
                ...
                logger.info("✅ [DB SAVE SUCCESS] Сигнал %s сохранен в signals_log (result: PENDING, entry_time=%s)",
                           symbol, entry_time_str)
            except Exception as e:
                logger.error("❌ [DB SAVE ERROR] Ошибка сохранения %s в signals_log: %s", symbol, e, exc_info=True)
                import traceback
                logger.error("❌ [DB SAVE ERROR] Traceback: %s", traceback.format_exc())

        except Exception as e:
            logger.error("❌ [DB SAVE CRITICAL ERROR] Критическая ошибка сохранения сигнала %s в БД: %s", symbol, e, exc_info=True)
            import traceback
        ...
        if signal_sent_successfully:
            logger.info("✅ [PRODUCTION] %s %s отправлен пользователю %s", symbol, signal_type, user_data.get('user_id', 'N/A'))
        else:
            logger.warning("⚠️ [PRODUCTION] %s %s НЕ был отправлен пользователю %s (signal_sent_successfully=False)", symbol, signal_type, user_data.get('user_id', 'N/A'))
        ...
        try:
            ...
            logger.info("🔍 [AUTO CHECK] Начало проверки автоисполнения для %s (user_id=%s, signal_sent=%s, message_id=%s)",
                       symbol, user_id_local, signal_sent_successfully, message_id_result)

            if user_id_local:
                mode = await adb.get_user_mode(int(user_id_local))
                logger.info("🔍 [AUTO CHECK] %s: режим пользователя = %s (env=%s)", symbol, mode, ATRA_ENV)
                ...
                if mode == 'auto':
                    logger.info("🔍 [AUTO CHECK] %s: режим пользователя = auto, проверяем условия автоисполнения", symbol)
                    ...
                    logger.debug("🔍 [AUTO CHECK] %s: signal_sent_successfully = %s, message_id_result = %s",
                    ...
                    logger.info("✅ [AUTO CHECK] %s: сигнал успешно отправлен (signal_sent_successfully=True), продолжаем автоисполнение", symbol)
                    ...
                    if actual_entry_amount is None or actual_entry_amount <= 0:
                        ...
                        logger.warning("⚠️ [AUTO] %s: entry_amount_usdt не установлена выше, используем расчет: %.2f USDT", symbol, actual_entry_amount)
                    ...
                    if actual_leverage is None or actual_leverage <= 0:
                        ...
                        logger.warning("⚠️ [AUTO] %s: leverage не установлена выше, используем значение из user_data: %.1fx", symbol, actual_leverage)
                    ...
                    logger.info("🤖 [AUTO] %s: запуск автоисполнения для user %s", symbol, user_id_local)
                    logger.info("🤖 [AUTO] %s: параметры - баланс=%.2f, сумма=%.2f USDT, плечо=%.1fx, цена=%.8f, режим=%s",
                    ...
                    try:
                        ...
                        if actual_trade_mode == 'spot' and signal_type.upper() == 'SHORT':
                            logger.warning("❌ [AUTO] %s: SHORT сигнал заблокирован для spot режима (доступен только LONG)", symbol)
                            return  # Пропускаем автоисполнение, сигнал уже отправлен

                        logger.info("🤖 [AUTO] %s: Вызов execute_and_open... (сумма=%.2f USDT, плечо=%dx)", symbol, actual_entry_amount, actual_leverage)
                        ...
                        if success:
                            ...
                            try:
                                ...
                                if any(p.get('symbol', '').upper() == symbol.upper() for p in positions):
                                    ...
                                    logger.warning("⚠️ [AUTO] %s: Позиция не найдена в БД после открытия! Проверьте логи auto_execution.", symbol)
                            except Exception as pos_check_exc:
                                logger.debug("⚠️ [AUTO] %s: Ошибка проверки позиции в БД: %s", symbol, pos_check_exc)
                        else:
                            logger.warning("❌ [AUTO] %s не удалось открыть автоматически. Проверьте логи auto_execution для деталей.", symbol)

                    except Exception as e:
                        logger.error("❌ [AUTO] Ошибка автоисполнения %s: %s", symbol, e, exc_info=True)
                else:
                    logger.info("👤 [MANUAL] %s: пользователь в ручном режиме (mode=%s) - авто-исполнение НЕ будет вызвано", symbol, mode)
            else:
        ...
        if signal_sent_successfully:
            logger.info("✅ [SEND_SIGNAL SUCCESS] %s %s: Сигнал успешно отправлен пользователю %s (источник: send_signal в signal_live.py)",
                       symbol, signal_type, user_data.get("user_id"))
            if trace is not None:
                trace.finish(status="success")
        else:  # pylint: disable=unreachable
            logger.warning("⚠️ [SEND_SIGNAL FAILED] %s %s: Сигнал НЕ был отправлен пользователю %s (signal_sent_successfully=False)",
                          symbol, signal_type, user_data.get("user_id"))
            if trace is not None:
        ...
        logger.error("❌ [SEND_SIGNAL ERROR] КРИТИЧЕСКАЯ ОШИБКА в send_signal для %s %s: %s",
    ...
    try:
        for signal in signal_history:
            if (signal.get("symbol") == symbol and
                ...
                time.time() - signal.get("timestamp", 0) < 300):  # 5 минут
                logger.debug("🚫 Сигнал для %s уже был отправлен пользователю %s недавно.", symbol, user_id)
                return True
        return False
    except Exception as e:
...
def check_ai_volume_filter(df: pd.DataFrame, ai_params: Dict[str, Any]) -> bool:
    ...
    if passed:
        logger.debug("📊 [VOLUME PASS] %s: объем %.0f в норме",
                    df.attrs.get('symbol', 'UNKNOWN') if hasattr(df, 'attrs') else 'UNKNOWN',
                    current_volume)
    else:
        logger.debug("📊 [VOLUME BLOCK] %s: %s",
                    df.attrs.get('symbol', 'UNKNOWN') if hasattr(df, 'attrs') else 'UNKNOWN',
                    reason)
...
async def check_ml_filter(
    ...
    """
    # ML фильтр: включён для объяснимости факторов ИИ. Условия отключения/включения: docs/SIGNALS_TODO_REENABLE.md
    USE_ML_FILTER = True
    ...
    if not LIGHTGBM_AVAILABLE or not lightgbm_predictor:
        logger.debug("🔍 [ML CHECK] ML фильтр отключен: LIGHTGBM_AVAILABLE=%s, predictor=%s",
    ...
    try:
        ...
        if 'volatility' in df.columns:
            ...
            if volatility_value > 0.5:
                logger.warning("⚠️ [ML VOLATILITY] %s: volatility очень высокий (%.4f)! Проверьте расчет.", symbol, volatility_value)
        elif regime_data:
            volatility_pct = regime_data.get('volatility_pct', 0.0)
            market_conditions['volatility'] = float(volatility_pct)
            logger.debug("📊 [ML VOLATILITY] %s: volatility из regime_data: %.4f", symbol, volatility_pct)
        else:
        ...
        try:
            if len(df) >= 3:
                ...
                logger.info("✅ [ML LAG FEATURES] %s: Вычислены lag features из DataFrame: %s",
                          symbol, list(historical_indicators.keys())[:10])
        except Exception as e:
            logger.warning("⚠️ [ML LAG FEATURES] %s: Ошибка вычисления lag features: %s (используем fallback)", symbol, e)
...
async def check_new_filters(
    ...
    if not strict_mode:
        logger.debug("🔓 [check_new_filters] %s: Используется МЯГКИЙ РЕЖИМ (ослабленная фильтрация)", symbol)
    ...
    if dominance_filter:
        try:
            ...
            if not strict_mode:
                logger.debug("🔓 [check_new_filters] %s: Пропускаем DominanceTrendFilter в мягком режиме", symbol)
            else:
            ...
            logger.warning(
                "⚠️ Ошибка DominanceTrendFilter для %s: %s (пропускаем фильтр)",
                symbol, e
            )
    ...
    if interest_zone_filter:
        try:
            ...
            if not strict_mode:
                logger.debug("🔓 [check_new_filters] %s: Пропускаем InterestZoneFilter в мягком режиме", symbol)
            else:
            ...
            logger.warning(
                "⚠️ Ошибка InterestZoneFilter для %s: %s (пропускаем фильтр)",
                symbol, e
            )
    ...
    if fibonacci_filter:
        try:
            ...
            logger.warning(
                "⚠️ Ошибка FibonacciZoneFilter для %s: %s (пропускаем фильтр)",
                symbol, e
            )
    ...
    if volume_imbalance_filter and USE_VOLUME_IMBALANCE_FILTER:
        try:
            ...
            if volume_imbalance_result is None:
                logger.warning(
                    "⚠️ [VolumeImbalance] %s: Фильтр вернул None (пропускаем проверку)",
                    symbol
                )
    ...
    if INSTITUTIONAL_PATTERNS_FILTER_AVAILABLE and check_institutional_patterns_filter:
        try:
            if USE_INSTITUTIONAL_PATTERNS_FILTER and len(df) > 0:
                ...
                if not ip_ok:
                    logger.info(
                        "📊 [InstitutionalPatterns] %s: Блокировка - %s",
                        symbol, ip_reason
                    )
                    return False, f"InstitutionalPatternsFilter: {ip_reason}"
        except Exception as e:
            logger.warning(
                "⚠️ Ошибка InstitutionalPatternsFilter для %s: %s (пропускаем фильтр)",
                symbol, e
            )
...
async def check_all_trend_alignments(symbol: str, signal_type: str, df: Any = None) -> bool:
    ...
    if SMART_TREND_FILTER_AVAILABLE and smart_trend_filter:
        try:
            ...
            logger.warning("⚠️ [TREND_CHECK] %s: ошибка SmartTrendFilter: %s, используем fallback", symbol, e)
            # Fallback на старую логику

    # Fallback: проверяем все три тренда (старая логика)
    logger.debug("⚠️ [TREND_CHECK] %s: SmartTrendFilter недоступен, проверяем все три тренда", symbol)
...
async def check_btc_alignment(symbol: str, signal_type: str) -> bool:
    ...
    try:
        ...
        if trend_strength < min_trend_strength:
            ...
            logger.debug("✅ [BTC FILTER] %s: слабый тренд (%.3f%% < %.3f%%) - разрешаем торговлю в боковике",
        ...
        if signal_type == "BUY" and btc_trend == "SELL":
            if trend_strength > strong_trend_threshold:
                logger.warning("🚫 [BTC FILTER] %s: LONG против сильного BTC тренда (strength=%.3f%%) - блокируем",
                ...
                logger.debug("⚠️ [BTC FILTER] %s: LONG против слабого BTC тренда (strength=%.3f%%) - разрешаем",
                           symbol, trend_strength * 100)
                return True

        if signal_type == "SELL" and btc_trend == "BUY":
            if trend_strength > strong_trend_threshold:
                logger.warning("🚫 [BTC FILTER] %s: SHORT против сильного BTC тренда (strength=%.3f%%) - блокируем",
                             symbol, trend_strength * 100)
                # Логируем блокировку
                try:
                    from src.utils.filter_logger import log_filter_check_async  # type: ignore
                ...
                logger.debug("⚠️ [BTC FILTER] %s: SHORT против слабого BTC тренда (strength=%.3f%%) - разрешаем",
                           symbol, trend_strength * 100)
                return True

        logger.debug("✅ [BTC FILTER] %s: тренд совпадает с BTC (%s, strength=%.3f%%)",
...
def calculate_direction_confidence(
    ...
    try:
        ...
        if not result:
            ...
            logger.warning("🚫 [DIRECTION CHECK] %s (%s): недостаточно подтверждений (%d/4, требуется %d). Отсутствуют: %s",
                         signal_type, trade_mode, confirmations, min_confirmations, ", ".join(missing_checks) if missing_checks else "неизвестно")
        else:
            logger.info("✅ [DIRECTION CHECK] %s (%s): %d/4 подтверждений (требуется %d)", signal_type, trade_mode, confirmations, min_confirmations)
...
def check_ai_volatility_filter(df: pd.DataFrame, ai_params: Dict[str, Any]) -> bool:
    ...
    logger.debug("📊 Волатильность: текущая=%.2f%%, диапазон=[%.2f%%, %.2f%%]",
                current_volatility, min_volatility_pct, max_volatility_pct)

    result = min_volatility_pct <= current_volatility <= max_volatility_pct

    if not result:
        logger.info("❌ [%s] Волатильность ОТКЛОНЕНА: текущая=%.4f (%.2f%%), диапазон=[%.4f-%.4f] ([%.2f%%-%.2f%%])",
    ...
    try:
        from src.utils.filter_logger import log_filter_check_async
        symbol = df.attrs.get('symbol', 'UNKNOWN') if hasattr(df, 'attrs') else 'UNKNOWN'
        reason = None if result else f"Волатильность {current_volatility:.2f}% вне диапазона [{min_volatility_pct:.2f}%, {max_volatility_pct:.2f}%]"
        # asyncio.create_task(log_filter_check_async(
        #     symbol=symbol,
        #     filter_type='ai_volatility',
        #     passed=result,
        ...
        try:
            ...
            if data_ok and circles_count is not None:
                if circles_count > 0:
                    ...
                    _ = get_anomaly_emoji(circles_count / 5.0)  # emoji - не используется в текущей версии
                    ...
                    logger.info("[FVG] %s: обнаружены аномалии уровня %d - %s",
...
async def _run_hybrid_signal_system_fixed_impl():
    ...
    while True:
        ...
        try:
            ...
            logger.debug("🔍 [CYCLE DEBUG] Цикл #%d начат в %s", cycle_count, time.strftime("%Y-%m-%d %H:%M:%S"))

            if RISK_FLAGS_AVAILABLE and risk_flags_manager:
                if risk_flags_manager.is_active("emergency_stop"):
                    logger.warning("🚫 Цикл #%d: emergency_stop активен, пропускаем генерацию сигналов.", cycle_count)
                    await asyncio.sleep(60)
                    continue
                if risk_flags_manager.is_active("weak_setup_stop"):
                    logger.warning("🚫 Цикл #%d: weak_setup_stop активен, пропускаем генерацию сигналов.", cycle_count)
                    await asyncio.sleep(60)
                    continue
            ...
            if REGIME_DETECTOR_AVAILABLE and regime_detector:
                try:
                    ...
                    if btc_df is not None and len(btc_df) >= 200:
                        ...
                        logger.info("📊 Рыночный режим: %s (уверенность: %.0f%%)",
            ...
            user_data_dict = await load_user_data()
            logger.info("🔍 [CYCLE DEBUG] Загружено пользователей: %d", len(user_data_dict) if user_data_dict else 0)
            if not user_data_dict:
                logger.warning("⚠️ Нет данных пользователей для отправки сигналов")
                await asyncio.sleep(60)
                continue

            logger.debug("🔄 [REFRESH] Загружено %d пользователей (баланс обновлён)", len(user_data_dict))
            ...
            logger.info("📊 Анализируем %d символов для %d пользователей", len(symbols), len(user_data_dict))
            ...
            if cycle_count % 5 == 0:
                ...
                logger.info("📊 HEALTH CHECK: Очередь %d/%d, TTL %ds",
    ...
    while True:
        try:
            ...
            if health['status'] != 'HEALTHY':
                ...
                logger.debug("✅ Система корреляций здорова: %s сигналов в истории", health.get('signal_history_count', 0))
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, json=data, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    ...
                    logger.warning("⚠️ Не удалось отправить алерт администратору: статус %d", response.status)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "cannot run an event loop while another event loop is running" in str(e):
            logger.warning("Обнаружен запущенный event loop. "
                         "Запускаем систему в текущем loop.")
