...
try:
    ...
    try:
        ...
        logger.warning(
            "BB_DIR_NEAR_MID_EPSILON_STRICT не найден, используется значение по умолчанию: 0.07"
        )
...
def strict_entry_signal(df: pd.DataFrame, i: int) -> Tuple[Optional[str], Optional[Decimal]]:
    ...
    try:
        ...
        symbol_name = (
            df.attrs.get("symbol") or df.columns[0].split("_")[0] if not df.empty else "UNKNOWN"
        )
        ...
        if not (long_base_ok or short_base_ok):
            ...
            if not long_base_ok:
                failed = [
                    name
                    for j, name in enumerate(["BB", "EMA", "RSI", "VOL_R", "VOLAT", "MOM", "TREND"])
                    if not long_conditions[j]
                ]
                log_filter_check(
                    symbol_name,
                    "strict_base_long",
                    ...
                    {"score": sum(long_conditions)},
                )
            if not short_base_ok:
                failed = [
                    name
                    for j, name in enumerate(["BB", "EMA", "RSI", "VOL_R", "VOLAT", "MOM", "TREND"])
                    if not short_conditions[j]
                ]
                log_filter_check(
                    symbol_name,
                    "strict_base_short",
...
def get_entry_signal_by_mode(
    df: pd.DataFrame, i: int, mode: str = "strict"
