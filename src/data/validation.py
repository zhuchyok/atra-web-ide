...
class PriceValidator(DataValidator):
    ...
    def validate_timestamp(
        self, timestamp: Union[str, int, float, datetime]
...
class VolumeValidator(DataValidator):
    ...
    def validate_volume_change(self, current: float, previous: float) -> bool:
        ...
        if change_pct > 1000:  # 10x изменение
            self.add_warning(
                "volume_change", f"Very large volume change: {change_pct:.1f}%", change_pct
            )

        return True

    def validate_market_cap(
        self, market_cap: Union[float, int, str]
        ...
        try:
            ...
            if market_cap_float > 1e15:  # > 1 квадриллион
                self.add_warning(
                    "market_cap",
                    f"Very large market cap: ${market_cap_float:,.0f}",
                    market_cap_float,
                )
