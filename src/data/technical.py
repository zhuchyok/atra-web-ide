...
Использует Rust для высокопроизводительных вычислений (10-100x быстрее Python)
"""
...
class TechnicalIndicators:
    ...
    def calculate_rsi(prices: List[float], period: int = 14) -> Optional[float]:
        ...
        try:
            ...
            if rust_accelerator and rust_accelerator.available:
                rsi_values = rust_accelerator.calculate_rsi(prices, period)
    ...
    def calculate_momentum(prices: List[float], period: int = 10) -> Optional[float]:
        ...
        try:
            ...
            # Используем Rust если доступен
            if rust_accelerator and rust_accelerator.available:
                momentum_values = rust_accelerator.calculate_momentum(prices, period)
                if momentum_values and len(momentum_values) > 0:
                    momentum = momentum_values[-1]
                    return round(float(momentum), 2) if momentum != 0.0 else None
                return None
    ...
    def calculate_volume_ratio(current_volume: float, avg_volume: float) -> Optional[float]:
        ...
        try:
            ...
            # Используем Rust если доступен (для списков)
            # Для одиночных значений используем прямое вычисление
            if rust_accelerator and rust_accelerator.available:
                # Rust версия принимает списки, но для совместимости используем прямое вычисление
    ...
    def calculate_fear_greed_index(prices: List[float], volumes: List[float]) -> Optional[float]:
        ...
        try:
            ...
            # Используем Rust если доступен
            if rust_accelerator and rust_accelerator.available:
                index_values = rust_accelerator.calculate_fear_greed_index(prices, volumes)
                if index_values and len(index_values) > 0:
                    return round(float(index_values[-1]), 2)
                return None
