...
try:
    from ai_filter_optimizer import AIFilterOptimizer, get_filter_optimizer
...
if AI_FILTER_OPTIMIZER_AVAILABLE and _optimizer_instance:
    ...
    class FilterOptimizerWrapper:
        ...
        async def get_optimized_filter_params(self, symbol: str) -> Optional[Dict[str, Any]]:
            ...
            try:
                ...
                if params:
                    return {"symbol": symbol, "parameters": params, "source": "ai_filter_optimizer"}
                return None
            except Exception as e:
        ...
        async def optimize_strategy(
            self, symbol: str, signals_data: list, optimization_config: Dict[str, Any]
                ...
                try:
                    ...
                    if loop.is_running():
                        ...
                        performance = loop.run_until_complete(
                            self._optimizer.get_recent_performance()
                        )
        ...
        async def optimize_filter_system(
            self,
    ...
    logger.info(
        "✅ filter_optimizer обёртка создана из ai_filter_optimizer (реальная функциональность)"
    )
    ...
    class FilterOptimizerStub:
        ...
        async def optimize_strategy(self, symbol, signals_data, optimization_config):
            logger.warning(
                f"⚠️ Попытка оптимизировать стратегию для {symbol}, но оптимизатор недоступен"
            )
            return None

        def run_optimization_cycle(self, symbol):
            logger.warning(
                f"⚠️ Попытка запустить цикл оптимизации для {symbol}, но оптимизатор недоступен"
            )
            return {
                "status": "unavailable",
                "message": "ai_filter_optimizer module not available",
        ...
        def get_filter_performance_report(self):
            logger.warning(
                "⚠️ Попытка получить отчёт о производительности, но оптимизатор недоступен"
            )
            return {"status": "unavailable", "message": "ai_filter_optimizer module not available"}
