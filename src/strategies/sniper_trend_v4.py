...
class SniperTrendStrategy(BaseStrategy):
    ...
    async def analyze(self, symbol: str, df: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if (
                close > last_ema4h
                and rsi > self.rsi_long
                and atr > atr_sma
                and close > Decimal(str(prev["high"]))
            ):
                side = "BUY"
            elif (
                close < last_ema4h
                and rsi < self.rsi_short
                and atr > atr_sma
                and close < Decimal(str(prev["low"]))
            ):
                side = "SELL"
            ...
            if not risk_check["allowed"]:
                logger.warning(
                    f"⚠️ {self.name}: Сигнал {side} по {symbol} отклонен риск-менеджером: {risk_check['reason']}"
                )
                return None
