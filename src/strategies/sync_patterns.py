...
def main():
    ...
    if win_count == 0 or loss_count == 0:
        ...
        if os.path.exists(patterns_file):
            ...
            if len(existing_backups) >= 5:
                for old_backup in existing_backups[4:]:  # Оставляем 5, удаляем остальные
                    try:
                        ...
                        logger.debug("Удален старый бэкап паттернов: %s", os.path.basename(old_backup))
                    except Exception:
                        pass
            ...
            if existing_backups:
                ...
                if hours_since_backup < 6:
                    should_backup = False
                    logger.debug("Пропускаем бэкап паттернов (последний был %.1f часов назад)", hours_since_backup)
        ...
        if win_count >= 50 and loss_count >= 50:  # Минимум для обучения
            ...
            logger.warning("⚠️ Недостаточно данных для переобучения (нужно минимум 50 WIN и 50 LOSS)")
