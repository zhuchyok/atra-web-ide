            ...
            try:
                ...
                try:
                    ...
                    if not correlation_check.get("allowed", True):
                        ...
                        trace.finish(
                            status="error", metadata={"reason": "correlation_risk_blocked"}
                        )
                        trace_completed = True
                        return False

                    logger.info(
                        "✅ [AUTO] %s: корреляционная проверка пройдена (%s)",
                        symbol,
                        correlation_check.get("reason", "NO_RISK"),
                    )
                    trace.record(
                        step="observe",
                        name="correlation_check_passed",
                        metadata={"reason": correlation_check.get("reason", "NO_RISK")},
                    )

                    # Сохраняем сигнал в историю для будущих проверок
                    ...
                    logger.error(
                        "❌ [AUTO] %s: ошибка проверки корреляции: %s", symbol, correlation_exc
                    )
                    # В случае ошибки проверки корреляции - пропускаем проверку (не блокируем торговлю)
                    logger.warning(
                        "⚠️ [AUTO] %s: продолжаем без проверки корреляции из-за ошибки", symbol
                    )

                # 4) Валидация размера позиции
