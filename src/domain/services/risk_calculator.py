...
from ...core.contracts import invariant, postcondition, precondition
...
class RiskCalculator:
    ...
    @precondition(
        lambda account_balance, risk_percentage, entry_price, stop_loss: (
            ...
            and Decimal("0.1") <= risk_percentage <= Decimal("10.0")
            and entry_price.value > Decimal("0")
            and stop_loss.value > Decimal("0")
        ),
    ...
    @postcondition(
        lambda result, account_balance, risk_percentage, entry_price, stop_loss: (
            result >= Decimal("0")
            and result * entry_price.value
            <= account_balance * Decimal("2")  # Позиция не должна превышать 2x баланса
        ),
    ...
    @staticmethod
    @precondition(
        lambda positions: positions is not None, "Invalid input: positions must not be None"
    )
    @postcondition(
        lambda result, positions: (
            result >= Decimal("0") and result <= Decimal("100")  # Риск не может превышать 100%
        ),
