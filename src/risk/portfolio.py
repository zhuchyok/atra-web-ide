...
class PortfolioRiskManager:
        ...
        try:
            ...
            if self._risk_flags:
                if self._risk_flags.is_active("emergency_stop"):
                    logger.warning("🚨 [PORTFOLIO RISK] emergency_stop активен — блокируем открытие.")
                    return {
                        'allowed': False,
                ...
                if self._risk_flags.is_active("weak_setup_stop"):
                    logger.warning("🚨 [PORTFOLIO RISK] weak_setup_stop активен — блокируем открытие.")
                    return {
                        'allowed': False,
            ...
            if (
                ...
                logger.warning("🚨 [PORTFOLIO RISK] Достигнут лимит просадки портфеля: %.2f%% >= %.2f%%",
            ...
            if daily_loss_pct >= self.risk_limits['max_daily_loss_pct']:
                ...
                logger.warning("🚨 [PORTFOLIO RISK] Достигнут дневной лимит убытков: %.2f%% >= %.2f%%",
            ...
            if self.portfolio_metrics.open_positions_count >= self.risk_limits['max_open_positions']:
                ...
                logger.warning("🚨 [PORTFOLIO RISK] Достигнут лимит открытых позиций: %d >= %d",
            ...
            if position_pct > self.risk_limits['max_capital_per_position_pct']:
                ...
                logger.warning("🚨 [PORTFOLIO RISK] Размер позиции превышает лимит: %.2f%% > %.2f%%",
            ...
            # Позиция разрешена
            logger.debug("✅ [PORTFOLIO RISK] Позиция разрешена (risk score: %.2f, drawdown: %.2f%%, позиций: %d)",
        ...
        try:
            ...
            if self.db:
                try:
                    ...
                    logger.debug("⚠️ [PORTFOLIO RISK] Ошибка получения данных из trades/signals_log: %s", db_err)
    ...
    def get_position_size_adjustment(self, base_size_usdt: float) -> float:
        ...
        try:
            ...
            if multiplier < 1.0:
                logger.info("📉 [PORTFOLIO RISK] Размер позиции скорректирован: %.2f → %.2f USDT (risk score: %.2f)",
    ...
    def _get_system_setting(self, key: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug("⚠️ [PORTFOLIO RISK] Не удалось прочитать system_settings[%s]: %s", key, err)
            return None
    ...
    def _set_system_setting(self, key: str, value: str) -> None:
        ...
        try:
            ...
            logger.debug("⚠️ [PORTFOLIO RISK] Не удалось сохранить system_settings[%s]: %s", key, err)
...
async def check_position_auto_close(
    ...
    try:
        ...
        if pnl_pct <= -CRITICAL_LOSS_PER_POSITION_PCT:
            logger.warning(
                "🚨 [CRITICAL LOSS] %s: PnL=%.2f%% <= -%.1f%%, "
                "требуется немедленное закрытие!",
