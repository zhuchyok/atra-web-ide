        ...
        try:
            ...
            if win_rate is not None:
                if win_rate < 0.60:  # Низкий Win Rate → УЖЕСТОЧАЕМ
                    ...
                    logger.info(
                        "🔒 [ADAPTIVE RSI] Низкий WR (%.1f%%) → УЖЕСТОЧАЕМ: LONG=%.1f, SHORT=%.1f",
                        win_rate * 100,
                        rsi_long,
                        rsi_short,
                    )
                    ...
                    logger.info(
                        "✅ [ADAPTIVE RSI] Высокий WR (%.1f%%) + PF (%.2f) → Слегка ослабляем: LONG=%.1f, SHORT=%.1f",
                        win_rate * 100,
                        profit_factor,
                        rsi_long,
                        rsi_short,
            ...
            if market_volatility is not None:
                if market_volatility > 4.0:  # Высокая волатильность → УЖЕСТОЧАЕМ
                    ...
                    rsi_short += 2.0  # 30 → 32
                    logger.debug(
                        "📈 [ADAPTIVE RSI] Высокая волатильность → УЖЕСТОЧАЕМ: LONG=%.1f, SHORT=%.1f",
                        rsi_long,
                        rsi_short,
                    )
                    ...
                    logger.debug(
                        "📉 [ADAPTIVE RSI] Низкая волатильность → Стандартные пороги: LONG=%.1f, SHORT=%.1f",
                        rsi_long,
                        rsi_short,
                    )
            ...
            if volume_ratio is not None:
                if (
                    volume_ratio < 0.5
                ):  # Очень низкий объем → УЖЕСТОЧАЕМ (избегаем неликвидных сигналов)
                    rsi_long -= 1.0
                    rsi_short += 1.0
                    logger.debug(
                        "📊 [ADAPTIVE RSI] Очень низкий объем → УЖЕСТОЧАЕМ: LONG=%.1f, SHORT=%.1f",
                        rsi_long,
                        rsi_short,
                    )
                    ...
                    rsi_short += 0.5
                    logger.debug(
                        "📊 [ADAPTIVE RSI] Высокий объем → Слегка ужесточаем: LONG=%.1f, SHORT=%.1f",
                        rsi_long,
                        rsi_short,
                    )
        ...
        try:
            ...
            if win_rate is not None:
                if win_rate < 0.60:  # Низкий Win Rate → УЖЕСТОЧАЕМ
                    ...
                    logger.info(
                        "🔒 [ADAPTIVE VOLUME] Низкий WR (%.1f%%) → УЖЕСТОЧАЕМ: ratio=%.2f (mode=%s)",
                        win_rate * 100,
                        volume_ratio,
                        filter_mode,
                    )
                    ...
                    logger.info(
                        "✅ [ADAPTIVE VOLUME] Высокий WR (%.1f%%) + PF (%.2f) → Слегка ослабляем: ratio=%.2f (mode=%s)",
                        win_rate * 100,
                        profit_factor,
                        volume_ratio,
                        filter_mode,
            ...
            if market_volatility is not None:
                if market_volatility > 4.0:  # Высокая волатильность → УЖЕСТОЧАЕМ
                    ...
                    logger.debug(
                        "📈 [ADAPTIVE VOLUME] Высокая волатильность → УЖЕСТОЧАЕМ: ratio=%.2f (mode=%s)",
                        volume_ratio,
                        filter_mode,
                    )
                elif market_volatility < 1.0:  # Низкая волатильность → Стандартные пороги
                    # Не ослабляем, оставляем базовые значения
                    logger.debug(
                        "📉 [ADAPTIVE VOLUME] Низкая волатильность → Стандартные пороги: ratio=%.2f (mode=%s)",
                        volume_ratio,
                        filter_mode,
                    )
                    ...
                    if optimized_params:
                        ...
                        if ai_volume_ratio:
                            ...
                            if win_rate and win_rate < 0.60:
                                ...
                                )  # Не ослабляем при низком WR
                            else:
                                volume_ratio = ai_volume_ratio
                            logger.debug(
                                "🤖 [ADAPTIVE VOLUME] Используем AI-оптимизированный: %.2f",
                                volume_ratio,
                            )
                except Exception as e:
        ...
        try:
            ...
            if win_rate is not None:
                if win_rate < 0.60:  # Низкий Win Rate → УЖЕСТОЧАЕМ
                    ...
                    logger.info(
                        "🔒 [ADAPTIVE QUALITY] Низкий WR (%.1f%%) → УЖЕСТОЧАЕМ: score=%.3f",
                        win_rate * 100,
                        quality_score,
                    )
                elif win_rate > 0.75 and profit_factor and profit_factor > 2.0:
                    ...
                    logger.info(
                        "✅ [ADAPTIVE QUALITY] Высокий WR (%.1f%%) + PF (%.2f) → Слегка ослабляем: score=%.3f",
                        win_rate * 100,
                        profit_factor,
                        quality_score,
                    )
            ...
            if market_volatility is not None:
                if market_volatility > 4.0:  # Высокая волатильность → УЖЕСТОЧАЕМ
                    ...
                    logger.debug(
                        "📈 [ADAPTIVE QUALITY] Высокая волатильность → УЖЕСТОЧАЕМ: score=%.3f",
                        quality_score,
                    )
                elif market_volatility < 1.0:  # Низкая волатильность → Стандартные пороги
                    # Не ослабляем, оставляем базовые значения
                    logger.debug(
                        "📉 [ADAPTIVE QUALITY] Низкая волатильность → Стандартные пороги: score=%.3f",
                        quality_score,
                    )
            ...
            if volume_ratio is not None:
                if volume_ratio < 0.5:  # Очень низкий объем → УЖЕСТОЧАЕМ
                    quality_score += 0.02
                    logger.debug(
                        "📊 [ADAPTIVE QUALITY] Очень низкий объем → УЖЕСТОЧАЕМ: score=%.3f",
                        quality_score,
                    )
                        ...
                        if quality_params:
                            ...
                            if adaptive_quality:
                                ...
                                if win_rate and win_rate < 0.60:
                                    ...
                                    quality_score = adaptive_quality
                                logger.debug(
                                    "🤖 [ADAPTIVE QUALITY] Используем AI-оптимизированный: %.3f",
                                    quality_score,
                                )
                except Exception as e:
            ...
            # Ограничиваем диапазон
            quality_score = max(
                self.quality_score_range[0], min(self.quality_score_range[1], quality_score)
            )
                    ...
                    for line in f:
                        try:
                            ...
                            if "RSI для LONG на 15%" in idea:
                                ...
                                logger.info(
                                    "🧪 [RESEARCH] Применено смягчение RSI для LONG (Research Lab)"
                                )
                            elif "Quality Score для LONG до 0.85" in idea:
                                self.base_quality_score = 0.85
                                logger.info(
                                    "🧪 [RESEARCH] Применено увеличение Quality Score (Research Lab)"
                                )
    ...
    async def update_from_ai_optimization(self, metrics: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if optimized_params and "soft_volume_ratio" in optimized_params:
                ...
                logger.info(
                    "🤖 [ADAPTIVE] Обновлен base_volume_ratio: %.2f", self.base_volume_ratio
                )
