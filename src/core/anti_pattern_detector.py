...
class AntiPatternDetector:
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern in critical_operations:
                ...
                for match in matches:
                    ...
                    if (
                        f"if {var_name} is None" not in context
                        and f"if {var_name} is not None" not in context
                        and f"assert {var_name} is not None" not in context
                    ):
                        patterns.append(
                            AntiPattern(
    ...
    def check_comparison_with_none(
                ...
                for match in matches:
                    ...
                    patterns.append(
                        AntiPattern(
                            ...
                            severity="info",
                            code_snippet=line.strip(),
                            suggestion=f"Use: {var_name} is None"
                            if op == "=="
                            else f"Use: {var_name} is not None",
                        )
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for pattern_type in AntiPatternType:
            by_type[pattern_type.value] = sum(
                1 for p in self._detected_patterns if p.pattern_type == pattern_type
            )

        by_severity = {}
        for severity in ["info", "warning", "error", "critical"]:
            by_severity[severity] = sum(
                1 for p in self._detected_patterns if p.severity == severity
            )

        return {"total_patterns": total, "by_type": by_type, "by_severity": by_severity}
