...
class StateMachineValidator:
    ...
    def register_state_machine(self, class_name: str, rules: List[StateTransitionRule]) -> None:
        """
    ...
    def validate_transition(
        self, obj: Any, from_state: Any, to_state: Any, context: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
        ...
        for rule in matching_rules:
            if to_state in rule.to_states:
                ...
                transition = StateTransition(
                    from_state=from_state, to_state=to_state, context=context
                )
                self._transition_history[class_name].append(transition)

                logger.debug(f"Valid transition for {class_name}: {from_state} -> {to_state}")
                return True
...
def valid_transition(from_state: Any, to_states: List[Any], condition: Optional[Callable] = None):
    """
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if new_state != current_state:
                ...
                validator.validate_transition(
                    self, current_state, new_state, context={"function": func.__name__}
                )
