...
from src.core.state_machine import StateMachineValidator, StateTransitionRule, get_state_validator
...
def register_state_machines():
    ...
    order_rules = [
        ...
        StateTransitionRule(from_state=OrderStatus.FILLED, to_states=set()),
        StateTransitionRule(from_state=OrderStatus.CANCELLED, to_states=set()),
        StateTransitionRule(from_state=OrderStatus.REJECTED, to_states=set()),
    ]
    ...
    position_rules = [
        ...
        StateTransitionRule(
            from_state=PositionStatus.PARTIALLY_CLOSED, to_states={PositionStatus.CLOSED}
        ),
        # Финальное состояние - переходы не разрешены
        StateTransitionRule(from_state=PositionStatus.CLOSED, to_states=set()),
    ]
