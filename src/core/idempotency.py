...
class IdempotencyManager:
    ...
    def save(self, key: str, result: Any, ttl_hours: Optional[int] = None) -> None:
        ...
        idempotency_key = IdempotencyKey(
            key=key, created_at=datetime.now(timezone.utc), result=result, expires_at=expires_at
        )
    ...
    def cleanup_expired(self) -> int:
        ...
        """
        expired_keys = [
            key for key, idempotency_key in self._keys.items() if idempotency_key.is_expired()
        ]
