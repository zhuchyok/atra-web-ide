...
class ConfigValidator:
    ...
    def add_rule(
        ...
        rule = ValidationRule(
            field_name=field_name, rule_type=rule_type, message=message, validator=validator
        )
    ...
    def validate(self, config: Any) -> ConfigValidationResult:
        ...
        for rule in rules:
            ...
            # Проверка диапазона
                ...
                if not rule.validator(value):
                    errors.append(f"{field_name}: {rule.message or 'Value out of range'}")

            # Кастомная валидация
            elif rule.rule_type == "custom" and rule.validator:
                try:
                    if not rule.validator(value, config):
                        errors.append(f"{field_name}: {rule.message or 'Validation failed'}")
        ...
        return ConfigValidationResult(is_valid=len(errors) == 0, errors=errors, warnings=warnings)

    def _check_type(self, value: Any, expected_type: Type) -> bool:
        """Проверка типа значения"""
        # Обработка Optional
        if get_origin(expected_type) is Optional or (
            hasattr(expected_type, "__origin__") and expected_type.__origin__ is type(None)
        ):
    ...
    def decorator(cls: Type) -> Type:
        ...
        if required_fields:
            for field_name in required_fields:
                validator.add_rule(cls, field_name, "required", f"Field '{field_name}' is required")
