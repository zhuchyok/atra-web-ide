    ...
    try:
        ...
        try:
            ...
            if isinstance(result, list):
                ...
                serializable_result = [
                    list(row) if isinstance(row, tuple) else row for row in result
                ]
...
def invalidate_cache_pattern(pattern: str) -> int:
    ...
    try:
        ...
        if deleted_count > 0:
            logger.info(
                "✅ [Redis] Инвалидировано %d ключей по паттерну: %s", deleted_count, pattern
            )
...
def get_cache_stats() -> Dict[str, Any]:
    ...
    try:
        redis_client = get_redis_client()
        if not redis_client:
            return {"enabled": False, "message": "Redis недоступен"}
