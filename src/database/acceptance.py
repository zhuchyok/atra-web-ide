...
class AcceptanceDatabase:
    ...
    async def close_active_position_by_symbol(
        self,
        user_id: int,
        symbol: str,
        exit_price: float = None,
        exit_reason: str = "CLOSED_BY_SYNC",
    ) -> bool:
        ...
        position_query = """
            ...
            LIMIT 1
        """
        position_rows = await self._execute_with_retry(
            position_query, (user_id, symbol), is_write=False
        )
        ...
        if result and position_data["entry_time"]:
            try:
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    "⚠️ Ошибка обновления signals_log при закрытии позиции %s: %s", symbol, e
                )
