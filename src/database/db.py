...
def backup_file(filepath, backup_dir=BACKUP_DIR, max_backups: int = 10):
    """
    Создает резервную копию файла с временной меткой и ротацией старых бэкапов
    ...
    try:
        ...
        try:
            ...
            backup_pattern = str(Path(backup_dir) / f"{base_name}_*")
            import glob
            ...
            if len(existing_backups) > max_backups:
                for old_backup in existing_backups[max_backups:]:
                    try:
                        ...
                        logging.debug(f"🗑️ Удален старый бэкап: {old_backup}")
                    except Exception as e:
                        logging.warning(f"⚠️ Не удалось удалить старый бэкап {old_backup}: {e}")
        except Exception as e:
            logging.debug(f"⚠️ Ошибка ротации бэкапов: {e}")
...
class Database:
    ...
    def _init_tables(self):
        ...
        # Таблицы для сессионной модели стратегий (концепция из agent.md)
        self.cursor.execute(
            """
            CREATE TABLE IF NOT EXISTS strategy_sessions (
                ...
                title TEXT NOT NULL,
                description TEXT,
                status TEXT NOT NULL CHECK (status IN ('discovery', 'planning', 'decomposing', 'executing', 'done', 'archived')),
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
            )
            """
        ...
        self.cursor.execute(
            ...
            CREATE TABLE IF NOT EXISTS strategy_plans (
                id TEXT PRIMARY KEY,
                session_id TEXT NOT NULL,
                level TEXT NOT NULL CHECK (level IN ('master', 'sub', 'subsub')),
                parent_plan_id TEXT,
                role_hint TEXT,
                title TEXT NOT NULL,
                markdown_body TEXT NOT NULL,
                status TEXT NOT NULL CHECK (status IN ('active', 'in_progress', 'completed', 'obsolete', 'archived')),
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (session_id) REFERENCES strategy_sessions(id) ON DELETE CASCADE,
                FOREIGN KEY (parent_plan_id) REFERENCES strategy_plans(id) ON DELETE SET NULL
            )
            """
        )
        ...
        self.cursor.execute(
            ...
            CREATE TABLE IF NOT EXISTS strategy_actions (
                ...
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (session_id) REFERENCES strategy_sessions(id) ON DELETE CASCADE,
                FOREIGN KEY (plan_id) REFERENCES strategy_plans(id) ON DELETE SET NULL
            )
            """
        )
        
        # Индексы для сессионных таблиц
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_strategy_sessions_status ON strategy_sessions(status)")
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_strategy_questions_session_id ON strategy_questions(session_id)")
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_strategy_plans_session_id ON strategy_plans(session_id)")
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_strategy_plans_parent_id ON strategy_plans(parent_plan_id)")
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_strategy_plans_status ON strategy_plans(status)")
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_strategy_actions_session_id ON strategy_actions(session_id)")
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_strategy_actions_plan_id ON strategy_actions(plan_id)")
    ...
    _last_backup_ts = 0
    def periodic_backup(self, min_interval_sec: int = 21600):
        """
        Выполняет периодическое резервное копирование базы данных.
        
        По умолчанию: каждые 6 часов (21600 секунд) вместо 10 минут.
        Это уменьшает нагрузку на диск с 144 бэкапов/день до 4 бэкапов/день.
        """
        now = time.time()
        if now - Database._last_backup_ts >= min_interval_sec:
            backup_file(self.db_path, max_backups=10)  # Храним только последние 10
