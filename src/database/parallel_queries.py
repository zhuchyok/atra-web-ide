...
async def execute_queries_parallel(
    db, queries: List[Tuple[str, Tuple]], max_workers: int = 4
) -> List[Any]:
    """
    ...
    async def execute_single_query(query: str, params: Tuple):
        ...
        return await asyncio.to_thread(
            db.execute_with_retry, query, params, is_write=False, max_retries=3, use_cache=True
        )
...
async def get_multiple_user_data_parallel(
    ...
    queries = [
        ("SELECT data FROM users_data WHERE user_id=?", (str(user_id),)) for user_id in user_ids
    ]
