...
try:
    from src.filters.smart_trend_filter import (
        get_smart_trend_filter,  # pylint: disable=ungrouped-imports
    )
...
async def calculate_anomaly_circles_with_fallback(symbol: str, signal_type: str) -> tuple:
    ...
    try:
        ...
        if volume_24h < min_volume_threshold:
            logger.warning(
                "[Anomaly] %s: низкий объем торгов $%.0f < $5M — монета 'стоит'", symbol, volume_24h
            )
...
try:
    ...
    if not asyncio.iscoroutinefunction(get_filtered_top_usdt_pairs_fast):
        logger.warning(
            "⚠️ get_filtered_top_usdt_pairs_fast не является async функцией, устанавливаем None"
        )
    ...
    try:
        ...
        if not asyncio.iscoroutinefunction(get_filtered_top_usdt_pairs_fast):
            logger.warning(
                "⚠️ get_filtered_top_usdt_pairs_fast из exchange_api не является async функцией, устанавливаем None"
            )
            get_filtered_top_usdt_pairs_fast = None
    except ImportError:
        get_filtered_top_usdt_pairs_fast = None
...
try:
    # pylint: disable=ungrouped-imports
    from src.bot_telegram.enhanced_delivery import (
    ...
    try:
        if len(df) < 50:
            logger.warning(
                ...
                "требуется минимум 50 свечей. Получено: %d",
                len(df),
            )
            return df
        ...
        logger.debug(
            "✅ Добавлены технические индикаторы (ускорение Rust: %s)", RUST_INDICATORS_AVAILABLE
        )
        return df
    except Exception as e:
...
async def _generate_signal_impl(
    ...
    try:
        ...
        score = calculate_ai_signal_score(
            df, ai_params_data, symbol
        )  # Передаем полный ai_params_data
        ...
        # Альтернативный паттерн 2: Цена выше EMA + восходящий тренд + RSI не перекуплен
            ...
            # 🆕 ПРОВЕРКА НОВЫХ ФИЛЬТРОВ
            new_filters_passed, new_filters_reason = await check_new_filters(
                symbol, signal_type, signal_price, df
            )
            if not new_filters_passed:
                logger.warning(
        ...
        if TELEGRAM_INTEGRATION_AVAILABLE:
            try:
                ...
                try:
                    ...
                    if trade_mode == "futures":
                        try:
                            ...
                            leverage_raw = get_dynamic_leverage(
                                df,
                                len(df) - 1,
                                base_leverage,
                                symbol,
                ...
                if regime_multipliers:
                    ...
                    logger.info(
                        "🎛️ [%s] Режим %s: базовая сумма %.2f → %.2f USDT (x%.2f)",
                        symbol,
                        regime_data.get("regime", "N/A"),
                        base_entry_amount,
                        entry_amount_usdt,
                        regime_multiplier_used,
                    )
                ...
                try:
                    if "rsi" in technical_data and technical_data["rsi"] != 50.0:
                        ...
                        ai_confidence = (
                            rsi_confidence * 0.4 + macd_confidence * 0.3 + volume_confidence * 0.3
                        )
        ...
        try:
            ...
            if user_id_local:
                ...
                if mode == "auto":
                    ...
                    logger.info(
                        "🤖 [AUTO] %s: параметры - баланс=%.2f, сумма=%.2f USDT, плечо=%dx, цена=%.8f, режим=%s",
                        symbol,
...
def check_ai_volume_filter(df: pd.DataFrame, ai_params: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        log_filter_check_async(symbol=symbol, filter_type="ai_volume", passed=passed, reason=reason)
    except (ImportError, Exception):
...
async def check_new_filters(
    symbol: str, signal_type: str, entry_price: float, df: pd.DataFrame
) -> Tuple[bool, Optional[str]]:
    """
        ...
        try:
            ...
            if LOGGING_AVAILABLE:
                log_filter_check_async(
                    ...
                    passed=dominance_result.passed
                    if hasattr(dominance_result, "passed")
                    else bool(dominance_result),
        ...
        try:
            ...
            if LOGGING_AVAILABLE:
                log_filter_check_async(
                    ...
                    passed=interest_zone_result.passed
                    if hasattr(interest_zone_result, "passed")
                    else bool(interest_zone_result),
                    reason=interest_zone_result.reason
                    if hasattr(interest_zone_result, "reason")
                    else None,
                )
        ...
        try:
            ...
            if LOGGING_AVAILABLE:
                log_filter_check_async(
                    ...
                    passed=fibonacci_result.passed
                    if hasattr(fibonacci_result, "passed")
                    else bool(fibonacci_result),
        ...
        try:
            ...
            if LOGGING_AVAILABLE and volume_imbalance_result is not None:
                log_filter_check_async(
                    ...
                    passed=volume_imbalance_result.passed
                    if hasattr(volume_imbalance_result, "passed")
                    else bool(volume_imbalance_result),
                    reason=volume_imbalance_result.reason
                    if hasattr(volume_imbalance_result, "reason")
                    else None,
                )
...
def check_rsi_warning(df: pd.DataFrame, signal_type: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            log_filter_check_async(
                symbol=symbol, filter_type="rsi_warning", passed=passed, reason=reason
            )
        except (ImportError, Exception):
            pass  # Логирование недоступно, продолжаем
...
def check_ai_volatility_filter(df: pd.DataFrame, ai_params: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        log_filter_check_async(
            symbol=symbol, filter_type="ai_volatility", passed=result, reason=reason
        )
    except (ImportError, Exception):
        pass  # Логирование недоступно, продолжаем
