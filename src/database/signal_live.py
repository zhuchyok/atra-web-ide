...
try:
    ...
    # Если agent_identity недоступен, не используем заглушку
    authorize_agent_action = None
    logger = logging.getLogger(__name__)
    if not logger.handlers:
        logger.addHandler(logging.StreamHandler())
    logger.warning("⚠️ observability.agent_identity недоступен, авторизация агента отключена")

# 🔧 ИСПРАВЛЕНО: Импортируем каждый модуль отдельно, чтобы если один не работает, остальные работали
get_guidance = None
get_lm_judge = None
get_tracer = None
...
try:
    ...
    # observability.guidance недоступен, используем None из инициализации выше
    pass

try:
    from observability.lm_judge import get_lm_judge
except (ImportError, AttributeError, Exception):
    # observability.lm_judge недоступен, используем None из инициализации выше
    pass

try:
    from observability.tracing import get_tracer
except (ImportError, AttributeError, Exception):
    # observability.tracing недоступен, используем None из инициализации выше
    pass

try:
    from observability.prompt_manager import get_prompt_manager
except (ImportError, AttributeError, Exception):
    # observability.prompt_manager недоступен, используем None из инициализации выше
    pass

try:
    from observability.context_engine import get_context_engine
except (ImportError, AttributeError, Exception):
    # observability.context_engine недоступен, используем None из инициализации выше
    pass
...
def _log_mtf_event(
    ...
    try:
        if _mtf_event_db is None:
            from src.database.db import Database  # локальный импорт, чтобы избежать циклов
...
def _log_position_sizing_event(event: Dict[str, Any]) -> None:
    ...
    try:
        if _sizing_audit_db is None:
            from src.database.db import Database  # локальный импорт для избежания циклов
...
async def calculate_anomaly_circles_with_fallback(symbol: str, signal_type: str) -> tuple:
    ...
    try:
        ...
        # Монета с низким абсолютным объемом (< $5M) = низкая ликвидность
...
try:
    ...
    if not asyncio.iscoroutinefunction(get_filtered_top_usdt_pairs_fast):
        logger.warning("⚠️ get_filtered_top_usdt_pairs_fast не является async функцией, устанавливаем None")
    ...
    try:
        ...
        # Проверяем, что это async функция
        if not asyncio.iscoroutinefunction(get_filtered_top_usdt_pairs_fast):
            logger.warning("⚠️ get_filtered_top_usdt_pairs_fast из exchange_api не является async функцией, устанавливаем None")
            get_filtered_top_usdt_pairs_fast = None
    except ImportError:
        get_filtered_top_usdt_pairs_fast = None
...
def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Добавляет технические индикаторы к DataFrame с использованием Rust ускорения"""
    try:
        if len(df) < 50:
            logger.warning("Недостаточно данных для расчета индикаторов, "
                          "требуется минимум 50 свечей. Получено: %d", len(df))
            return df

        # Переводим в списки для Rust
        closes = df['close'].tolist()
        highs = df['high'].tolist()
        lows = df['low'].tolist()

        if RUST_INDICATORS_AVAILABLE and indicator_calculator:
            # 🚀 Использование Rust для тяжелых индикаторов
        ...
        # Bollinger Bands
            ...
            # Fallback на медленный Python (ta)
        ...
        # Индикаторы, которых пока нет в Rust (остаются на ta)
        # Volume Ratio
        df['volume_ratio'] = df['volume'] / df['volume'].rolling(window=20).mean()

        logger.debug("✅ Добавлены технические индикаторы (ускорение Rust: %s)", RUST_INDICATORS_AVAILABLE)
        return df
    except Exception as e:
        logger.error("❌ Ошибка добавления индикаторов: %s", e)
        return df
...
async def _generate_signal_impl(
    ...
    try:
        ...
        score = calculate_ai_signal_score(df, ai_params_data, symbol)  # Передаем полный ai_params_data
        ...
        try:
            ...
            liquidity_ok, liquidity_details = await check_liquidity(
                ...
                depth_levels=20,
                require_both=False,  # 🔧 ИСПРАВЛЕНИЕ: Достаточно одного условия (volume ИЛИ depth)
            )
            if not liquidity_ok:
                logger.info(
                    (
                        "🚫 %s: Недостаточная ликвидность "
                ...
                logger.debug(
                    (
                        "✅ %s: Ликвидность достаточна "
                        "(depth: %.2f USD ≥ %.2f USD, 24h volume: %.2f USD ≥ %.2f USD)"
                    ),
                    symbol,
                    liquidity_details.get('depth_usd', 0) or 0,
                    min_depth_required,
        ...
        if TELEGRAM_INTEGRATION_AVAILABLE:
            try:
                ...
                try:
                    ...
                    # Fallback к динамическому расчету плеча без ИИ
                    if trade_mode == "futures":
                        try:
                    ...
                    # Базовый расчет риска и суммы входа
                ...
                if regime_multipliers:
                    ...
                    logger.info("🎛️ [%s] Режим %s: базовая сумма %.2f → %.2f USDT (x%.2f)",
        ...
        try:
            ...
            if user_id_local:
                ...
                if mode == 'auto':
                    ...
                    logger.info("🤖 [AUTO] %s: параметры - баланс=%.2f, сумма=%.2f USDT, плечо=%dx, цена=%.8f, режим=%s",
...
def check_ai_volume_filter(df: pd.DataFrame, ai_params: Dict[str, Any]) -> bool:
    """ИИ-оптимизированный фильтр по объему."""
    if 'volume' not in df.columns or df.empty:
        return False
    # Используем наш оптимизированный порог вместо ИИ-параметров
    min_volume_usd = 10  # Минимальный порог для тестирования
    current_volume = df['volume'].iloc[-1]

    # Логируем для диагностики
    logger.debug("📊 Объем: текущий=%.0f, минимум=%.0f", current_volume, min_volume_usd)

    # Простая проверка: текущий объем выше минимального порога
    ...
    try:
        ...
        reason = None if passed else f"Объем {current_volume:.0f} < {min_volume_usd:.0f}"
...
def check_rsi_warning(df: pd.DataFrame, signal_type: str) -> bool:
    ...
    try:
        ...
        if group != "A":
            # Legacy логика (ИСПРАВЛЕНО: вернули к стандартным порогам 70/30)
            if signal_type == "BUY" and rsi_value > 70:  # 🔧 ИСПРАВЛЕНО: было 65, вернули к 70
                ctx['decision'] = 'reject'
                ...
                return False
            if signal_type == "SELL" and rsi_value < 30:  # 🔧 ИСПРАВЛЕНО: было 35, вернули к 30
                ctx['decision'] = 'reject'
