            ...
            if self.enable_metrics:
                async with self._lock:
                    ...
                    self.metrics.queue_max_size = max(
                        self.metrics.queue_max_size, self.queue.qsize()
                    )
    ...
    async def _worker(self):
        ...
        while self.is_running:
            try:
                ...
                try:
                    operation, future = await asyncio.wait_for(self.queue.get(), timeout=1.0)
                except asyncio.TimeoutError:
                    continue
            ...
            try:
                ...
                if operation.operation_type == WriteOperationType.EXECUTE:
                    result = await asyncio.to_thread(
                        self.db_executor, operation.query, operation.params, operation.is_write
                    )
                ...
                logger.error(
                    f"❌ [WriteQueue] Ошибка выполнения операции после {attempt + 1} попыток: {e}"
                )
                raise
...
async def get_write_queue(db_executor: Optional[Callable] = None, **kwargs) -> DatabaseWriteQueue:
