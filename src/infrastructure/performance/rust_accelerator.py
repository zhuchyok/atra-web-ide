...
try:
    ...
    print(
        "⚠️  Rust acceleration module not available. Install with: cd rust-atra && cargo build --release"
    )
...
class RustAccelerator:
    ...
    def __init__(self):
        ...
        if not self.available:
            raise RuntimeError(
                "Rust module not available. Build with: cd rust-atra && cargo build --release"
            )
    ...
    def check_anomaly_filter(
        ...
        Returns:
            Dictionary with passed, reason, and score
        """
        result = atra_rs.check_anomaly_filter(
            closes, volumes, lookback, volume_threshold, price_z_threshold
        )
        return {"passed": result.passed, "reason": result.reason, "score": result.score}
    ...
    def check_trend_strength_filter(
        ...
        Returns:
            Dictionary with passed, reason, and score
        """
        result = atra_rs.check_trend_strength_filter(
            closes, signal_direction, sma_short, sma_long, min_strength
        )
        return {"passed": result.passed, "reason": result.reason, "score": result.score}
    ...
    def calculate_exit_levels(
        ...
        """
        return atra_rs.calculate_exit_levels(
            entry_price, signal_direction, atr, atr_multiplier, rr_ratio
        )
    ...
    def calculate_position_size(
        ...
        Returns:
            Position size
        """
        return atra_rs.calculate_position_size(
