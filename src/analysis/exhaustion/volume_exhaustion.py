...
class VolumeExhaustion:
        ...
        try:
            ...
            for i in range(self.lookback, len(df)):
                ...
                if side == "long":
                    ...
                    price_movement = (
                        (df["close"].iloc[i] - df["close"].iloc[i - self.lookback])
                        / df["close"].iloc[i - self.lookback]
                        * 100
                    )
                elif side == "short":
                    # Для SHORT: движение вниз
                    price_movement = (
                        (df["close"].iloc[i - self.lookback] - df["close"].iloc[i])
                        / df["close"].iloc[i - self.lookback]
                        * 100
                    )
                else:
                    ...
                    price_movement = (
                        abs(df["close"].iloc[i] - df["close"].iloc[i - self.lookback])
                        / df["close"].iloc[i - self.lookback]
                        * 100
                    )
                ...
                if price_movement >= self.price_movement_threshold:
                    ...
                    volume_decline = (
                        (avg_volume - current_volume) / avg_volume if avg_volume > 0 else 0
                    )
    ...
    def is_exhausted(
        self, df: pd.DataFrame, i: int, side: Optional[str] = None, threshold: float = 0.7
    ) -> bool:
        """
