...
class LiquidityExhaustion:
    ...
    def calculate(self, df: pd.DataFrame, side: Optional[str] = None) -> pd.Series:
        """
        ...
        try:
            ...
            for i in range(self.lookback, len(df)):
                ...
                price_ranges = (
                    df["high"].iloc[i - self.lookback : i] - df["low"].iloc[i - self.lookback : i]
                )
                ...
                volatility_decline = (
                    (avg_volatility - current_volatility) / avg_volatility
                    if avg_volatility > 0
                    else 0
                )
                ...
                # Комбинируем факторы
                volume_exhaustion = (
                    min(1.0, volume_decline / self.volume_decline_threshold)
                    if volume_decline > 0
                    else 0.0
                )
                volatility_exhaustion = (
                    min(1.0, volatility_decline / self.volatility_decline_threshold)
                    if volatility_decline > 0
                    else 0.0
                )
    ...
    def is_exhausted(
        self, df: pd.DataFrame, i: int, side: Optional[str] = None, threshold: float = 0.6
    ) -> bool:
        """
    ...
    def get_exhaustion_level(
        self, df: pd.DataFrame, i: int, side: Optional[str] = None
    ) -> Optional[float]:
        """
    ...
    def get_signal(self, df: pd.DataFrame, i: int, side: Optional[str] = None) -> Optional[str]:
        """
