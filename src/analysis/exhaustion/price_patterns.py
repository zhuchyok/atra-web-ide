...
class PriceExhaustionPatterns:
    ...
    def detect_patterns(self, df: pd.DataFrame, i: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                is_bullish
                and upper_wick >= body_size * self.wick_ratio
            ...
            if (
                is_bearish
                and lower_wick >= body_size * self.wick_ratio
            ...
            if (
                is_bearish
                and prev["close"] > prev["open"]  # Предыдущая бычья
                and current["open"] > prev["close"]  # Текущая открывается выше
                and current["close"] < prev["open"]
            ):  # Текущая закрывается ниже
            ...
            if (
                is_bullish
                and prev["close"] < prev["open"]  # Предыдущая медвежья
                and current["open"] < prev["close"]  # Текущая открывается ниже
                and current["close"] > prev["open"]
            ):  # Текущая закрывается выше
                patterns["bullish_engulfing"] = True
                patterns["exhaustion_score"] += 0.5

            # 6. Spinning Top (неопределенность)
            if body_size / total_range < 0.3 and upper_wick > body_size and lower_wick > body_size:
    ...
    def is_exhausted(
        self, df: pd.DataFrame, i: int, side: Optional[str] = None, threshold: float = 0.5
    ) -> bool:
        """
        ...
        try:
            ...
            if side == "long":
                bearish_patterns = (
                    patterns["shooting_star"] or patterns["bearish_engulfing"] or patterns["doji"]
                )
            ...
            # Для SHORT: ищем бычьи паттерны
                ...
                bullish_patterns = (
                    patterns["hammer"] or patterns["bullish_engulfing"] or patterns["doji"]
                )
    ...
    def get_exhaustion_score(self, df: pd.DataFrame, i: int) -> float:
        """
