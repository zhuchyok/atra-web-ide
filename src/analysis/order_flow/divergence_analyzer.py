...
class DivergenceAnalyzer:
    ...
    def detect_divergence(
        ...
        try:
            ...
            indicator_change = (
                (indicator_end - indicator_start) / abs(indicator_start)
                if indicator_start != 0
                else 0
            )

            # Определяем направление трендов
            price_trend = (
                "up" if price_change > 0.01 else ("down" if price_change < -0.01 else "sideways")
            )
            indicator_trend = (
                "up"
                if indicator_change > 0.01
                else ("down" if indicator_change < -0.01 else "sideways")
            )
    ...
    def _check_bullish_divergence(
        ...
        indicator_trough1_idx = (
            indicator_troughs[-2] if len(indicator_troughs) >= 2 else indicator_troughs[-1]
        )
        ...
        if price_falling and indicator_rising:
            ...
            price_change_pct = (
                abs((price_trough2 - price_trough1) / price_trough1) if price_trough1 != 0 else 0
            )
            indicator_change_pct = (
                abs((indicator_trough2 - indicator_trough1) / abs(indicator_trough1))
                if indicator_trough1 != 0
                else 0
            )
    ...
    def _check_bearish_divergence(
        ...
        indicator_peak1_idx = (
            indicator_peaks[-2] if len(indicator_peaks) >= 2 else indicator_peaks[-1]
        )
        ...
        if price_rising and indicator_falling:
            # Рассчитываем силу дивергенции
            price_change_pct = (
                abs((price_peak2 - price_peak1) / price_peak1) if price_peak1 != 0 else 0
            )
            indicator_change_pct = (
                abs((indicator_peak2 - indicator_peak1) / abs(indicator_peak1))
                if indicator_peak1 != 0
                else 0
            )
            strength = min(1.0, (price_change_pct + indicator_change_pct) / 2)
