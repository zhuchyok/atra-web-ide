...
class AuctionMarketTheory:
    ...
    def _log_metrics(self, phase: Optional[MarketPhase], balance_score: Optional[float]):
        ...
        try:
            ...
            if phase:
                logger.debug(f"AMT Metrics: phase={phase.value}, balance_score={balance_score:.3f}")
    ...
    def detect_market_phase(
        ...
        try:
            ...
            # Определение фазы
            if (
                balance_deviation < self.balance_threshold
                and volatility < 0.02
    ...
    def get_auction_levels(
        ...
        try:
            ...
            if "volume" in window_df.columns:
                ...
                volume_weighted_price = (typical_price * window_df["volume"]).sum() / window_df[
                    "volume"
                ].sum()
