...
from config import CRYPTOPANIC_API_KEY, NEWSDATA_API_KEY
            ...
            async with session.get(url, params=params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    for post in data.get("results", []):
                        currencies = [
                            curr.get("code", "").upper() for curr in post.get("currencies", [])
                        ]
                        if base_symbol.upper() in currencies:
                            ...
                            news_type = "neutral"
                            if sentiment == "positive" or votes.get("positive", 0) > votes.get(
                                "negative", 0
                            ):
                                news_type = "positive"
                            elif sentiment == "negative" or votes.get("negative", 0) > votes.get(
                                "positive", 0
                            ):
                                news_type = "negative"
            ...
            async with session.get(url, params=params) as resp:
                if resp.status == 200:
                    ...
                    for article in data.get("results", []):
                        ...
                        news_type = "neutral"
                        if any(
                            kw in title or kw in description or kw in content
...
async def get_news_multi_source(symbol: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(
            f"[NewsFilter] Получено новостей для {symbol}: {len(all_news)} уникальных из {len(news_sources)} источников"
        )
        return all_news
    except Exception as e:
    ...
    try:
        ...
        for news in news_list:
            ...
            if news_type == "negative":
                logger.info(
                    f"[NewsFilter] Найдены негативные новости для {symbol}: {news.get('title', '')}"
                )
                return True
    ...
    try:
        ...
        for news in news_list:
            ...
            if news_type == "positive":
                logger.debug(
                    f"[NewsFilter] Найдены позитивные новости для {symbol}: {news.get('title', '')}"
                )
                return True
...
async def get_symbol_news_analysis(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not all_news:
            return {"sentiment": "neutral", "top_news": [], "score": 0}
