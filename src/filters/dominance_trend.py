...
from src.market.dominance import BTCDominanceAnalyzer, get_dominance_analyzer
        ...
        try:
            ...
            async with analyzer:
                ...
                if dominance_data is None:
                    logger.warning(
                        "⚠️ DominanceTrendFilter: данные доминации недоступны, пропускаем фильтр"
                    )
                ...
                if direction == "LONG":
                    ...
                    if (
                        self.block_long_on_rising
                        and trend == "rising"
                        and abs(change_pct) >= self.dominance_threshold_pct
                    ):
