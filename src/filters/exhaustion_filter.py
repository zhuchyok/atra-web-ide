...
def check_exhaustion_early_exit(
    ...
    try:
        ...
        volume_exhausted = volume_exhaustion.is_exhausted(
            df, i, side=side, threshold=0.7 if strict_mode else 0.5
        )
        price_exhausted = price_patterns.is_exhausted(
            df, i, side=side, threshold=0.5 if strict_mode else 0.3
        )
        liquidity_exhausted = liquidity_exhaustion.is_exhausted(
            df, i, side=side, threshold=0.6 if strict_mode else 0.4
        )
        ...
        if confirmations >= required_confirmations:
            ...
            if pnl_pct > 0:
                ...
                return (
                    False,
                    f"Позиция в убытке ({pnl_pct:.2f}%), не выходим по exhaustion",
                    details,
                )
