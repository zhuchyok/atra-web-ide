...
def check_order_flow_filter(
    ...
    try:
        ...
        if side.lower() == "long":
            ...
            if not cdv_ok:
                reasons.append(
                    f"CDV не подтверждает покупки (signal={cdv_signal}, value={cdv_value})"
                )
            if not vd_ok:
            ...
            return (
                False,
                f"LONG: недостаточно подтверждений Order Flow ({confirmations}/{required_confirmations}). {', '.join(reasons)}",
            )
            ...
            if not cdv_ok:
                reasons.append(
                    f"CDV не подтверждает продажи (signal={cdv_signal}, value={cdv_value})"
                )
            if not vd_ok:
            ...
            return (
                False,
                f"SHORT: недостаточно подтверждений Order Flow ({confirmations}/{required_confirmations}). {', '.join(reasons)}",
            )
