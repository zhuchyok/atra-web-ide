...
class CalculateRiskUseCase:
    ...
    async def execute(self) -> dict:
        ...
        total_exposure = sum(
            pos.entry_price.value * pos.quantity for pos in positions if pos.status.value == "open"
        )

        # Calculate total PnL
        total_pnl = sum(pos.pnl or Decimal("0") for pos in positions if pos.pnl is not None)
