...
Вероника — локальный агент-исполнитель корпорации ATRA. Выполняет задачи от координатора (Виктория) или из очереди БД: работа с файлами проекта, код, исследование. Запускается как **один сервис на порту 8011** (общий для всех проектов; контекст передаётся в запросах).
