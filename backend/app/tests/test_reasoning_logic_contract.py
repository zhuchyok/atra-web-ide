...
def _make_fake_response(json_data: dict, status_code: int = 200):
    """Синхронный response: status_code=200 (sync path), .json() возвращает dict, .raise_for_status() — ничего."""
    r = MagicMock()
...
async def test_victoria_client_needs_clarification_returns_questions():
    ...
    assert result.get("raw", {}).get("clarification_questions") == [
        "Какой именно проект?",
        "За какой период?",
    ]
