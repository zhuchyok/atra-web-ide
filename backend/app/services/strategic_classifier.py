...
def is_strategic_question(content: str) -> Tuple[bool, str]:
    ...
    if (
        any(phrase in content_lower for phrase in ["стоит ли", "нужно ли", "правильно ли"])
        and len(content_lower) > 30
    ):
        return True, "strategic_phrase"

    # 7. Эвристика: вопросы про выбор ("какой лучше", "выбрать X или Y")
    if (
        any(phrase in content_lower for phrase in ["какой лучше", "что лучше", "выбрать", "или"])
        and len(content_lower) > 40
    ):
        # Проверяем, что это не простой технический вопрос
        if not any(
            word in content_lower for word in ["синтаксис", "пример", "как написать", "функция"]
        ):
