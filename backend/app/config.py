...
class Settings(BaseSettings):
    ...
    victoria_url: str = os.getenv(
        "VICTORIA_URL", "http://localhost:8010"
    )  # Общий порт для всех проектов
    ...
    rag_light_cache_ttl: int = int(
        os.getenv("RAG_LIGHT_CACHE_TTL", "300")
    )  # секунды, in-memory кэш
    # RAG Context Cache (Ollama + MLX): кэш результатов векторного поиска, -150ms при хите
    rag_context_cache_enabled: bool = (
        os.getenv("RAG_CONTEXT_CACHE_ENABLED", "true").lower() == "true"
    )
    rag_cache_ttl_sec: int = int(
        os.getenv("RAG_CACHE_TTL_SEC", os.getenv("RAG_LIGHT_CACHE_TTL", "300"))
    )
    ...
    plan_cache_redis_enabled: bool = (
        os.getenv("PLAN_CACHE_REDIS_ENABLED", "false").lower() == "true"
    )
    plan_cache_maxsize: int = int(os.getenv("PLAN_CACHE_MAXSIZE", "100"))
    plan_cache_min_gen_time: float = float(
        os.getenv("PLAN_CACHE_MIN_GEN_TIME", "2.0")
    )  # кэшировать если генерация > N сек
    ...
    conversation_context_max_messages: int = int(
        os.getenv("CONVERSATION_CONTEXT_MAX_MESSAGES", "50")
    )
    conversation_context_max_chars: int = int(os.getenv("CONVERSATION_CONTEXT_MAX_CHARS", "8000"))
    conversation_context_use_redis: bool = (
        os.getenv("CONVERSATION_CONTEXT_USE_REDIS", "false").lower() == "true"
    )
    ...
    multimodal_vision_enabled: bool = (
        os.getenv("MULTIMODAL_VISION_ENABLED", "true").lower() == "true"
    )
    ...
    rag_light_prefetch_enabled: bool = (
        os.getenv("RAG_LIGHT_PREFETCH_ENABLED", "false").lower() == "true"
    )
    ...
    embedding_fallback_enabled: bool = (
        os.getenv("EMBEDDING_FALLBACK_ENABLED", "true").lower() == "true"
    )
    # Прогрев embeddings при старте (фоново, P95 < 300ms)
    rag_embedding_warmup_enabled: bool = (
        os.getenv("RAG_EMBEDDING_WARMUP_ENABLED", "true").lower() == "true"
    )
