...
async def get_autocomplete(
    request: AutocompleteRequest, victoria: VictoriaClient = Depends(get_victoria_client)
    ...
    try:
        ...
        before_cursor = (
            current_line[: request.column] if request.column <= len(current_line) else current_line
        )
...
def _generate_simple_completions(before_cursor: str, language: str) -> List[Dict[str, str]]:
    ...
    if language.lower() == "python":
        if "def " in before_cursor or "class " in before_cursor:
            completions.append(
                {"label": "pass", "type": "keyword", "detail": "pass statement", "insert": "pass"}
            )
        if "import " in before_cursor:
            completions.append(
                {"label": "os", "type": "module", "detail": "OS module", "insert": "os"}
            )
            completions.append(
                {"label": "json", "type": "module", "detail": "JSON module", "insert": "json"}
            )
    ...
    # JavaScript
        ...
        if "console." in before_cursor:
            completions.append(
                {"label": "log", "type": "function", "detail": "console.log()", "insert": "log()"}
            )
...
async def lint_code(request: LintRequest) -> LintResponse:
    """
