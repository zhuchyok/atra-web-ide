...
def get_metric(
    metric_class: Type[_M], name: str, documentation: str, labelnames=(), **kwargs
) -> _M:
    """
    ...
    try:
        ...
        if (
            hasattr(_isolated_registry, "_names_to_collectors")
            and name in _isolated_registry._names_to_collectors
        ):
...
# === Основные метрики ===
USER_REQUESTS = get_metric(
    Counter, "chat_requests_total", "Total user requests", ["mode", "endpoint"]
)
REQUEST_DURATION = get_metric(
    Histogram, "chat_request_duration_seconds", "Request duration in seconds", ["mode", "endpoint"]
)

# === RAG метрики ===
RAG_REQUESTS = get_metric(
    Counter, "rag_requests_total", "Total RAG requests", ["mode", "type", "path"]
)
RAG_DURATION = get_metric(
    Histogram, "rag_duration_seconds", "RAG processing duration", ["mode", "type"]
)
RAG_CACHE_HITS = get_metric(Counter, "rag_cache_hits_total", "RAG cache hits", ["cache_type"])
RAG_CHUNKS_RETURNED = get_metric(
    Histogram,
    "rag_chunks_returned",
...
# === Embedding метрики ===
EMBEDDING_REQUESTS = get_metric(
    Counter, "embedding_requests_total", "Total embedding requests", ["source"]
)
EMBEDDING_DURATION = get_metric(
    Histogram, "embedding_duration_seconds", "Embedding generation duration", ["source"]
)
EMBEDDING_BATCH_SIZE = get_metric(
    Histogram, "embedding_batch_size", "Size of embedding batches", buckets=[1, 2, 5, 10, 20]
)

# === [SINGULARITY 24.3] Advanced Caching Metrics ===
SEMANTIC_CACHE_HITS = get_metric(
    Counter, "semantic_cache_hits_total", "Total semantic cache hits", ["cache_type"]
)
EMBEDDING_COLLAPSED = get_metric(
    Counter, "embedding_requests_collapsed_total", "Total embedding requests collapsed"
)
EMBEDDING_BACKPRESSURE_THROTTLE = get_metric(
    Counter,
    "embedding_backpressure_throttle_total",
    "Total embedding requests delayed by backpressure",
)

# === [SINGULARITY 25.0] Ollama Backpressure Metrics ===
OLLAMA_BACKPRESSURE_SKIPS = get_metric(
    Counter,
    "ollama_backpressure_skips_total",
    "Ollama requests skipped due to global slot limit (Redis semaphore full)",
)

# === LLM метрики ===
LLM_REQUESTS = get_metric(
    Counter, "llm_requests_total", "Total LLM requests", ["provider", "model"]
)
LLM_DURATION = get_metric(Histogram, "llm_duration_seconds", "LLM response duration", ["provider"])
LLM_TOKENS = get_metric(
    Counter, "llm_tokens_total", "Total tokens processed", ["provider", "direction"]
)
...
TELEGRAM_BOT_MESSAGES = get_metric(
    Counter, "telegram_bot_messages_total", "Total messages processed by Telegram bot"
)
TELEGRAM_BOT_ERRORS = get_metric(
    Counter, "telegram_bot_errors_total", "Total errors in Telegram bot"
)
TELEGRAM_BOT_HEARTBEAT_AGE = get_metric(
    Gauge, "telegram_bot_heartbeat_age_seconds", "Seconds since last Telegram bot heartbeat"
)

# П.4 PRINCIPLE_EXPERTS_FIRST
CHAT_EXPERT_ANSWER_TOTAL = get_metric(
    Counter, "chat_expert_answer_total", "Chat responses from designated expert path", ["source"]
)
...
ASK_VICTORIA_TOTAL = get_metric(
    Counter, "ask_victoria_total", "Total ask_victoria API calls", ["status"]
)
