...
def main():
    ...
    try:
        from database_service import db_connection, fetch_data, fetch_parallel, quick_db_check
    ...
    try:
        fetch_data(
            "SELECT anomaly_type, severity, detected_at FROM anomaly_detection_logs ORDER BY detected_at DESC LIMIT 1"
        )
    ...
    try:
        fetch_data(
            "SELECT content, created_at FROM knowledge_nodes WHERE metadata->>'type' IN ('board_decision','board_directive','board_consult') ORDER BY created_at DESC LIMIT 1"
        )
        ok.append("Обзор: board decisions")
    except Exception as e:
    ...
    try:
        fetch_data(
            "SELECT AVG((metadata->>'audit_score')::int) as avg_score FROM tasks WHERE metadata->>'audit_score' IS NOT NULL"
        )
        fetch_data(
    ...
    try:
        fetch_data(
            "SELECT objective, department, period, created_at FROM okrs ORDER BY created_at DESC LIMIT 1"
        )
        ok.append("Стратегия: okrs (или пусто)")
    except Exception:
        fetch_data(
            "SELECT content, metadata, created_at FROM knowledge_nodes WHERE metadata->>'type' = 'okr' ORDER BY created_at DESC LIMIT 1"
        )
    ...
    try:
        fetch_data(
            "SELECT COUNT(*) as total_nodes, (SELECT COUNT(*) FROM knowledge_links) as total_links FROM knowledge_nodes"
        )
    ...
    try:
        fetch_data(
            "SELECT id, idea, result, created_at FROM simulations ORDER BY created_at DESC LIMIT 1"
        )
    ...
    try:
        fetch_data(
            "SELECT topic, status, consensus_summary, created_at FROM expert_discussions ORDER BY created_at DESC LIMIT 1"
        )
