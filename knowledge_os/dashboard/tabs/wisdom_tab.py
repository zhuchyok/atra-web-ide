...
def render_wisdom_tab():
    ...
    with col2:
        # Количество SOP
        sop_count = fetch_data("SELECT COUNT(*) as count FROM knowledge_nodes WHERE metadata->>'type' = 'sop_document'")
        count = sop_count[0]['count'] if sop_count else 0
        st.metric("Создано SOP", count)

    with col3:
        # Количество Mentorship Notes
        mentorship_count = fetch_data("SELECT COUNT(*) as count FROM knowledge_nodes WHERE metadata->>'type' = 'mentorship_note'")
    ...
    with col4:
        ...
        wisdom_nodes = fetch_data("SELECT COUNT(*) as count FROM knowledge_nodes WHERE metadata->>'type' IN ('meta_wisdom', 'mentorship_note', 'sop_document')")
    ...
    if mentorship_data:
        for note in mentorship_data:
            with st.expander(f"📌 {note['expert']} — Оценка: {note['score']}/10 ({note['created_at'].strftime('%d.%m %H:%M')})"):
    ...
    if sop_data:
        for sop in sop_data:
            ...
            with col_a:
                ...
                st.caption(f"Создано: {sop['created_at'].strftime('%d.%m.%Y')} | Путь: `{sop['file_path']}`")
            with col_b:
    ...
    if wisdom_over_time:
        ...
        fig = px.line(df, x='day', y='count', title='Накопление Meta-Knowledge (Wisdom)', 
