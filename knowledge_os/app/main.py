...
try:
    from json_fast import dumps as json_fast_dumps
    from json_fast import loads as json_fast_loads
except ImportError:
...
async def search_knowledge(query: str, domain: str = None) -> str:
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute(
            "UPDATE knowledge_nodes SET usage_count = usage_count + 1 WHERE id = ANY($1)", node_ids
        )

        result_text = "\n".join(
            [f"Similarity {r['similarity']:.2f}: {r['content']}" for r in results]
        )
