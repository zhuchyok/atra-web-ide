...
class ReinforcementLearning:
    ...
    async def record_action(
        self, action_type: str, parameters: Dict[str, Any], state: str
    ) -> Action:
        """Записать выполненное действие"""
        action_id = f"action_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"

        action = Action(action_id=action_id, action_type=action_type, parameters=parameters)
    ...
    async def self_reward(
        ...
        return await self.assign_reward(
            action_id=action_id, reward_value=reward_value, reward_type="self_reward"
        )
