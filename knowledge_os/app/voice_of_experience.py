...
class VoiceOfExperience:
    def __init__(self):
        self.failure_keywords = ['error', 'failed', 'timeout', 'crash', 'broken', 'bug', 'issue', 'down']
    ...
    async def get_warnings(self, task_description: str) -> str:
        ...
        """
        logger.info(f"🔍 [EXPERIENCE] Analyzing task for potential pitfalls: {task_description[:50]}...")
        ...
        try:
            ...
            """, f"%{task_description[:30]}%", [f"%{k}%" for k in self.failure_keywords])
