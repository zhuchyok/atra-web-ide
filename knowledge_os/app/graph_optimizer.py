...
class GraphOptimizer:
    ...
    async def semantic_pruning(self, threshold: float = 0.3, min_usage: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            if to_delete > 0:
                # Удаляем
                await conn.execute("DELETE FROM knowledge_links WHERE strength < $1", threshold)
                logger.info(
                    f"✂️ [PRUNING] Удалено {to_delete} слабых связей (strength < {threshold})"
                )
    ...
    async def cache_hot_paths(self):
        ...
        try:
            ...
            if hot_paths:
                ...
                logger.info(
                    f"🚀 [CACHING] Закешировано {len(serializable_paths)} горячих путей в Redis"
                )
