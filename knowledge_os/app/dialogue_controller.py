...
from consensus_agent import ConsensusAgent, AgentResponse
...
class DialogueController:
    ...
    def __init__(self, event_bus: EventBus):
        ...
        self.consensus_agent = ConsensusAgent(model_name=os.getenv("VICTORIA_MODEL", "victoria-wisdom-v3.5:latest"))

    def start(self):
        """Запуск контроллера и подписка на события"""
        # [SINGULARITY 24.3] DEBUG: Log subscriptions
        import os
        logger.info(f"🎭 [DIALOGUE] (PID: {os.getpid()}) Subscribing to events on EventBus ID: {id(self.event_bus)}")
    ...
    async def handle_dialogue_consensus_local(self, event: Event):
        ...
        logger.info(f"🏆 [DIALOGUE] Local EventBus received consensus for {dialogue_id} (Score: {score:.2f})")
    ...
    async def handle_dialogue_request(self, event: Event):
        ...
        COLLECTION_TIMEOUT = int(os.getenv("DIALOGUE_COLLECTION_TIMEOUT", "190"))
        ...
        async def _collect_with_timeout():
            ...
            if dialogue and dialogue["status"] == "collecting":
                ...
                logger.warning(f"⏰ [DIALOGUE] Collection timeout for {dialogue_id}: got {n_got}/{n_exp} responses, starting consensus")
    ...
    async def handle_expert_response(self, event: Event):
        ...
        logger.info(f"📥 [DIALOGUE] Received response from {expert_name} for dialogue {dialogue_id} ({len(response_text) if response_text else 0} chars)")
        ...
        if len(dialogue["responses"]) >= len(dialogue["experts"]):
            logger.info(f"🏁 [DIALOGUE] All {len(dialogue['experts'])} responses collected for {dialogue_id}, starting consensus")
            await self._reach_consensus(dialogue_id)
        else:
            logger.info(f"⏳ [DIALOGUE] Collected {len(dialogue['responses'])}/{len(dialogue['experts'])} responses for {dialogue_id}")
    ...
    async def _select_experts(self, query: str) -> List[str]:
        ...
        if not experts:
            ...
            if "Игорь" not in experts: experts.add("Игорь")
            else: experts.add("Дмитрий")
    ...
    async def _reach_consensus(self, dialogue_id: str):
        ...
        if not dialogue["responses"]:
            logger.warning(f"⚠️ [DIALOGUE] No expert responses for {dialogue_id}, publishing empty consensus")
            await self.event_bus.publish(Event(
                event_id=str(uuid.uuid4()),
        ...
        try:
            ...
            logger.info(f"✅ [DIALOGUE] Direct consensus for {dialogue_id}: {n_got}/{n_experts} experts, score={score:.2f}")
            
        except Exception as e:
            logger.error(f"❌ [DIALOGUE] Consensus failed for {dialogue_id}: {e}")
            # Fallback: просто склеить ответы
            fallback_answer = "\n\n".join([f"**{n}**: {r}" for n, r in dialogue["responses"].items()])
            await self.event_bus.publish(Event(
                event_id=str(uuid.uuid4()),
                event_type=EventType.DIALOGUE_CONSENSUS,
                payload={
