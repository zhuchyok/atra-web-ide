...
class SecretManager:
    ...
    def __init__(self, master_key: Optional[str] = None):
        ...
        if not master_key:
            ...
            logger.warning(
                "⚠️ [SECRET MANAGER] Используется производный ключ. Установите SECRET_MASTER_KEY для безопасности."
            )
        else:
    ...
    async def encrypt_secret(self, key_name: str, secret_value: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "ℹ️ asyncpg не установлен, секрет не сохранен в БД (только зашифрован, опциональный компонент)"
                )
