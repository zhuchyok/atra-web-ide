...
class MLRouterABTest:
    ...
    async def should_switch_to_ml(
        self, min_samples: int = 100, improvement_threshold: float = 0.1
    ) -> bool:
        """
        ...
        if tokens_improvement >= improvement_threshold and performance_diff >= -0.05:
            logger.info(
                f"✅ [AB TEST] ML routing shows improvement: {tokens_improvement:.2%} more tokens saved"
            )
            return True
