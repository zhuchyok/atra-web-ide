...
class ProjectAuditOptimizer:
    ...
    async def scan_structure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ [AUDIT] Structure scanned: {len(all_files)} files, README: {bool(structure['readme'])}"
            )
    ...
    async def select_key_modules(self, structure: Dict[str, Any]) -> List[str]:
        ...
        """
        logger.info("🔍 [AUDIT] Phase 2: Selecting key modules")
    ...
    async def analyze_with_timeout(
        ...
        """
        results = {"phases": {}, "final_report": None, "elapsed": {}, "errors": []}

        try:
            # Phase 1: Structure scan (уже выполнен)
            if progress_callback:
                await progress_callback(
                    {"phase": "structure", "progress": 20, "status": "complete"}
                )
            ...
            if progress_callback:
                await progress_callback(
                    {"phase": "experts", "progress": 50, "status": "delegating"}
                )
            ...
            if progress_callback:
                await progress_callback(
                    {"phase": "synthesis", "progress": 90, "status": "generating"}
                )
...
async def audit_project_chunked(
    project_path: str, language: str, goal: str, progress_callback: Optional[callable] = None
) -> Dict[str, Any]:
    """
    ...
    Usage:
        async def on_progress(data):
            # TODO: Convert f-string to %s formatting for performance
            logger.info(f"Progress: {data['progress']}% - {data['status']}")
    ...
    try:
        structure = await asyncio.wait_for(
            optimizer.scan_structure(), timeout=AUDIT_STRUCTURE_TIMEOUT
        )
    ...
    result = await optimizer.analyze_with_timeout(goal, structure, key_modules, progress_callback)
