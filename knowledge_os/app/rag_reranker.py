...
class RAGReranker:
    ...
    def __init__(self, model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2"):
        ...
        if HAS_CROSS_ENCODER:
            try:
                # Загружаем модель (лениво)
                self.model = CrossEncoder(model_name, device="cpu") # На Mac Studio CPU достаточно быстро
                logger.info(f"✅ [RERANKER] Модель {model_name} загружена")
            except Exception as e:
                logger.error(f"❌ [RERANKER] Ошибка загрузки модели {model_name}: {e}")

    def rerank(self, query: str, nodes: List[Dict[str, Any]], top_k: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(f"🎯 [RERANKER] Переранжировано {len(nodes)} узлов, top-1 score: {reranked_nodes[0]['rerank_score']:.2f}")
