...
class ChecklistGenerator:
    ...
    async def generate_checklist(
        self, task_type: str, based_on_errors: bool = False
    ) -> Optional[Checklist]:
        """
        ...
        try:
            ...
            if conn:
                try:
                    ...
                    logger.info(
                        f"✅ [CHECKLIST GENERATOR] Создан чеклист {checklist_id} для {task_type} ({len(items)} пунктов)"
                    )
                    return checklist
        ...
        try:
            ...
            if conn:
                try:
                    ...
                    logger.info(
                        f"✅ [CHECKLIST GENERATOR] Чеклист {checklist_id} обновлен ({len(checklist.items)} пунктов)"
                    )
                    return True
