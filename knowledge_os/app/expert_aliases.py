...
class ExpertAliasManager:
    ...
    async def load_aliases(self, force: bool = False) -> bool:
        ...
        try:
            ...
            logger.info(
                "Загружено %d экспертов, %d алиасов из БД", len(self._experts), len(self._aliases)
            )
            return True
    ...
    def extract_expert_and_message(self, text: str) -> Tuple[Optional[str], str]:
        """
...
async def extract_expert_from_message(
    text: str, default_expert: str = "Виктория"
) -> Tuple[str, str]:
    """
