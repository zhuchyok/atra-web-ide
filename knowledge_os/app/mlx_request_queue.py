...
class MLXRequestQueue:
    ...
    async def add_request(
        ...
        if self.queue.full():
            logger.warning(f"⚠️ Очередь переполнена ({self.max_queue_size}), отклоняем запрос {request_id}")
        ...
        try:
            while True:
                ...
                if request.is_expired():
                    logger.warning(
                        f"⚠️ Запрос {request.request_id} истек по таймауту "
                        f"({request.timeout}с)"
                    )
    ...
    async def _execute_request(self, request: QueuedRequest):
        ...
        try:
            ...
            duration = (datetime.now() - start_time).total_seconds()
            logger.error(
                f"❌ Ошибка выполнения запроса {request.request_id} "
                f"за {duration:.2f}с: {e}",
                exc_info=True
            )
            raise
        finally:
