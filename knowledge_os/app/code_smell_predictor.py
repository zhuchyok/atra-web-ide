...
class CodeSmellPredictor:
    ...
    def calculate_bug_probability(
        self, code: str, file_path: str, history: Optional[Dict] = None
    ) -> float:
        """
    ...
    def predict_bugs(
        self, file_path: str, code: str, history: Optional[Dict] = None
    ) -> BugPrediction:
        """
    ...
    async def save_prediction(self, prediction: BugPrediction) -> Optional[str]:
        ...
        async with pool.acquire() as conn:
            try:
                ...
                logger.debug(
                    f"✅ Saved bug prediction: {prediction.file_path} (probability: {prediction.bug_probability:.2f})"
                )
