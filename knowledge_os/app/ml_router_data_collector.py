...
class MLRouterDataCollector:
    ...
    async def collect_routing_decision(
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                logger.debug(
                    f"✅ [ML DATA] Saved routing decision: {selected_route} for {task_type}"
                )
                return True
    ...
    async def collect_prediction(
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                logger.debug(
                    f"✅ [ML DATA] Saved prediction: {predicted_route} (confidence: {confidence})"
                )
                return True
        except Exception as e:
    ...
    async def get_training_data(self, limit: int = 1000, days: int = 30) -> list:
