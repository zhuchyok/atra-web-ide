...
async def estimate_cost(
    prompt_tokens: int, completion_tokens: int, model_type: str = "gpt-4o-mini"
) -> float:
    """
    ...
    pricing = {
        ...
        "cursor-agent": {
            "input": 0.0,
            "output": 0.0,
        },  # Cursor-agent может быть локальным или облачным
    }
        ...
        async with pool.acquire() as conn:
            ...
            if not expert_id and expert_name:
                ...
                expert_id = await conn.fetchval(
                    "SELECT id FROM experts WHERE name = $1", resolved_name
                )
            ...
            logger.debug(
                f"✅ Logged AI interaction: {total_tokens} tokens, ${cost_usd:.6f} (source: {source})"
            )
            return str(log_id)
...
async def log_ai_interaction_sync(prompt: str, response: str, **kwargs) -> Optional[str]:
    """
...
def log_ai_interaction_fire_and_forget(prompt: str, response: str, **kwargs) -> None:
    """
