...
class AdvancedEnsemble:
    ...
    def __init__(self):
        ...
        self.model_specialization: Dict[str, List[str]] = {
            "reasoning": ["victoria-wisdom-30b", "phi3.5:3.8b"],
            "planning": ["victoria-wisdom-30b", "phi3.5:3.8b"],
            "coding": ["victoria-wisdom-30b", "phi3.5:3.8b"],
            "fast": ["phi3.5:3.8b", "tinyllama:1.1b-chat"],
            "complex": ["victoria-wisdom-30b", "phi3.5:3.8b"],
        }
    ...
    async def select_models_for_task(self, goal: str, max_models: int = 3) -> List[str]:
        """
        ...
        if task_type in self.model_performance:
            ...
            sorted_models = sorted(
                ...
                key=lambda m: (
                    ...
                    performances.get(
                        m, ModelPerformance(m, task_type, 0.5, 0.5, 1.0)
                    ).average_confidence,
                ),
                reverse=True,
            )
    ...
    async def weighted_voting(
        self, model_results: Dict[str, Any], weights: Optional[Dict[str, float]] = None
    ) -> EnsembleResult:
        """
        ...
        if not model_results:
            return EnsembleResult(
                final_result=None, confidence=0.0, votes={}, weights={}, method="weighted_voting"
            )
    ...
    async def best_of_n(
        self, goal: str, n: int = 3, context: Dict[str, Any] = None
    ) -> EnsembleResult:
        """
        Best-of-N выборка
    ...
    def update_model_performance(
        self, model_name: str, task_type: str, success: bool, confidence: float, latency: float
    ):
        ...
        if model_name not in self.model_performance[task_type]:
            ...
            perf.success_rate = (
                perf.success_rate * perf.total_uses + (1.0 if success else 0.0)
            ) / (perf.total_uses + 1)
            perf.average_confidence = (perf.average_confidence * perf.total_uses + confidence) / (
                perf.total_uses + 1
            )
            perf.average_latency = (perf.average_latency * perf.total_uses + latency) / (
                perf.total_uses + 1
            )
            perf.total_uses += 1
