...
class MetaArchitect:
    ...
    async def verify_mutation_safety(
        ...
        try:
            ...
            if not mutated_function:
                return {
                    "score": 0.0,
                    "risks": [f"Function {function_name} not found in mutated code"],
                }
...
"""
            ...
            if SafetyVerifier:
                ...
                safety_report = await verifier.verify_mutation(
                    spot["module_name"], spot["function_name"], mutated_code
                )

                if safety_report["safety_score"] < 80:
                    logger.warning(
                        f"🛑 [SAFETY VIOLATION] Mutation for {spot['function_name']} rejected. Score: {safety_report['safety_score']}. Risks: {safety_report['risks']}"
                    )
                ...
                safety_report = await self.verify_mutation_safety(
                    spot["module_name"], spot["function_name"], mutated_code
                )
                if safety_report["score"] < 0.7:
                    logger.warning(
                        f"🛑 [SAFETY VIOLATION] Mutation for {spot['function_name']} rejected. Score: {safety_report['score']}. Risks: {safety_report['risks']}"
                    )
        ...
        for spot in hot_spots:
            ...
            prod_spot = next(
                (
                    s
                    for s in hot_spots
                    if s["module_name"] == original_module
                    and s["function_name"] == spot["function_name"]
                ),
                None,
            )
            ...
            if prod_spot:
                ...
                logger.info(
                    f"📊 [RECURSIVE LEARNING] {original_module}.{spot['function_name']}: Shadow is {improvement:.1%} faster than Production."
                )

                if improvement > 0.05 and spot["call_count"] > 50:  # 5% threshold, 50 calls min
                    logger.info(
                        f"🏆 [WINNER] Shadow version of {original_module} is a winner! Triggering Hot-Swap."
                    )
        ...
        try:
            ...
            for task in tasks:
                ...
                file_path_rel = await run_smart_agent_async(
                    analysis_prompt, expert_name="Виктория", category="meta_architect_analysis"
                )
                ...
                new_code = await run_smart_agent_async(
                    patch_prompt, expert_name="Виктория", category="meta_architect_patching"
                )
                ...
                try:
                    ...
                    """,
                        task["id"],
                        f"Code patched in {full_path}",
                    )

                    # 6. Log knowledge
                    node_content = (
                        f"🔧 GLOBAL SELF-PATCH: Meta-Architect исправил баг в {full_path}"
                    )
                    node_meta = json.dumps(
                        {"type": "self_patch", "file": full_path, "task_id": str(task["id"])}
                    )
                    await conn.execute(
                        """
                        INSERT INTO knowledge_nodes (domain_id, content, is_verified, confidence_score, metadata)
                    ...
                    """,
                        task["id"],
                        f"Syntax error in generated patch: {exc}",
                    )
