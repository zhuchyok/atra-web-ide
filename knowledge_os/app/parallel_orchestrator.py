...
class ParallelOrchestrator:
        ...
        for task in subtasks:
            tasks.append(
                run_smart_agent_async(
                    task["prompt"], expert_name=task["expert"], category=task.get("category")
                )
            )
    ...
    async def auto_decompose_and_run(self, main_prompt: str):
        ...
        plan_json = await run_smart_agent_async(
            decomposer_prompt, expert_name="Виктория", category="fast"
        )
        ...
        try:
            ...
            synthesis_prompt = "На основе этих результатов дай финальный ответ:\n"
            for i, res in enumerate(results):
                synthesis_prompt += f"Результат {i + 1}: {res}\n"

            final_answer = await run_smart_agent_async(
