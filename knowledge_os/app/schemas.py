...
def parse_agent_response(raw_response: Union[str, Dict, Any]) -> AgentResponse:
    ...
    if raw_response is None:
        return AgentResponse(status="error", error="Empty response: NoneType")
    ...
    if isinstance(raw_response, dict):
        ...
        output = (
            raw_response.get("output") or raw_response.get("response") or raw_response.get("text")
        )
        ...
        if status == "processing":
            return AgentResponse(
                status="processing", job_id=raw_response.get("job_id"), output=output
            )
