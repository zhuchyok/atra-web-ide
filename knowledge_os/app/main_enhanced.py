...
from enhanced_search import SearchMode, detect_search_mode, enhanced_search_knowledge
from global_scout import GlobalScout
from knowledge_graph import KnowledgeGraph, LinkType
from mcp.server.fastmcp import FastMCP
from translator import KnowledgeTranslator, MultilingualSearch, UILocalizer

# Create FastMCP server
mcp = FastMCP("KnowledgeOS", host="0.0.0.0", port=8000, sse_path="/sse", message_path="/messages")
...
@mcp.tool()
async def search_knowledge(query: str, domain: str = None, mode: str = None, limit: int = 5) -> str:
    """
    ...
    result = await enhanced_search_knowledge(
        query=query, domain=domain, mode=search_mode, limit=limit, use_cache=True
    )
...
async def search_knowledge_detailed(
    query: str, domain: str = None, mode: str = None, limit: int = 5
) -> str:
    """
    ...
    async with db_pool.acquire() as conn:
        ...
        if not domain_id:
            domain_id = await conn.fetchval(
                "INSERT INTO domains (name) VALUES ($1) RETURNING id", domain
            )
        ...
        return "✅ Knowledge captured and stored. Cache invalidated."


@mcp.tool()
async def validate_knowledge_external(
    knowledge_id: int = None, content: str = None, domain: str = None
) -> str:
    """
...
async def get_learned_insights(
    expert_id: str = None, learning_type: str = None, limit: int = 10
) -> str:
    """
...
async def translate_knowledge(
    knowledge_id: str, target_language: str, source_language: str = None
) -> str:
    """
    ...
    translated = await translator.translate_knowledge(
        knowledge_id, target_language, source_language
    )
...
async def search_multilang(
    query: str, language: str = "auto", domain: str = None, limit: int = 10
) -> str:
    """
...
@mcp.tool()
async def get_ui_translation(key: str, language: str = "en", context: str = None) -> str:
    """
