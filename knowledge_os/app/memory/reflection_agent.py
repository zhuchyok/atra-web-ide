...
class ReflectionAgent:
    ...
    async def _reflect_on_department(self, dept: str, journals: list):
        ...
        journal_texts = []
        for j in journals:
            journal_texts.append(f"Expert: {j['expert_name']}\nTask: {j['summary']}\nLearnings: {j['learnings']}")
...
"""
        try:
            ...
            resp = await run_smart_agent_async(reflection_prompt, expert_name="Victoria", category="reasoning")
    ...
    async def _store_semantic_insight(self, dept: str, insight: dict):
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                content = f"💎 [CRYSTAL OF WISDOM - {dept}]: {insight['title']}\n\n{insight['content']}"
                metadata = {
                ...
                """, content, json.dumps(metadata), domain_id)
