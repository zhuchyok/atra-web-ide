...
class AutonomousToolCreator:
    ...
    async def identify_skill_need(self, error_message: str, goal: str) -> Optional[str]:
        ...
        try:
            ...
            response = await run_smart_agent_async(
                analysis_prompt, expert_name="Architect", category="reasoning"
            )
    ...
    async def generate_tool_code(self, tool_spec: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            code = await run_smart_agent_async(
                code_prompt, expert_name="Developer", category="coding"
            )
    ...
    async def test_and_register_tool(
        self, tool_name: str, description: str, code: str, category: str = "dynamic"
    ) -> bool:
        """
...
"""
    ...
    async def create_tool_on_the_fly(self, error_message: str, goal: str) -> bool:
        ...
        if spec and spec.get("needs_new_tool"):
            logger.info(
                f"💡 [TOOL CREATOR] Victoria decided to create a new tool: {spec['tool_name']}"
            )
            code = await self.generate_tool_code(spec)
            if code:
                return await self.test_and_register_tool(
                    spec["tool_name"], spec["tool_description"], code
                )
        return False
