    ...
    async def run_semantic_merge(self, similarity_threshold: float = 0.95):
        ...
        try:
            ...
            for dup in duplicates:
                ...
                """)
                if table_exists:
                    await conn.execute("UPDATE knowledge_links SET source_id = $1 WHERE source_id = $2", dup['id1'], dup['id2'])
                    await conn.execute("UPDATE knowledge_links SET target_id = $1 WHERE target_id = $2", dup['id1'], dup['id2'])
                
                # 3. Удаляем дубликат
                await conn.execute("DELETE FROM knowledge_nodes WHERE id = $1", dup['id2'])
                merged_count += 1
                logger.info(f"🤝 [MEMORY CYCLE] Semantic Merge: Merged {dup['id2']} into {dup['id1']}")

            if merged_count > 0:
                logger.info(f"♻️ [MEMORY CYCLE] Semantic Merge complete: {merged_count} duplicates merged.")
            return merged_count
        finally:
            await conn.close()
