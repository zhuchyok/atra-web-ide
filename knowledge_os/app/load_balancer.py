...
class LoadBalancer:
    ...
    def update_node_load(
        self, node_name: str, routing_key: str, latency: float, success: bool = True
    ):
            ...
            async with self._lock:
                if routing_key not in self.node_loads:
                    self.node_loads[routing_key] = NodeLoad(
                        node_name=node_name, routing_key=routing_key
                    )
    ...
    def end_request(self, routing_key: str):
        ...
        async def _end():
            async with self._lock:
                if routing_key in self.node_loads:
                    self.node_loads[routing_key].active_requests = max(
                        0, self.node_loads[routing_key].active_requests - 1
                    )
    ...
    def round_robin_select(
        self, available_nodes: List[Dict[str, Any]], last_selected: Optional[str] = None
