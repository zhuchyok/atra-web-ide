...
class QueryOrchestrator:
    ...
    def classify_query(self, query: str) -> QueryType:
        ...
        scores = {
            ...
            QueryType.OPTIMIZATION: sum(
                1 for kw in self.OPTIMIZATION_KEYWORDS if kw in query_lower
            ),
            QueryType.CODE: sum(1 for kw in self.CODE_KEYWORDS if kw in query_lower),
            QueryType.ARCHITECTURE: sum(
                1 for kw in self.ARCHITECTURE_KEYWORDS if kw in query_lower
            ),
        }
        ...
        if max_score > 0:
            ...
            logger.debug(
                f"🔍 [QUERY ORCHESTRATOR] Запрос классифицирован как: {query_type.value} (score: {max_score})"
            )
            return query_type
    ...
    def normalize_query(self, query: str) -> NormalizedQuery:
        ...
        logger.debug(
            f"📝 [QUERY ORCHESTRATOR] Запрос нормализован: goal={goal[:50]}..., constraints={len(constraints)}, preferences={len(preferences)}"
        )
    ...
    def _extract_goal(self, query: str) -> str:
        ...
        query = re.sub(
            r"^(здравствуйте|привет|добрый\s+(?:день|вечер|утро))[,\s]*",
            "",
            query,
            flags=re.IGNORECASE,
        )
        query = re.sub(
            r"[,\s]*(спасибо|благодарю|благодарность)[,\s]*$", "", query, flags=re.IGNORECASE
        )

        # Извлекаем основную часть запроса
        goal_match = re.search(
            r"(?:сделай|создай|напиши|реализуй|разработай|оптимизируй|улучши|исправь|проверь|проанализируй)\s+(.+?)(?:\.|$|при|с|для|в)",
            query,
            re.IGNORECASE,
        )
        if goal_match:
    ...
    def select_role(self, query_type: QueryType, context: Optional[Dict] = None) -> str:
        ...
        if candidates:
            ...
            logger.debug(
                f"👤 [QUERY ORCHESTRATOR] Выбрана роль: {selected} для типа {query_type.value}"
            )
            return selected
    ...
    async def enrich_context_from_db_async(
        ...
        try:
            ...
            try:
                ...
                if rows:
                    logger.debug(
                        "QueryOrchestrator: подобрано %s фрагментов из knowledge_nodes", len(rows)
                    )
            finally:
                await conn.close()
        except Exception as e:
...
"""
    ...
    def format_context(self, context: PromptContext) -> str:
        ...
        try:
            ...
            context_parts.append(
                "Доступные эксперты и услуги (при необходимости делегируй): "
                + get_expert_services_text(18)
            )
        except ImportError:
            pass
    ...
    def optimize_context(
        self, context: PromptContext, max_length: int, max_window_percent: float = 0.7
    ) -> PromptContext:
        """
        ...
        if context.session_summary and len(context.session_summary) > actual_max_length // 2:
            if ContextCompressor:
                context.session_summary = ContextCompressor.compress_all(
                    context.session_summary[: actual_max_length // 2]
                )
            else:
        ...
        logger.debug(
            f"📉 [QUERY ORCHESTRATOR] Контекст оптимизирован: max_length={actual_max_length}"
        )
