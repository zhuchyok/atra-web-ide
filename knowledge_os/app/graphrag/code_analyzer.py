...
async def sync_code_to_graph(directory: str = "knowledge_os/app/"):
    ...
    try:
        ...
        if not domain_id:
            domain_id = await conn.fetchval("INSERT INTO domains (name) VALUES ('Codebase') RETURNING id")
        ...
        for entity in analyzer.entities:
            ...
            node_id = await conn.fetchval("""
                ...
                RETURNING id
            """, domain_id, entity.content, str(embedding) if embedding else None, json.dumps(metadata), f"code:{entity.file_path}")
            
            if not node_id:
                # Если конфликт не сработал по сложной логике, ищем существующий
                node_id = await conn.fetchval("SELECT id FROM knowledge_nodes WHERE content = $1 AND domain_id = $2", entity.content, domain_id)
        ...
        for link in analyzer.links:
            ...
            if not source_id:
                ...
                source_id = await conn.fetchval("SELECT id FROM knowledge_nodes WHERE metadata->>'file_path' = $1", link.source_name)
            
            if not target_id:
                # Ищем по имени в той же кодовой базе
                target_id = await conn.fetchval("SELECT id FROM knowledge_nodes WHERE content LIKE $1 AND domain_id = $2", f"%{link.target_name}%", domain_id)
            ...
            if source_id and target_id:
                ...
                """, source_id, target_id, link.link_type, json.dumps(link.metadata))
