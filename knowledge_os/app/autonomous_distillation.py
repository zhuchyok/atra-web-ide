...
class AutonomousDistiller:
    ...
    async def detect_successful_responses(
        self, min_performance: float = 0.8, days: int = 7
    ) -> List[Dict]:
    ...
    async def generate_synthetic_variations(
        self, query: str, response: str, count: int = 3
    ) -> List[Dict]:
        """
    ...
    async def save_distillation_examples(self, examples: List[Dict], category: str = "general"):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"✅ [AUTONOMOUS DISTILLATION] Сохранено {len(examples)} примеров для категории {category}"
                )
            finally:
                await conn.close()
        except Exception as e:
    ...
    async def run_autonomous_distillation(self, category: str = "general"):
        ...
        if not successful:
            logger.info(
                f"ℹ️ [AUTONOMOUS DISTILLATION] Не найдено успешных ответов для категории {category}"
            )
            return
        ...
        if all_examples:
            ...
            logger.info(
                f"✅ [AUTONOMOUS DISTILLATION] Создано {len(all_examples)} synthetic examples"
            )
