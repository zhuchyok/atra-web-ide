        ...
        try:
            ...
            try:
                ...
                mentors = await conn.fetch(
                    ...
                    LIMIT 10
                """,
                    mentee_rating.mentorship_score,
                    mentee_rating.total_tasks,
                    mentee_id,
                )
    ...
    async def get_active_mentor(self, mentee_id: str) -> Optional[MentorAssignment]:
        ...
        try:
            ...
            if row:
                ...
                self._assignments_cache[f"{assignment.mentor_id}:{assignment.mentee_id}"] = (
                    assignment
                )
                return assignment
    ...
    async def provide_guidance(self, mentor_id: str, mentee_id: str, guidance: str) -> bool:
        ...
        try:
            ...
            if not assignment or assignment.mentor_id != mentor_id:
                logger.warning(
                    f"⚠️ [MENTOR SYSTEM] Нет активного назначения {mentor_id} → {mentee_id}"
                )
                return False
            ...
            try:
                ...
                logger.info(
                    f"✅ [MENTOR SYSTEM] Рекомендация передана от {mentor_id} к {mentee_id} (#{assignment.guidance_count})"
                )
                return True
