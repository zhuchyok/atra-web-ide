...
class MetricsCollector:
    ...
    async def collect_tokens_per_second(
        self, tokens: int, duration_seconds: float, source: str
    ) -> None:
        ...
        metric = Metric(
            ...
            unit="tokens/s",
            source=source,
            metadata={"tokens": tokens, "duration_seconds": duration_seconds},
        )
    ...
    async def collect_cost_per_response(
        ...
        metric = Metric(
            ...
            unit="USD",
            source=source,
            metadata={"model_name": model_name, "tokens_used": tokens_used},
        )
        await self._add_metric(metric)
    ...
    async def collect_compression_ratio(
        self, original_size: int, compressed_size: int, source: str
    ) -> None:
    ...
    async def get_metrics_stats(
        self, metric_name: str, hours: int = 24, source: Optional[str] = None
    ) -> Dict[str, Any]:
