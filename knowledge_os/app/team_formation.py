...
class TeamFormationSystem:
    ...
    async def form_team(
        self, task_id: str, task_type: str, required_expertise: List[str], team_size: int = 3
    ) -> Optional[Team]:
        """
        ...
        try:
            ...
            try:
                ...
                if len(selected_agents) < 2:  # Минимум 2 агента для команды
                    logger.warning(
                        f"⚠️ [TEAM FORMATION] Недостаточно агентов для команды (требуется {team_size}, найдено {len(selected_agents)})"
                    )
                    return None
                ...
                if table_exists:
                    ...
                    """,
                        ...
                        team.status,
                        team.created_at,
                        json.dumps([asdict(m) for m in members]),
                    )
                ...
                logger.info(
                    f"✅ [TEAM FORMATION] Сформирована команда {team_id} для задачи {task_id} ({len(members)} участников)"
                )
                return team
            ...
            if row:
                ...
                members = [
                    TeamMember(
                        ...
                        assigned_at=datetime.fromisoformat(m["assigned_at"])
                        if isinstance(m["assigned_at"], str)
                        else m["assigned_at"],
                    )
    ...
    async def evaluate_team_performance(self, team_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": team.status,
                "duration_hours": (team.completed_at - team.created_at).total_seconds() / 3600
