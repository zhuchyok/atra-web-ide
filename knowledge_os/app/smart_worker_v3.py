...
async def process_task(task):
    ...
    await pool.execute(
        "UPDATE tasks SET status = 'in_progress', updated_at = NOW() WHERE id = $1", task_id
    )
    ...
    if report:
        ...
        domain_id = await pool.fetchval(
            "SELECT domain_id FROM experts WHERE name = $1 LIMIT 1", expert_name
        )
        await pool.execute(
            """
        ...
        """,
            domain_id,
            f"📊 ОТЧЕТ ЭКСПЕРТА ({expert_name}): {task['title']}\n\n{report}",
            json.dumps(
                {"task_id": str(task_id), "expert": expert_name, "source": "autonomous_worker"}
            ),
        )
        ...
        await pool.execute(
            "UPDATE tasks SET status = 'pending', updated_at = NOW() WHERE id = $1", task_id
        )
        print(f"[{datetime.now()}] Task {task_id} FAILED, reverted to pending.")


async def main():
    print(f"[{datetime.now()}] Smart Worker v3.0 (AI-Driven) started...")
    while True:
        try:
            pool = await get_pool()
            # Find pending tasks
            ...
            if tasks:
                print(f"Found {len(tasks)} pending tasks. Processing sequentially...")
                for task in tasks:
