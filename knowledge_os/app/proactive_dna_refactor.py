...
"""
                ...
                # Используем LocalAIRouter напрямую для синхронного ожидания
                ...
                try:
                    ...
                    court_res = await self.court.verify_decision(
                        f"Refactor DNA for {expert_name}", 
                        new_dna
                    )

                    if not court_res.get("valid"):
                        logger.warning(f"🚨 [COURT] Refactor rejected: {court_res.get('violations')}")
                        return
    ...
    def _summarize_experience(self, events: List, logs: List) -> str:
        ...
        for l in logs:
            ...
            summary += f"Q: {l['user_query'][:50]} | Score: {l['feedback_score']} | Error: {error_val}\n"
