...
class PerpetualEvolution:
        ...
        while True:
            try:
                ...
                if warnings:
                    ...
                    task['implementation_plan'] = f"### [ГОЛОС ОПЫТА: ПРЕДУПРЕЖДЕНИЯ]\n{warnings}\n\n### [ПЛАН]\n{task['implementation_plan']}"
                ...
                if not court_result.get("valid"):
                    logger.warning(f"🚨 [EVOLUTION] CONSTITUTIONAL VETO! {court_result.get('violations')}")
                ...
                if success:
                    ...
                    await self.experience.log_failure(task['title'], "Implementation failed or timed out")
    ...
    async def research_next_upgrade(self) -> dict:
        ...
        logger.info("🔍 [EVOLUTION] Researching next upgrade from AI Giants (Google, OpenAI, Meta)...")
        
        # ВРЕМЕННЫЙ ПЛАН (Fallback), если модель галлюцинирует с форматом
        fallback_task = {
            "title": "Hierarchy of Reasoning (Adaptive Model Selection)",
            "reasoning": "Inspired by OpenAI's o1 and Google's Gemini dynamic routing. Optimizes local resource usage.",
            "implementation_plan": "1. Update LocalRouter to support 'thinking' models. 2. Implement logic to route complex queries to qwen3-coder:30b and simple ones to phi3.5.",
            "test_scenario": "Send a simple 'Hello' and a complex 'Write a refactor plan' and verify different models are used."
        }
        ...
        try:
            # Используем qwen3-coder:30b для более точного следования формату
            # ВАЖНО: Роутер возвращает (content_string, node_name)
            from local_router import LocalAIRouter
            router = LocalAIRouter()
            response_data = await router.run_local_llm(prompt, category="coding", model="qwen3-coder:30b")
    ...
    async def execute_upgrade(self, task: dict) -> bool:
        ...
        """, f"🚀 EVOLUTION: {task['title']}", task['implementation_plan'], json.dumps(task))
        await conn.close()
    ...
    async def verify_and_log(self, task: dict):
        ...
        """, f"✅ УСПЕШНОЕ ВНЕДРЕНИЕ: {task['title']}. {task['reasoning']}", json.dumps({"type": "evolution_log", "task": task}))
        await conn.close()
