...
class TaskPrioritizer:
    ...
    def calculate_priority_score(
        self, criticality: float, impact: float, complexity: float, dependencies_count: int
        ...
        try:
            ...
            if conn:
                try:
                    ...
                    logger.info(
                        f"✅ [TASK PRIORITIZER] Добавлена задача {task_id} с приоритетом {priority.name}"
                    )
                    return task_id
    ...
    async def assign_task(self, task_id: str, agent_id: str) -> bool:
        ...
        try:
            ...
            if task.assigned_agent and task.assigned_agent != agent_id:
                logger.warning(
                    f"⚠️ [TASK PRIORITIZER] Задача {task_id} уже назначена агенту {task.assigned_agent}"
                )
                return False
            ...
            if conn:
                try:
                    ...
                    logger.info(
                        f"✅ [TASK PRIORITIZER] Задача {task_id} назначена агенту {agent_id}"
                    )
                    return True
        ...
        try:
            ...
            if agent_id:
                ...
                if agent_load >= 5:  # Максимум 5 задач на агента
                    logger.debug(
                        f"⚠️ [TASK PRIORITIZER] Агент {agent_id} перегружен ({agent_load} задач)"
                    )
                    return None
    ...
    async def complete_task(self, task_id: str, success: bool = True) -> bool:
        ...
        try:
            ...
            if task.assigned_agent:
                self._agent_loads[task.assigned_agent] = max(
                    0, self._agent_loads.get(task.assigned_agent, 0) - 1
                )
            ...
            if conn:
                try:
                    ...
                    logger.info(
                        f"✅ [TASK PRIORITIZER] Задача {task_id} отмечена как {task.status.value}"
                    )
                    return True
