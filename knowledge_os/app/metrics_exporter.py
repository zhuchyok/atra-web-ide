    ...
    async def _get_latency_metrics(self) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                if stats:
                    metrics.append("# HELP singularity_latency_seconds Latency in seconds")
                    metrics.append("# TYPE singularity_latency_seconds summary")
                    metrics.append(
                        f'singularity_latency_seconds{{quantile="0.5"}} {stats["p50"] or 0}'
                    )
                    metrics.append(
                        f'singularity_latency_seconds{{quantile="0.95"}} {stats["p95"] or 0}'
                    )
                    metrics.append(
                        f'singularity_latency_seconds{{quantile="0.99"}} {stats["p99"] or 0}'
                    )
            finally:
                await conn.close()
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if stats and stats["hit_rate"]:
                    metrics.append("# HELP singularity_cache_hit_rate Cache hit rate")
                    metrics.append("# TYPE singularity_cache_hit_rate gauge")
    ...
    async def _get_token_metrics(self) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                if stats:
                    metrics.append("# HELP singularity_tokens_saved_total Total tokens saved")
                    metrics.append("# TYPE singularity_tokens_saved_total counter")
                    metrics.append(f"singularity_tokens_saved_total {stats['total_saved'] or 0}")

                    metrics.append("# HELP singularity_requests_total Total requests")
                    metrics.append("# TYPE singularity_requests_total counter")
        ...
        try:
            ...
            try:
                ...
                if stats and stats["error_rate"] is not None:
                    metrics.append("# HELP singularity_error_rate Error rate")
                    metrics.append("# TYPE singularity_error_rate gauge")
    ...
    async def _get_request_metrics(self) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                if stats:
                    metrics.append("# HELP singularity_requests_per_minute Requests per minute")
                    metrics.append("# TYPE singularity_requests_per_minute gauge")
                    metrics.append(
                        f"singularity_requests_per_minute {stats['requests_per_minute'] or 0}"
                    )
            finally:
                await conn.close()
        except Exception as e:
