...
async def check_model_available(model_name: str, mlx_url: str = None, timeout: float = 2.0) -> bool:
    ...
    for model in models:
        if model.get("name") == model_name:
            logger.debug(
                f"⚠️ Модель {model_name} найдена в MLX, но exists=False, пробуем использовать"
            )
...
async def select_available_model(
    priorities: List[str], mlx_url: str = None, category: str = "unknown"
) -> Optional[str]:
    """
...
async def get_best_model_for_category(
    category: str, model_priorities: Dict[str, List[str]], mlx_url: str = None
) -> Optional[str]:
    """
