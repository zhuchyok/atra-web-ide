...
class CollectiveBrainstorming:
    ...
    def __init__(self, topic: str, context: Optional[str] = None):
        ...
        self.experts = [
            {"name": "Игорь", "role": "Backend & Infrastructure Architect", "focus": "scalability, database, security, performance"},
            {"name": "Анна", "role": "Frontend & UX Specialist", "focus": "user interaction, state management, Svelte, UI/UX"},
            {"name": "Елена", "role": "QA & Security Engineer", "focus": "testing, edge cases, vulnerabilities, stability"},
            {"name": "Дмитрий", "role": "ML & Embedding Specialist", "focus": "vector search, embeddings, model optimization, pgvector"}
        ]
        self.lead = {"name": "Виктория", "role": "Team Lead & Strategist", "focus": "coordination, alignment with goals, final decision"}
    ...
    async def run_session(self) -> Dict[str, Any]:
        ...
        await self._run_phase("EXPLORATION", "Проанализируйте задачу и дайте краткий комментарий со своей точки зрения.")
        
        # Phase 2: Clarification (Experts ask each other questions)
        await self._run_phase("CLARIFICATION", "Задайте один уточняющий вопрос коллегам, чтобы прояснить неочевидные детали реализации.")
        
        # Phase 3: Approaches (Propose 2-3 approaches)
        await self._run_phase("APPROACHES", "Предложите 2-3 варианта реализации с учетом ваших компетенций. Укажите плюсы и минусы.")
...
"""
    ...
    async def _get_expert_response(self, expert: Dict, prompt: str, phase: str) -> Optional[BrainstormingMessage]:
        try:
            from ai_core import run_smart_agent_async
            content = await run_smart_agent_async(prompt, expert_name=expert['name'], category="reasoning")
            return BrainstormingMessage(sender=expert['name'], role=expert['role'], content=content, phase=phase)
...
"""
        from ai_core import run_smart_agent_async
        return await run_smart_agent_async(prompt, expert_name=self.lead['name'], category="reasoning")
