...
class PersonalityManager:
    ...
    def analyze_persona_needs(
        ...
        if len(query) < 50 or any(
            kw in query_lower for kw in ["быстро", "кратко", "статус", "чек", "fix it", "run"]
        ):
        ...
        if any(
            kw in query_lower
            for kw in ["почему", "архитектура", "как лучше", "сравнение", "выбор", "стратегия"]
        ):
...
""",
    ...
    def adapt_prompt(
        self, query: str, system_prompt: str, context: Optional[Dict[str, Any]] = None
    ) -> str:
        """
