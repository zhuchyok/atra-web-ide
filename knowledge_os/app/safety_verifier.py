...
class SafetyVerifier:
    ...
    async def verify_mutation(self, module_name: str, function_name: str, mutated_code: str) -> Dict[str, Any]:
        """
    ...
    async def _get_downstream_dependencies(self, module_name: str, function_name: str) -> List[Dict[str, Any]]:
    ...
    def _generate_audit_prompt(self, module: str, function: str, args: List[str], code: str, deps: List[Dict[str, Any]]) -> str:
        deps_str = json.dumps(deps, indent=2) if deps else "No downstream dependencies found in graph."
...
"""
    ...
    def _parse_audit_result(self, audit_json: str) -> Dict[str, Any]:
        try:
            if not audit_json:
                return {"safety_score": 0, "risks": ["No response from judge model"], "recommendation": "abort"}
