...
from digital_constitution import get_constitution_context, CONSTITUTION_PRINCIPLES
...
class ConstitutionalCourt:
    ...
    async def verify_decision(self, topic: str, decision: str) -> dict:
        ...
        # Используем qwen3-coder:30b для строгого следования формату
        from local_router import LocalAIRouter
        router = LocalAIRouter()
        response_data = await router.run_local_llm(prompt, category="reasoning", model="qwen3-coder:30b")
        ...
        try:
            ...
            if not result.get("valid"):
                logger.warning(f"🚨 [COURT] CONSTITUTIONAL VIOLATION DETECTED: {result.get('violations')}")
            else:
                logger.info("✅ [COURT] Decision is constitutionally valid.")
                
            return result
        except Exception as e:
            logger.error(f"❌ [COURT] Error parsing court decision: {e}. Raw response: {response[:200]}...")
            return {"valid": True, "violations": [], "feedback": "Ошибка проверки, пропущено по умолчанию."}
