...
class AutoPromptOptimizer:
    ...
    def _extract_keywords_from_prompt(self, prompt: str) -> List[str]:
        ...
        keywords = []
        important_phrases = [
            "кратко", "лаконично", "по делу", "структурированно",
            "с примерами", "с кодом", "детально", "профессионально"
        ]
    ...
    async def suggest_improvements(
        ...
        if negative_feedback:
            ...
            for feedback in negative_feedback:
                ...
                elif any(kw in comment or kw in query for kw in ['непонятно', 'неясно', 'запутанно']):
