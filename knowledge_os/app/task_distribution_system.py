...
async def _collect_and_review_by_managers(self, assignments: List[TaskAssignment]) -> List[Dict]:
    """
    ...
    for department, dept_assignments in by_department.items():
        for assignment in dept_assignments:
            ...
            reviewed_assignment = await self.manager_review_task(
                assignment, original_requirements=assignment.subtask
            )
            ...
            if reviewed_assignment.status == TaskStatus.REVIEWED:
                ...
                logger.warning(
                    f"⚠️ Задача {reviewed_assignment.task_id} не прошла проверку управляющим"
                )
    ...
    for department, dept_results in by_department.items():
        ...
        if not dept_head:
            ...
            try:
                ...
                synthesis_result = await victoria.solve(
                    synthesis_prompt, method="extended_thinking"
                )
                aggregated = synthesis_result.get("result", "") or synthesis_result.get(
                    "output", ""
                )
        ...
        try:
            ...
            logger.info(
                f"✅ [DEPARTMENT HEAD] {dept_head['name']} синтезировал результаты отдела '{department}'"
            )
