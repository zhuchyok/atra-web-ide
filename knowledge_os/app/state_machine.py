...
class StateGraph:
    ...
    def add_conditional_edges(
        self, from_node: str, condition_func: Callable, edge_map: Dict[str, str]
    ):
        """
    ...
    def _get_next_node(self, current_node: str, state: AgentState) -> Optional[str]:
        ...
        if len(candidates) == 0:
            ...
            logger.warning(
                f"⚠️ Несколько кандидатов для перехода из {current_node}, выбран: {candidates[0]}"
            )
            return candidates[0]
...
async def example_workflow():
    ...
    graph.add_conditional_edges(
        "victoria", route_decision, {"veronica": "veronica", "finish": "finish"}
    )
