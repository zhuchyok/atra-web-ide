...
class ResourceMonitor:
    ...
    def __init__(self):
        ...
        if self.mlx_url and not self.mlx_url.startswith("http"):
            self.mlx_url = f"http://{self.mlx_url}"
        if self.ollama_url and not self.ollama_url.startswith("http"):
    ...
    async def get_system_resources(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "disk": {"used_percent": disk_used_percent},
                "timestamp": datetime.now().isoformat(),
            }
            ...
            async with httpx.AsyncClient(timeout=2.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return {
                        ...
                        "is_overloaded": (
                            data.get("active_requests", 0) >= data.get("max_concurrent", 5)
                            or system_resources.get("ram", {}).get("used_percent", 0) > 90
                            or system_resources.get("cpu", {}).get("percent", 0) > 90
                        ),
                    }
        except Exception as e:
            ...
            async with httpx.AsyncClient(timeout=2.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return {
                        ...
                        "system": system_resources,
                        "is_overloaded": (
                            system_resources.get("ram", {}).get("used_percent", 0) > 90
                            or system_resources.get("cpu", {}).get("percent", 0) > 90
                        ),
                    }
        except Exception as e:
    ...
    def should_throttle_mlx(self, health: Dict) -> bool:
        ...
        return (
            health.get("is_overloaded", False)
            or ram_percent > 85
            or cpu_percent > 85
            or (temp and temp > 80)
        )
