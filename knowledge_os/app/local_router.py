...
# Мозг и руки — victoria-wisdom-30b. Быстрые задачи: сканер выбирает gemma3n:e4b при наличии, иначе tinyllama
OLLAMA_MODELS_FALLBACK = {
    "reasoning": "victoria-wisdom-30b",
    "coding": "victoria-wisdom-30b",
    "chat": "victoria-wisdom-30b",
    "fast": "tinyllama:1.1b-chat",  # при работающем сканере fast → gemma3n:e4b если есть
    "vision": "moondream",
...
class LocalAIRouter:
                ...
                try:
                    async with httpx.AsyncClient() as client:
                        ...
                        if category == "reasoning" or is_vip:
                            ...
                            logger.info(f"🌟 [VIP HEADER] Added X-Request-Priority: high for {category or 'VIP'}")
                        else:
    ...
    async def run_local_llm_streaming(
        ...
        if category == "reasoning" or is_vip:
            headers["X-Request-Priority"] = "high"
            logger.info(f"🌟 [VIP HEADER] Added X-Request-Priority: high for streaming {category or 'VIP'}")
