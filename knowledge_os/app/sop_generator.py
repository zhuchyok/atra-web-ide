...
class SOPGenerator:
    ...
    async def run_sop_cycle(self, limit: int = 3):
        ...
        """
        logger.info(f"📜 [SOP] Starting SOP generation cycle...")
    ...
    async def generate_sop(self, conn, task: asyncpg.Record):
        ...
        sop_content = await run_smart_agent_async(sop_prompt, expert_name="Виктория", category="reasoning")
        ...
        try:
            ...
            if not domain_id:
                domain_id = await conn.fetchval("INSERT INTO domains (name) VALUES ('SOP') RETURNING id")
            
            content_kn = f"📜 NEW SOP: {title}\nFile: docs/SOP/{filename}\n\nSummary: {sop_content[:500]}..."
            meta_kn = json.dumps({
                "type": "sop_document",
                "task_id": str(task_id),
