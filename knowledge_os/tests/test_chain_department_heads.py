...
async def test_chain_should_use_department_heads_returns_true_for_request_goal():
    ...
    assert department == "Strategy/Data", (
        "Просьба без ключевых слов должна дать отдел Strategy/Data"
    )
...
async def test_chain_execute_with_task_distribution_returns_result():
    ...
    """
    from app.task_distribution_system import TaskAssignment, TaskCollection, TaskStatus
    ...
    mock_task_dist.distribute_tasks_from_plan = AsyncMock(
        return_value=[assignment]
    )  # при task_plan_struct
    ...
    with patch(
        "app.task_distribution_system.get_task_distribution_system", return_value=mock_task_dist
    ):
        with patch.object(
            VictoriaEnhanced,
            "_synthesize_collected_results",
            new_callable=AsyncMock,
            return_value="Итог синтеза по анализу данных",
        ):
            v = VictoriaEnhanced()
...
async def test_chain_full_solve_returns_result_with_mocks():
    ...
    with patch.object(
        VictoriaEnhanced, "_should_use_department_heads", side_effect=mock_should_use
    ):
        with patch(
            "app.task_distribution_system.get_task_distribution_system", return_value=mock_task_dist
        ):
            with patch.object(
                ...
                "_synthesize_collected_results",
                new_callable=AsyncMock,
                return_value="Финальный итог по анализу данных",
            ):
                v = VictoriaEnhanced()
