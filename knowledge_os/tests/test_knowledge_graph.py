...
async def test_create_link(db_connection, test_domain_id, knowledge_nodes_id_is_uuid):
    ...
    if not knowledge_nodes_id_is_uuid:
        pytest.skip(
            "knowledge_nodes.id не UUID — knowledge_links ожидает UUID (см. init.sql vs бэкап БД)"
        )
    ...
    link_id = await graph.create_link(
        str(node1_id), str(node2_id), LinkType.DEPENDS_ON, strength=0.9
    )
...
async def test_get_related_nodes(db_connection, test_domain_id, knowledge_nodes_id_is_uuid):
    ...
    # Create link
    await graph.create_link(str(node1_id), str(node2_id), LinkType.RELATED_TO)
