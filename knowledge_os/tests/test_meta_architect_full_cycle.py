...
async def test_meta_architect_full_cycle():
    ...
    with patch('knowledge_os.app.meta_architect.get_profiler', return_value=mock_profiler), \
         patch('knowledge_os.app.meta_architect.get_graphrag_service', return_value=mock_graphrag), \
         patch('knowledge_os.app.meta_architect.run_smart_agent_async', side_effect=mock_run_smart_agent), \
         patch('knowledge_os.app.safety_verifier.run_smart_agent_async', side_effect=mock_run_smart_agent), \
        ...
        mock_sv = MagicMock()
        mock_sv.verify_mutation = AsyncMock(return_value={
            "safety_score": 90,
            "risks": [],
            "recommendation": "proceed"
        })
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            assert "architecture_mutation" in args[2] # metadata is the 3rd arg in the second execute call? 
