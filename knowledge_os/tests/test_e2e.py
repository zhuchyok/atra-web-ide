...
from knowledge_os.app.contextual_learner import AdaptiveLearner, ContextualMemory
from knowledge_os.app.knowledge_graph import KnowledgeGraph, LinkType
from knowledge_os.app.security import Role, SecurityManager


@pytest.mark.asyncio
async def test_e2e_knowledge_creation_and_linking(
    db_connection, test_domain_id, knowledge_nodes_id_is_uuid
):
    ...
    # 2. Create link
    link_id = await graph.create_link(str(node1_id), str(node2_id), LinkType.ENHANCES)
    ...
    await db_connection.execute(
        "DELETE FROM knowledge_nodes WHERE id IN ($1, $2)", node1_id, node2_id
    )
...
async def test_e2e_user_authentication_flow(db_connection):
    ...
    user_id = await security.create_user(
        username="e2e_test_user", password="testpassword123", role=Role.USER, email="e2e@test.com"
    )
...
async def test_e2e_contextual_learning_flow(db_connection, test_expert_id):
    ...
    pattern_id = await memory.save_pattern(
        "query_pattern", "Test query", "Test response", 0.9, domain="test"
    )
