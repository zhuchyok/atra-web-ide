...
RUN_MULTIHOP_INTEGRATION_TESTS = os.getenv("RUN_MULTIHOP_INTEGRATION_TESTS", "false").lower() in (
    "1",
    "true",
    "yes",
)
pytestmark = pytest.mark.skipif(
    not RUN_MULTIHOP_INTEGRATION_TESTS,
    reason="Multi-hop DB integration tests are opt-in. Set RUN_MULTIHOP_INTEGRATION_TESTS=true.",
)
...
async def db_connection():
    try:
        ...
        pytest.skip(f"DB unavailable for multihop integration tests: {exc}")
...
async def test_domain_id(db_connection):
    ...
    await db_connection.execute(
        ...
        domain_id,
        f"test_domain_{domain_id.hex[:8]}",
    )
    yield domain_id
    # Cleanup
    await db_connection.execute(
        "DELETE FROM knowledge_links WHERE source_node_id IN (SELECT id FROM knowledge_nodes WHERE domain_id = $1)",
        domain_id,
    )
...
async def test_retrieve_with_hops_depth_limit(db_connection, test_domain_id):
    ...
    assert f"{prefix}_4_E" not in result_contents, (
        f"Node E should be excluded by depth limit (max 3). Results: {result_contents}"
    )
...
async def test_retrieve_with_hops_caching(db_connection, test_domain_id):
    ...
    assert any(r["content"] == f"{prefix}_A" for r in results1), (
        f"{prefix}_A not found in results: {result_contents1}"
    )
    assert any(r["content"] == f"{prefix}_B" for r in results1), (
        f"{prefix}_B not found in results: {result_contents1}"
    )
