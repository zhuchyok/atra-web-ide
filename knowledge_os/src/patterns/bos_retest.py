...
class ImprovedBOSRetestSystem:
    ...
    def check_bos_retest_improved(
        self, df: pd.DataFrame, i: int, symbol: str = None
        ...
        try:
            ...
            if not bos_result["has_bos"]:
                return (
                    False,
                    f"No BOS detected ({regime})",
                    {"regime": regime, "bos_result": bos_result, "structure": structure_data},
                )

            # Проверяем Retest с зонами
            retest_result = self._check_retest_improved(df, i, bos_result, structure_data, params)
            ...
            if retest_result["has_retest"] or time_bypass["bypass"]:
                ...
                return (
                    False,
                    f"BOS detected but no retest ({regime})",
                    {
                        "regime": regime,
                        "bos_result": bos_result,
                        "retest_result": retest_result,
    ...
    def _check_break_of_structure(
        self, current_price: float, zones: List[Dict], regime: str
    ) -> Dict[str, Any]:
    ...
    def _check_retest_improved(
        self, df: pd.DataFrame, i: int, bos_result: Dict, structure_data: Dict, params: Dict
    ) -> Dict[str, Any]:
    ...
    def _check_time_based_bypass(
        self, df: pd.DataFrame, i: int, bos_result: Dict, bypass_hours: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "reason": f"Time bypass after {time_elapsed}h"
                if bypass
                else f"Not enough time: {time_elapsed}h",
            }
...
def check_improved_bos_retest(df: pd.DataFrame, i: int, symbol: str = None) -> Tuple[bool, str]:
    ...
    """
    try:
        passed, message, details = improved_bos_retest_system.check_bos_retest_improved(
            df, i, symbol
        )
