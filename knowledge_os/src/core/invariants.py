...
from src.core.self_validation import ValidationLevel, get_validation_manager
    ...
    def check_long_tp_above_entry(signal):
        ...
        if signal.signal_type.upper() == "LONG":
            return (
                ...
                and signal.take_profit_2 > signal.entry_price
            )
        return True
            ...
            if hasattr(position, "pnl"):
                ...
                try:
                    ...
                    current = (
                        ...
                        if isinstance(position.current_price, Decimal)
    ...
    def check_order_filled_quantity_valid(order):
        ...
        if hasattr(order, "quantity") and hasattr(order, "filled_quantity"):
            ...
            filled = (
                ...
                if isinstance(order.filled_quantity, Decimal)
...
def register_risk_invariants():
    ...
    def check_risk_amount_within_balance(calculator):
        ...
        if hasattr(calculator, "risk_amount") and hasattr(calculator, "account_balance"):
            risk = (
                float(calculator.risk_amount)
                if isinstance(calculator.risk_amount, Decimal)
                else calculator.risk_amount
            )
            balance = (
                float(calculator.account_balance)
                if isinstance(calculator.account_balance, Decimal)
                else calculator.account_balance
            )
            return risk <= balance
        return True
    ...
    def check_risk_percentage_valid(calculator):
        ...
        if hasattr(calculator, "risk_percentage"):
            risk_pct = (
                float(calculator.risk_percentage)
                if isinstance(calculator.risk_percentage, Decimal)
                else calculator.risk_percentage
            )
    ...
    def check_portfolio_total_risk_valid(portfolio):
        ...
        if hasattr(portfolio, "total_risk") and hasattr(portfolio, "max_portfolio_risk"):
            total = (
                float(portfolio.total_risk)
                if isinstance(portfolio.total_risk, Decimal)
                else portfolio.total_risk
            )
            max_risk = (
                float(portfolio.max_portfolio_risk)
                if isinstance(portfolio.max_portfolio_risk, Decimal)
                else portfolio.max_portfolio_risk
            )
