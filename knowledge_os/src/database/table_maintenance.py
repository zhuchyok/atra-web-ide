...
class TableMaintenance:
        ...
        try:
            ...
            for table_row in tables:
                ...
                try:
                    ...
                    logger.warning(
                        "⚠️ [TableMaintenance] Ошибка анализа таблицы %s: %s", table_name, e
                    )
            ...
            result = {
                ...
                "fragmentation_pct": (total_fragmented / total_size * 100)
                if total_size > 0
                else 0.0,
                "tables": [
                    {
            ...
            return {"error": str(e), "total_tables": 0, "tables_needing_vacuum": 0}
    ...
    def _analyze_table(self, table_name: str) -> TableStats:
        ...
        page_info = self.db.execute_with_retry("PRAGMA page_count", (), is_write=False)
        page_count = page_info[0][0] if page_info else 0

        # Получаем размер страницы
        page_size_info = self.db.execute_with_retry("PRAGMA page_size", (), is_write=False)
    ...
    def get_vacuum_recommendations(self) -> List[str]:
        ...
        return [table["name"] for table in analysis["tables"] if table.get("needs_vacuum", False)]
