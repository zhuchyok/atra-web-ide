...
class ColumnOrderOptimizer:
    ...
    def optimize_column_order(
        ...
        logger.debug(
            ...
            table_name,
            len(fixed_size_columns),
            len(variable_size_columns),
        )
    ...
    def analyze_table_column_order(db, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            columns_info = db.execute_with_retry(
                f"PRAGMA table_info({table_name})", (), is_write=False
            )
            ...
            fixed_count = sum(
                1 for col in columns if col["type"].upper() in ["INTEGER", "REAL", "BOOLEAN"]
            )
