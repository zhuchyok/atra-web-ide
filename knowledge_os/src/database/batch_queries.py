...
class BatchQueryExecutor:
    ...
    def execute_batch(self) -> Dict[str, Any]:
        ...
        try:
            ...
            select_queries = [
                q for q in self._pending_queries if q.query.strip().upper().startswith("SELECT")
            ]
            other_queries = [
                q for q in self._pending_queries if not q.query.strip().upper().startswith("SELECT")
            ]
                ...
                for query_obj in select_queries:
                    try:
                        result = self.db.execute_with_retry(
                            query_obj.query, query_obj.params, is_write=False
                        )
                        if query_obj.key:
