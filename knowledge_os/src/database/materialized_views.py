...
class MaterializedViewManager:
    ...
    def create_materialized_view(
        self, view_name: str, base_query: str, refresh_interval_minutes: int = 60
    ) -> bool:
        """
        ...
        try:
            ...
            sample_result = self.db.execute_with_retry(
                base_query + " LIMIT 1", (), is_write=False, use_prepared=False
            )

            if not sample_result:
                logger.warning(
                    "⚠️ [MaterializedView] Нет данных для создания представления %s", view_name
                )
                return False
    ...
    def refresh_view(self, view_name: str, force: bool = False) -> bool:
        ...
        try:
            ...
            results = self.db.execute_with_retry(
                view.base_query, (), is_write=False, use_prepared=False
            )

            # Очищаем старый кэш
            self.db.execute_with_retry(f"DELETE FROM {cache_table}", (), is_write=True)
            ...
            logger.error(
                "❌ [MaterializedView] Ошибка обновления представления %s: %s", view_name, e
            )
            return False
    ...
    def get_view_stats(self) -> Dict[str, Any]:
        ...
        """
        stats = {"total_views": len(self.views), "views": []}
            ...
            stats["views"].append(
                {
                    ...
                    "last_refreshed": view.last_refreshed.isoformat()
                    if view.last_refreshed
                    else None,
