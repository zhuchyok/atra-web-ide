...
class AutonomousRollbackManager:
        ...
        while True:
            try:
                ...
                if is_abnormal:
                    ...
                    if get_utc_now() - last_snapshot_time >= timedelta(
                        hours=self.snapshot_interval_hours
                    ):
    ...
    async def _maybe_save_stable_snapshot(self):
        ...
        try:
            ...
            with self.db._lock:
                cur = self.db.conn.execute(
                    "SELECT pnl_percent FROM trades WHERE exit_time >= ?", (since,)
                )
                trades = cur.fetchall()
            ...
            if win_rate >= self.min_win_rate_for_stable:
                ...
                if settings:
                    logger.info(
                        f"✅ Performance is stable (WR={win_rate:.1f}%, PnL={total_pnl:.2f}%). Saving snapshot..."
                    )
    ...
    async def _perform_rollback(self, reason: str):
        ...
        try:
            ...
            with open("logs/evolution_steps.log", "a") as f:
                f.write(
                    f"{get_utc_now().isoformat()} — EMERGENCY ROLLBACK: {reason}. Reverted to last stable config.\n"
                )

            await self._alert_user(
                f"🛡️ <b>EMERGENCY ROLLBACK</b>\n\nReason: {reason}\n✅ System settings reverted to last stable configuration."
            )
