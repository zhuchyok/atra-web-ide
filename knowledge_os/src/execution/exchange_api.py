...
async def check_pending_symbols():
    ...
    try:
            ...
            if market_cap == 0:
                ...
                if vol >= 50_000_000:
                    ...
                    if not cursor.fetchone():
                        ...
                        logging.info(
                            "✅ %s: 24h volume %.1fM USD -> белый список", symbol, vol / 1_000_000
                        )
                    else:
                    ...
                    if vol < 25_000_000 and attempts >= 3:
                        ...
                        if not cursor.fetchone():
                            ...
                            logging.info(
                                "❌ %s: нет капы, 24h volume %.1fM, attempts=%d -> черный список",
                                symbol,
                                vol / 1_000_000,
                                attempts,
                            )
                        ...
                        logging.debug(
                            "⏳ %s: данных о капе нет (vol=%.1fM), оставляем pending",
                            symbol,
                            vol / 1_000_000,
                        )
                ...
                if not cursor.fetchone():
                    ...
                    moved_to_whitelist.append(symbol)
                    logging.info(
                        "✅ %s: %.1fM USD -> переводим в белый список",
                        symbol,
                ...
                if not cursor.fetchone():
                    ...
                    moved_to_blacklist.append(symbol)
                    logging.info(
                        "❌ %s: %.1fM USD -> переводим в черный список",
                        symbol,
                        market_cap / 1_000_000,
                    )
                else:
        ...
        logging.info(
            "✅ Проверка завершена: %d в белый список, %d в черный список, %d остались на проверке",
            len(moved_to_whitelist),
            len(moved_to_blacklist),
            len(still_pending),
        )
    ...
    try:
        ...
        for symbol in blacklisted_symbols:
            ...
            if market_cap >= min_market_cap:
                ...
                logging.info(
                    "⬆️ %s: %.1fM USD -> переводим в белый список", symbol, market_cap / 1_000_000
                )
    ...
    try:
        ...
        for symbol in whitelisted_symbols:
            ...
            if market_cap < min_market_cap:
                ...
                logging.info(
                    "⬇️ %s: %.1fM USD -> переводим в черный список", symbol, market_cap / 1_000_000
                )
...
__all__ = [
    ...
    "get_market_cap_data_aggressive",
    "try_all_sources_for_symbol",
]
