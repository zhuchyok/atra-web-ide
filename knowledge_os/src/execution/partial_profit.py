...
try:
    from src.filters.exhaustion_filter import (
        check_exhaustion_early_exit,
        get_exhaustion_recommendation,
    )
...
class PartialProfitManager:
    ...
    def setup_partial_take_profit(
        ...
        try:
            ...
            if position_size_usdt < self.settings["min_position_size_usdt"]:
                logger.debug(
                    "⚠️ [PARTIAL TP] %s: позиция слишком мала (%.2f USDT < %.2f)",
                    symbol,
                    position_size_usdt,
                    self.settings["min_position_size_usdt"],
                )
                return False
            ...
            logger.info(
                "🎯 [PARTIAL TP] %s %s: TP1=%.4f (+%.2f%%), TP2=%.4f (+%.2f%%), split=%d%%/%d%%",
                symbol,
                side,
                tp1_price,
                tp1_pct,
                tp2_price,
                tp2_pct,
                self.settings["tp1_split_pct"],
                self.settings["tp2_split_pct"],
            )
    ...
    def check_profit_targets(
        ...
        try:
            ...
            if side == "LONG":
                current_profit_pct = (
                    (current_price - targets["entry_price"]) / targets["entry_price"]
                ) * 100
                tp1_reached = current_price >= targets["tp1_price"]
                tp2_reached = current_price >= targets["tp2_price"]
            else:  # SHORT
                current_profit_pct = (
                    (targets["entry_price"] - current_price) / targets["entry_price"]
                ) * 100
                tp1_reached = current_price <= targets["tp1_price"]
                tp2_reached = current_price <= targets["tp2_price"]
            ...
            if (
                ...
                and current_index < len(df)
                and current_profit_pct > 0
            ):  # Только если позиция в прибыли
                try:
                        ...
                        if exhaustion_rec.get("should_exit"):
                            ...
                            if exit_type == "full" or targets["tp1_executed"]:
                                ...
                                return {
                                    ...
                                    "close_size_usdt": targets.get(
                                        "tp2_size_usdt", targets["position_size_usdt"]
                                    ),
                                    "close_percent": 100.0,
            ...
            if not targets["tp1_executed"] and tp1_reached:
                ...
                logger.info(
                    "✅ [TP1 HIT] %s %s: %.4f (+%.2f%%), закрываем %.2f USDT (50%% позиции)",
                    symbol,
                    side,
                    current_price,
                    current_profit_pct,
                    targets["tp1_size_usdt"],
                )
                ...
                if self.settings["move_sl_to_be_after_tp1"] and not targets["sl_moved_to_be"]:
                    ...
                    if side == "LONG":
                        breakeven_sl = targets["entry_price"] * (
                            1 + self.settings["breakeven_offset_pct"] / 100
                        )
                    else:  # SHORT
                        breakeven_sl = targets["entry_price"] * (
                            1 - self.settings["breakeven_offset_pct"] / 100
                    ...
                    logger.info(
                        "🛡️ [SL→BE] %s: SL перемещен в безубыток %.4f (+%.2f%%)",
                        symbol,
                        breakeven_sl,
                        self.settings["breakeven_offset_pct"],
                    )
            ...
            # TP2 достигнут?
                ...
                logger.info(
                    "🎉 [TP2 HIT] %s %s: %.4f (+%.2f%%), закрываем %.2f USDT (остаток 50%%)",
                    symbol,
                    side,
                    current_price,
                    current_profit_pct,
                    targets["tp2_size_usdt"],
                )
