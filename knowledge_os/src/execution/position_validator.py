...
class PositionSizeValidator:
    ...
    @precondition(
        lambda self, amount_usdt, user_balance, current_exposure_usdt: (
            amount_usdt >= 0 and user_balance >= 0 and current_exposure_usdt >= 0
        ),
    ...
    @postcondition(
        lambda result, self, amount_usdt, user_balance, current_exposure_usdt: (
            ...
            and "allowed" in result
            and "adjusted_amount" in result
            and "reason" in result
            and isinstance(result["allowed"], bool)
            and isinstance(result["adjusted_amount"], (int, float))
            and result["adjusted_amount"] >= 0
        ),
        "Invalid output: result must be dict with 'allowed', 'adjusted_amount', 'reason'",
    )
    async def validate_order_size(
        self, amount_usdt: float, user_balance: float, current_exposure_usdt: float = 0.0
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"allowed": False, "adjusted_amount": 0.0, "reason": f"Ошибка валидации: {e}"}
