...
def check_microstructure_filter(
    ...
    try:
        ...
        liquidity_zones = vp_analyzer.get_liquidity_zones(
            df.iloc[: i + 1], lookback_periods=100 if strict_mode else lookback
        )
        ...
        if side.lower() == "long":
            ...
            if strict_mode:
                ...
                return (
                    False,
                    f"LONG: цена не вблизи поддержки (price={current_price:.2f}, нет сильных уровней)",
                )
            else:
                if near_support or near_absorption:
                    reason = support_reason or absorption_reason
            ...
            if strict_mode:
                ...
                return (
                    False,
                    f"SHORT: цена не вблизи сопротивления (price={current_price:.2f}, нет сильных уровней)",
                )
            else:
                if near_resistance or near_absorption:
                    reason = resistance_reason or absorption_reason
