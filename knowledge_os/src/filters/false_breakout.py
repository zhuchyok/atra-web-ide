...
class FalseBreakoutDetector:
    ...
    async def analyze_breakout_quality(
        ...
        try:
            ...
            if is_false_breakout:
                ...
                logger.info("🚫 [FALSE BREAKOUT] %s %s: уверенность %.2f < %.2f (vol: %.2f, mom: %.2f, lvl: %.2f)",
                ...
                logger.debug("✅ [TRUE BREAKOUT] %s %s: уверенность %.2f (vol: %.2f, mom: %.2f, lvl: %.2f)",
    ...
    def _determine_confidence_threshold(
        ...
        if ml_override is not None:
            ...
            logger.debug("🤖 [ML_FALSE_BREAKOUT] Используем ML оптимизированный порог: %.3f", threshold)
            ...
            if regime_confidence is not None:
                threshold -= (0.05 * (1 - float(regime_confidence)))  # расслабляем при низкой уверенности
