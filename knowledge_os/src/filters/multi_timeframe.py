...
class MultiTimeframeConfirmer:
    ...
    async def check_confirmation(
        self, symbol: str, signal_type: str, price: float, df_h4: Optional[pd.DataFrame] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        if not self.enabled:
            return {"confirmed": True, "confidence": 1.0, "reason": "MTF confirmation disabled"}
        ...
        try:
            ...
            return {"confirmed": True, "confidence": 0.5, "reason": f"Error: {e}"}

    async def _check_h4_confirmation(
        self, df_h4: pd.DataFrame, signal_type: str, price: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"confirmed": confirmed, "confidence": confidence, "reason": reason}

        except Exception as e:
            logger.error("[MTF] Ошибка проверки H4: %s", e)
            return {"confirmed": True, "confidence": 0.5, "reason": f"H4 check error: {e}"}
