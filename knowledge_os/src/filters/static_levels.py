...
class StaticLevelsDetector:
    ...
    def _cluster_levels(
        self, extremes: List[Dict], tolerance_pct: float = 0.5, min_touches: int = 2
    ) -> List[Dict]:
        """
                    ...
                    clusters.append(
                        {
                            ...
                            "range": max(e["price"] for e in current_cluster)
                            - min(e["price"] for e in current_cluster),
                        }
                    )
    ...
    def is_near_level(
        self, price: float, levels: List[Dict], tolerance_pct: float = 1.0
    ...
    def get_levels_quality_bonus(
        self, price: float, side: str, levels: Dict[str, List[Dict]], max_bonus: float = 0.15
    ) -> float:
        """
        ...
        try:
            if side.upper() == "BUY":
                ...
                is_near, support = self.is_near_level(
                    price, levels.get("support", []), tolerance_pct=1.0
                )
                if is_near and support:
                    # Бонус пропорционален силе уровня
                    strength_bonus = min(support["strength"] * 0.02, max_bonus)
                    logger.debug(
                        f"✅ Бонус к LONG: близость к поддержке (сила: {support['strength']}, бонус: {strength_bonus:.2%})"
                    )
                    return strength_bonus

            elif side.upper() == "SELL":
                # Для SHORT: ищем близость к сопротивлению
                is_near, resistance = self.is_near_level(
                    price, levels.get("resistance", []), tolerance_pct=1.0
                )
                if is_near and resistance:
                    # Бонус пропорционален силе уровня
                    strength_bonus = min(resistance["strength"] * 0.02, max_bonus)
                    logger.debug(
                        f"✅ Бонус к SHORT: близость к сопротивлению (сила: {resistance['strength']}, бонус: {strength_bonus:.2%})"
                    )
                    return strength_bonus
