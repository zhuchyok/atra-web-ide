...
def check_trend_strength_filter(
    ...
    try:
        ...
        if not adx_ok:
            return (
                False,
                f"Слабый тренд (ADX={adx_value:.2f}, требуется > {'25' if strict_mode else adx_threshold})",
            )
        ...
        if side.lower() == "long":
            ...
            if require_direction or strict_mode:
                ...
                return (
                    False,
                    f"LONG: нет подтверждения восходящего тренда (ADX direction={adx_direction}, TSI={tsi_value:.2f})",
                )
            ...
            if require_direction or strict_mode:
                ...
                return (
                    False,
                    f"SHORT: нет подтверждения нисходящего тренда (ADX direction={adx_direction}, TSI={tsi_value:.2f})",
                )
