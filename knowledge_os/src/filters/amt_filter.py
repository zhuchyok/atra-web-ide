...
try:
    from src.metrics.decorators import FilterType, record_filter_metrics
...
def check_amt_filter(
    ...
    try:
        ...
        if phase == MarketPhase.BALANCE:
            ...
            result = (
                False,
                f"AMT: рынок в фазе баланса (balance_score={balance_score:.3f}), входы блокированы",
            )
            # Логирование метрик
            ...
            if side.lower() == "long":
                ...
                if balance_score > 0.5:
                    ...
                    if PROMETHEUS_METRICS_AVAILABLE:
                        try:
                            symbol = (
                                df.get("symbol", "unknown") if hasattr(df, "get") else "unknown"
                            )
                    ...
                    result = (
                        False,
                        f"AMT: дисбаланс в пользу продавцов (balance_score={balance_score:.3f}), LONG блокирован",
                    )
                    if METRICS_AVAILABLE:
                        try:
                            record_filter_metrics(
                ...
                if balance_score < 0.5:
                    ...
                    result = (
                        False,
                        f"AMT: дисбаланс в пользу покупателей (balance_score={balance_score:.3f}), SHORT блокирован",
                    )
                    if METRICS_AVAILABLE:
                        try:
                            record_filter_metrics(
            ...
            if strict_mode:
                result = (
                    False,
                    f"AMT: рынок в фазе аукциона (balance_score={balance_score:.3f}), входы блокированы в строгом режиме",
                )
                if METRICS_AVAILABLE:
                    try:
                        record_filter_metrics(
                ...
                if (
                    ...
                    result = (
                        False,
                        f"AMT: аукцион не подтверждает {side.upper()} (balance_score={balance_score:.3f})",
                    )
                    if METRICS_AVAILABLE:
                        try:
                            record_filter_metrics(
