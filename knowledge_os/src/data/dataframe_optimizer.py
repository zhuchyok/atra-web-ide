    ...
    try:
        ...
        for col in df.select_dtypes(include=["object"]).columns:
            ...
            if num_unique_values < num_total_values * 0.5:  # Если уникальных значений < 50%
                ...
                logger.debug(
                    "Конвертирован столбец %s в категорию (%d уникальных из %d)",
                    col,
                    num_unique_values,
                    num_total_values,
                )
        ...
        for col in df.select_dtypes(include=["float64"]).columns:
            ...
            if any(
                x in col.lower() for x in ["price", "close", "open", "high", "low", "exit", "entry"]
            ):
        ...
        if reduction_pct > 5:  # Логируем только если снижение > 5%
            logger.info(
                "Оптимизация DataFrame: память снижена на %.1f%% (%.1f MB -> %.1f MB)",
                reduction_pct,
                original_memory / 1024**2,
                optimized_memory / 1024**2,
            )
