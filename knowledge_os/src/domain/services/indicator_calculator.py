...
class IndicatorCalculator:
    ...
    def _calculate_adx_python(
        ...
        atr = self._calculate_ema_python(
            tr, 2 * period - 1
        )  # Wilder uses 2n-1 period for EMA equiv
        plus_di = [
            100 * p / val if val > 0 else 0
            for p, val in zip(self._calculate_ema_python(plus_dm, 2 * period - 1), atr)
        ]
        minus_di = [
            100 * m / val if val > 0 else 0
            for m, val in zip(self._calculate_ema_python(minus_dm, 2 * period - 1), atr)
        ]
