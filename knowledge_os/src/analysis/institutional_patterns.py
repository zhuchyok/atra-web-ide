...
class IcebergOrderDetector:
    ...
    def detect_iceberg_pattern(
        ...
        try:
            ...
            large_volume_indices = np.where(
                volumes > mean_volume + self.large_trade_threshold * std_volume
            )[0]
...
class SpoofingDetector:
    ...
    def detect_spoofing_pattern(
        ...
        try:
            ...
            mean_volume = np.mean(volumes)
            mean_price_change = (
                np.mean(price_changes[price_changes > 0]) if np.any(price_changes > 0) else 0
            )
            ...
            for j in range(1, len(volumes)):
                if price_changes[j] > 0:
                    ratio = (
                        volumes[j] / (price_changes[j] * mean_volume / mean_price_change)
            ...
            high_ratio_count = sum(
                1 for r in volume_price_ratios if r > 1.0 + self.volume_price_divergence_threshold
            )
            ...
            confidence = min(
                ...
                (
                    ...
                    + (current_volume / mean_volume - 1.0) * 0.2
                    if mean_volume > 0
                    else 0.0
                ),
            )
