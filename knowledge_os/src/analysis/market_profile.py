...
class TimePriceOpportunity:
    ...
    def calculate_tpo_profile(
        ...
        try:
            ...
            for _, row in recent_df.iterrows():
                ...
                if price_range == 0:
                    ...
                    for i in range(num_points):
                        price = (
                            row["low"] + (row["high"] - row["low"]) * (i / (num_points - 1))
                            if num_points > 1
                            else row["close"]
                        )
            ...
            # Создаем TPO профиль
            tpo_profile = {
                float(center): float(time)
                for center, time in zip(bin_centers, bin_times)
                if time > 0
            }
    ...
    def combine_with_volume_profile(
        ...
        try:
            ...
            # Взвешенное среднее
            combined_poc = (volume_poc * weight_volume + tpo_poc * weight_time) / (
                weight_volume + weight_time
            )
