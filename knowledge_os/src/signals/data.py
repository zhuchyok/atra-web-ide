...
def get_symbols() -> List[str]:
    ...
    try:
        ...
        if symbols:
            # Дополнительная фильтрация стейблкоинов и некорректных символов
            from config import STABLECOIN_SYMBOLS

            # Фильтруем:
            # 1. Стейблкоины
            # 2. Дублированные символы (CAKEUSDTUSDT, USDEUSDTUSDT и т.д.)
            # 3. Символы не заканчивающиеся на USDT
            filtered_symbols = [
                s for s in symbols
