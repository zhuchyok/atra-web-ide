...
def track_filter_metrics(filter_type: FilterType, rejection_reason_key: str = "rejection_reason"):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                logger.debug(
                    f"Метрики фильтра {filter_type.value}: "
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                processing_time = time.time() - start_time
                logger.debug(
                    f"Функция {func.__name__} ({metric_name}): "
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                processing_time = time.time() - start_time
                logger.info(f"Сигнал {signal_type}: время обработки {processing_time:.4f}s")
                ...
                processing_time = time.time() - start_time
                logger.error(
                    f"Ошибка в сигнале {signal_type}: "
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                processing_time = time.time() - start_time
                logger.debug(f"API {api_name}: время ответа {processing_time:.4f}s")
                ...
                processing_time = time.time() - start_time
                logger.error(
                    f"Ошибка API {api_name}: время до ошибки {processing_time:.4f}s, ошибка: {e}"
                )
                raise
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                processing_time = time.time() - start_time
                logger.debug(f"БД {operation_type}: время выполнения {processing_time:.4f}s")
...
class MetricsContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            logger.debug(
                f"Метрики {self.metric_name}: passed={self.passed}, time={processing_time:.4f}s"
            )
