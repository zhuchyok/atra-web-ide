    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if pending_count < 10:
                ...
                for i in range(10 - pending_count):
                    await conn.execute(
                        ...
                        f"Test Task {i}", "Backpressure test task"
                    )
                pending_count = 10
            ...
            if pending_count >= max_pending:
                print(f"SUCCESS: Backpressure would trigger (pending_count={pending_count} >= max_pending={max_pending})")
            else:
                print(f"FAILURE: Backpressure would NOT trigger (pending_count={pending_count} < max_pending={max_pending})")
