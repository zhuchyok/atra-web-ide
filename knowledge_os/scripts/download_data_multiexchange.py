...
def download_from_bybit(symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        df = pd.DataFrame(
            all_data,
            columns=["startTime", "open", "high", "low", "close", "volume", "turnover", "ignore"],
        )
...
def download_from_kucoin(symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        for _ in range(9):
            params = {"symbol": kucoin_symbol, "type": "1hour", "endAt": str(end_time)}
        ...
        # Конвертируем в DataFrame
        df = pd.DataFrame(
            all_data, columns=["time", "open", "close", "high", "low", "volume", "amount"]
        )
...
def download_from_coingecko(symbol: str) -> pd.DataFrame:
    ...
    try:
        print("   🔄 Пробуем CoinGecko...")
        # CoinGecko требует ID монеты, не символ
        # Это сложнее, пропускаем пока
        print("   ⚠️ CoinGecko: требует mapping символов на ID, пропускаем")
        return None
    except Exception as e:
...
def main():
    ...
    print("🔍 Биржи: Binance -> Bybit -> KuCoin")
    print()
