...
try:
    ...
    if _venv.exists():
        print(
            "❌ asyncpg не установлен в текущем окружении. Запустите: "
            + str(_venv)
            + " "
            + " ".join(sys.argv)
        )
        ...
        print(
            "❌ asyncpg не установлен. Установите: bash knowledge_os/scripts/setup_knowledge_os.sh или pip install asyncpg"
        )
    sys.exit(1)
...
DB_URL = os.getenv(
    ...
    os.getenv(
        "POSTGRES_URL",
        os.getenv("DATABASE_URL_LOCAL", "postgresql://admin:secret@localhost:5432/knowledge_os"),
    ),
)
    ...
    try:
        ...
        try:
            ...
            if rows:
                ...
                print(
                    "❌ Индекс на knowledge_nodes.embedding не найден. Примените миграции: python scripts/apply_migrations.py"
                )
            return 1
        finally:
            await conn.close()
