...
# Kill old ones
subprocess.run(
    "fuser -k 8000/tcp; pkill -f streamlit; pkill -f main_enhanced.py; pkill -f vector_core.py; pkill -f telegram_simple.py; pkill -f nightly_learner.py; pkill -f enhanced_orchestrator.py; pkill -f smart_worker",
    shell=True,
)
time.sleep(2)
...
# 1. Dashboard
start(
    f"cd /root/knowledge_os/dashboard && {PYTHON_EXEC} -m streamlit run app.py --server.port 5002 --server.address 0.0.0.0",
    "/root/dashboard_new.log",
)
...
start(
    f"while true; do {PYTHON_EXEC} /root/knowledge_os/app/enhanced_orchestrator.py; sleep 300; done",
    "/root/orchestrator_new.log",
)
