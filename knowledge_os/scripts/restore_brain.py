...
def restore_system(backup_path):
    ...
    if not run_cmd(f"tar -xzf {backup_path} -C {temp_dir}"):
        return
    ...
    run_cmd(
        "cd /root/knowledge_os/dashboard && nohup python3 -m streamlit run app.py --server.port 5002 --server.address 0.0.0.0 >> brain.log 2>&1 &"
    )
