    ...
    def generate_report(self, analyses: List[Dict[str, Any]]) -> str:
        ...
        for analysis in analyses:
            ...
            if metrics:
                ...
                report.append(
                    f"- **Avg Profit per Trade:** {metrics['avg_profit_per_trade']:.2f} USDT"
                )
                report.append("")
...
def main():
    ...
    parser.add_argument(
        "--results-dir", type=str, default=str(RESULTS_DIR), help="Директория с результатами"
    )
    parser.add_argument(
        "--output", type=str, default=str(OUTPUT_REPORT), help="Файл для сохранения отчета"
    )
