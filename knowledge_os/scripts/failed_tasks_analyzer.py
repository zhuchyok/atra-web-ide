        ...
        if not failed_groups:
            ...
            for group in failed_groups:
                ...
                if "timed out" in pattern.lower() or "Circuit Breaker" in pattern:
                    ...
                    exists = await conn.fetchval("SELECT count(*) FROM tasks WHERE title = $1 AND status IN ('pending', 'in_progress')", "🚨 SYSTEM AUDIT: Timeout Storm")
                    
                    if not exists:
                        await conn.execute("""
                            INSERT INTO tasks (title, description, status, priority, metadata)
                            VALUES ($1, $2, 'pending', 'high', $3)
                        """, "🚨 SYSTEM AUDIT: Timeout Storm", analysis_goal, '{"source": "failed_tasks_analyzer", "auto_generated": true}')
                ...
                if ids_to_delete:
                    import uuid
                    await conn.execute("DELETE FROM tasks WHERE id = ANY($1::uuid[])", [uuid.UUID(i) for i in ids_to_delete])
                    logger.info(f"🗑️ Cleaned up {len(ids_to_delete)} duplicate failed tasks.")

        # 3. Общая статистика для отчета
        stats = await conn.fetchrow("SELECT count(*) as total, count(*) FILTER (WHERE status = 'failed') as failed FROM tasks")
