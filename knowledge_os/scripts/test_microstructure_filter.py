...
# -*- coding: utf-8 -*-
"""
Упрощенный тест Microstructure фильтра (четвертый фильтр)
...
def simple_backtest_with_microstructure(df, symbol, microstructure_before_baseline=False):
    """Упрощенный бэктест с Microstructure фильтром"""
    ...
    # Настраиваем фильтры: VP, VWAP, Order Flow (если доступен), Microstructure
    os.environ['USE_VP_FILTER'] = 'True'
    os.environ['USE_VWAP_FILTER'] = 'True'
    os.environ['USE_ORDER_FLOW_FILTER'] = 'False'  # Отключаем Order Flow для теста
    ...
    def enhanced_soft_entry_signal(df, i):
        """Модифицированная версия с Microstructure"""
        if i < 25:
            return None, None
        ...
        try:
            ...
            # 3. Microstructure фильтр (если перед baseline)
            if microstructure_before_baseline and microstructure_available:
                try:
                    ms_ok, _ = check_microstructure_filter(df, i, "long", strict_mode=False)
                    if not ms_ok:
                        return None, None
            ...
            if long_base_ok:
                # Если Microstructure после baseline - проверяем его здесь
                if not microstructure_before_baseline and microstructure_available:
                    try:
                        ms_ok, _ = check_microstructure_filter(df, i, "long", strict_mode=False)
                        if not ms_ok:
                            return None, None
...
def test_microstructure():
    """Тестирует Microstructure фильтр"""
    print("="*80)
    print("🔍 ТЕСТ MICROSTRUCTURE ФИЛЬТРА (четвертый фильтр)")
    print("="*80)
    ...
    print()
    print("💡 ТЕСТИРУЕМ:")
    print("   1. Microstructure ПОСЛЕ baseline (VP+VWAP+baseline+MS)")
    print("   2. Microstructure ПЕРЕД baseline (VP+VWAP+MS+baseline)")
    print("="*80)
    print()
    ...
    for symbol in TEST_SYMBOLS:
        ...
        try:
            ...
            after_result = simple_backtest_with_microstructure(df, symbol, microstructure_before_baseline=False)
            before_result = simple_backtest_with_microstructure(df, symbol, microstructure_before_baseline=True)
    ...
    print("="*80)
    print(f"📈 MICROSTRUCTURE ПОСЛЕ baseline:")
    print(f"   Сделок: {total_after['trades']}, Return: {total_after['return']:+.2f}%")
    print(f"🔵 MICROSTRUCTURE ПЕРЕД baseline:")
    ...
    if return_diff > 0:
        print("   ✅ Microstructure ПЕРЕД baseline ЛУЧШЕ!")
    elif return_diff < 0:
        print("   ✅ Microstructure ПОСЛЕ baseline ЛУЧШЕ!")
    else:
