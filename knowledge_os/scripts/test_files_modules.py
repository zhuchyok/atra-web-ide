...
from scripts.test_config import PROJECT_ROOT, REQUIRED_FILES, REQUIRED_MODULES
...
def test_required_files_exist() -> TestResult:
    ...
    return TestResult(
        ...
        details={"files_checked": len(unique_files), "categories": list(REQUIRED_FILES.keys())},
    )
    ...
    if errors:
        return TestResult(
            ...
            details={
                ...
                "success_rate": f"{(len(successful) / len(filter_modules) * 100):.1f}%"
                if filter_modules
                else "0%",
            },
            recommendations=[
...
def test_directory_structure() -> TestResult:
    ...
    required_dirs = ["src", "src/filters", "src/strategies", "src/analysis", "scripts", "logs"]
    ...
    if missing_dirs:
        return TestResult(
            ...
            recommendations=["Создайте недостающие директории", "Проверьте структуру проекта"],
        )
