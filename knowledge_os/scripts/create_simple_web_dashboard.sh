...
# Скрипт для создания простого веб-дашборда
# Создает дашборд с простым HTML интерфейсом
...
echo "🔧 Создание простого веб-дашборда"
echo "================================="
...
expect << EOF
spawn scp -o StrictHostKeyChecking=no create_simple_web_dashboard.py $SERVER_USER@$SERVER_IP:$SERVER_PATH/
expect "password:"
send "$SERVER_PASSWORD\r"
expect eof
EOF

# Создаем и запускаем простой веб-дашборд
expect << EOF
...
# Создаем простой веб-дашборд
send "python3 create_simple_web_dashboard.py\r"
expect "# "
...
echo ""
echo "✅ Простой веб-дашборд создан и запущен!"
echo "🌐 Откройте в браузере: http://$SERVER_IP:5000"
echo "📊 Теперь будет простой веб-интерфейс с сигналами!"
