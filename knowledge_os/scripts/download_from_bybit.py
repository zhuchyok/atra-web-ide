...
BYBIT_COINS = ["EOSUSDT", "FTMUSDT", "KLAYUSDT", "MKRUSDT", "RNDRUSDT"]
    ...
    for idx, symbol in enumerate(BYBIT_COINS, 1):
        ...
        if output_file.exists():
            try:
                ...
                days_check = (
                    (
                        pd.to_datetime(df_check["open_time"]).max()
                        - pd.to_datetime(df_check["open_time"]).min()
                    ).days
                    if len(df_check) > 1
                    else 0
                )
                if len(df_check) > 8000 and days_check > 300:  # Больше 300 дней = год
                    print(
                        f"[{idx}/{len(BYBIT_COINS)}] ⏭️  {symbol} - уже загружено ({len(df_check)} свечей, {days_check} дней)"
                    )
                    successful += 1
                    continue
                else:
                    print(
                        f"[{idx}/{len(BYBIT_COINS)}] 🔄 {symbol} - мало данных ({len(df_check)} свечей, {days_check} дней), перезагружаем..."
                    )
            except:
                pass
