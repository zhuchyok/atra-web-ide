...
async def get_local_connection():
    ...
    try:
        ...
        print("✅ Подключено к локальной базе")
        return conn
    except Exception as e:
...
async def get_or_create_domain(local_conn, domain_id, server46_conn):
    ...
    domain = await server46_conn.fetchrow(
        "SELECT id, name, description, created_at FROM domains WHERE id = $1", domain_id
    )
    ...
    local_domain = await local_conn.fetchrow(
        "SELECT id FROM domains WHERE name = $1", domain["name"]
    )
...
async def migrate_node(local_conn, node, domain_id_map):
    ...
    existing = await local_conn.fetchrow("SELECT id FROM knowledge_nodes WHERE id = $1", node["id"])
    ...
    try:
        ...
        async with local_conn.transaction():
            for i, node in enumerate(server46_nodes):
                ...
                if (i + 1) % 100 == 0:
                    print(
                        f"   Обработано: {i + 1}/{len(server46_nodes)} (мигрировано: {migrated_count}, пропущено: {skipped_count})"
                    )
