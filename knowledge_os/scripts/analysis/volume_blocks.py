...
class VolumeBlocksAnalyzer:
        ...
        try:
            ...
            if "volume_ratio" not in analysis_data.columns:
                ...
                if "atr" in analysis_data.columns:
                    analysis_data["volume_ratio"] = (
                        analysis_data["atr"] / analysis_data["atr"].mean()
                    )
                else:
                    # Если нет ATR, используем волатильность как прокси
                    if "volatility" in analysis_data.columns:
                        analysis_data["volume_ratio"] = (
                            analysis_data["volatility"] / analysis_data["volatility"].mean()
                        )
                    else:
            ...
            high_volume_candles = analysis_data[
                analysis_data["volume_ratio"] > avg_volume * self.volume_threshold
            ]
    ...
    def _calculate_block_strength(
        self, blocks: List[Dict], current_price: float, block_type: str
    ) -> float:
        """
    ...
    def get_signal_enhancement(
        self, df: pd.DataFrame, current_index: int, signal_type: str
    ) -> Dict:
        """
...
def enhance_signal_with_blocks(
    df: pd.DataFrame,
    current_index: int,
    signal_type: str,
    signal_price: float,
    ...
    try:
        ...
        if signal_type == "LONG":
            ...
            elif buyer_strength > seller_strength and (
                buyer_strength / max(seller_strength, 0.1)
            ) >= (1 / filter_settings["strengthen_threshold"]):
                # Сигнал усилен
                enhancement_factor = 1.0 + (buyer_strength - seller_strength) * 0.3
                return (
                    f"{signal_type} 🔥",
            ...
            elif seller_strength > buyer_strength and (
                seller_strength / max(buyer_strength, 0.1)
            ) >= (1 / filter_settings["strengthen_threshold"]):
                # Сигнал усилен
                enhancement_factor = 1.0 + (seller_strength - buyer_strength) * 0.3
                return (
