...
def analyze_signal_blocks():
    ...
    # Паттерны блокировок
    patterns = {
        'Direction Check': re.compile(r'Direction.*confidence|Direction Check|не.*подтвержд|2/4|3/4', re.I),
        'Quality Score': re.compile(r'Quality.*Score|quality.*score|качество.*сигнала|0\.\d+.*quality', re.I),
        'RSI Warning': re.compile(r'RSI.*Warning|RSI.*блок|RSI.*65|RSI.*35|RSI.*>.*65|RSI.*<.*35', re.I),
        'MTF Confirmation': re.compile(r'MTF.*Confirmation|MTF.*блок|мультитаймфрейм|H4.*тренд', re.I),
        'BTC Alignment': re.compile(r'BTC.*alignment|BTC.*тренд|BTC.*блок|BTC.*не.*подтвержд', re.I),
    ...
    try:
        ...
        for i, line in enumerate(recent_lines):
            ...
            if any(keyword in line_lower for keyword in ['блок', 'отклонен', 'отклонён', 'rejected', 'blocked', 'не пройден', '🚫']):
                is_block = True
                block_count += 1
        ...
        if symbol_blocks:
            ...
            for symbol, stats in sorted_symbols:
                ...
                print(f"  • {symbol:12s} {stats['total']:3d} попыток, {stats['blocks']:3d} блокировок ({block_rate:5.1f}%) - {reasons_str}")
            print("")
