...
from scripts.test_config import PROJECT_ROOT, REPORT_CONFIG
...
def generate_markdown_report(results: List[TestResult], output_path: str = None) -> str:
    ...
    if output_path is None:
        output_path = os.path.join(REPORT_CONFIG["output_dir"], REPORT_CONFIG["markdown_report"])
    ...
    for result in results:
        ...
        if (
            "БД" in result.name
            or "база" in result.name.lower()
            or "database" in result.name.lower()
        ):
            category = "База данных"
        elif (
            "файл" in result.name.lower()
            or "модуль" in result.name.lower()
            or "file" in result.name.lower()
        ):
            category = "Файлы и модули"
        elif (
            "конфиг" in result.name.lower()
            or "config" in result.name.lower()
            or "окружение" in result.name.lower()
        ):
    ...
    markdown.append("|--------|-----------|---------|")
    markdown.append(
        f"| ✅ Успешно | {passed} | {passed / total * 100:.1f}% |"
        if total > 0
        else "| ✅ Успешно | 0 | 0% |"
    )
    markdown.append(
        f"| ❌ Провалено | {failed} | {failed / total * 100:.1f}% |"
        if total > 0
        else "| ❌ Провалено | 0 | 0% |"
    )
    markdown.append(
        f"| ⚠️  Предупреждения | {warnings} | {warnings / total * 100:.1f}% |"
        if total > 0
        else "| ⚠️  Предупреждения | 0 | 0% |"
    )
    markdown.append(
        f"| ⏭️  Пропущено | {skipped} | {skipped / total * 100:.1f}% |"
        if total > 0
        else "| ⏭️  Пропущено | 0 | 0% |"
    )
    markdown.append("")
...
if __name__ == "__main__":
    ...
    test_results = [
        TestResult(name="Тест 1", status=TestStatus.PASS, message="Успешно", duration=0.5),
        TestResult(
            name="Тест 2",
