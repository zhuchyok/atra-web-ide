...
def test_order_manager_structure() -> TestResult:
    ...
    try:
        ...
        required_methods = ["create_market_order", "create_limit_order", "create_stop_order"]
...
def test_order_parameters_validation() -> TestResult:
    ...
    try:
        ...
        found_methods = [method for method in validation_methods if hasattr(manager, method)]
    ...
    try:
        ...
        try:
            ...
            required_methods = [
                "log_order",
                "log_order_sync",
            ]  # Реальные методы из order_audit_log.py
            found_methods = [method for method in required_methods if hasattr(audit_log, method)]
            ...
            if conn:
                ...
                try:
                    cursor.execute(
                        "SELECT name FROM sqlite_master WHERE type='table' AND name='order_audit_log'"
                    )
                    if cursor.fetchone():
                        conn.close()
                        return TestResult(
