...
async def run_backtest(days: int, symbols: List[str] = None):
    ...
    logger.info("🪙 Монеты: %s (Оптимальный портфель на основе массового скрининга)", ", ".join(symbols))
    ...
    print(f"🔓 MACD фильтр: ОТКЛЮЧЕН")
    print(f"🔓 BB фильтр: ОТКЛЮЧЕН")
    ...
    for result in results_by_symbol:
        ...
        print(f"  {status} {result['symbol']}: {result['total_trades']} сделок, "
    ...
    print(f"  PnL: {actual_pnl:.2f}% {'✅' if actual_pnl >= target_pnl else '❌'} (цель: {target_pnl}%)")
    print(f"  Win Rate: {actual_wr:.2f}% {'✅' if actual_wr >= target_wr else '❌'} (цель: {target_wr}%)")
    print(f"  Profit Factor: {actual_pf:.2f} {'✅' if actual_pf >= target_pf else '❌'} (цель: {target_pf})")
...
async def main():
    ...
    parser.add_argument('--days', type=int, default=30, help='Количество дней для тестирования (30, 90, 365)')
    parser.add_argument('--symbols', type=str, default=None, help='Список монет через запятую (по умолчанию: BTCUSDT,ETHUSDT,BNBUSDT,SOLUSDT)')
