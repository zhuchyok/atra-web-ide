...
for i in range(200, min(5000, len(df))):
    ...
    try:
        ...
        # Комбинации
        all_except_bb = all(
            [ema_trend_up, rsi_oversold, volume_ok, volatility_ok, momentum_ok, trend_ok]
        )
        all_except_volume = all(
            [bb_lower_zone, ema_trend_up, rsi_oversold, volatility_ok, momentum_ok, trend_ok]
        )
        all_except_volatility = all(
            [bb_lower_zone, ema_trend_up, rsi_oversold, volume_ok, momentum_ok, trend_ok]
        )
        all_conditions = all(
            [
                bb_lower_zone,
...
if conditions_stats["all_except_bb"] > 0:
    print(
        f"   ✅ Без BB условия: {conditions_stats['all_except_bb']} раз - BB условие слишком строгое"
    )
if conditions_stats["all_except_volume"] > 0:
    print(
        f"   ✅ Без Volume условия: {conditions_stats['all_except_volume']} раз - Volume условие слишком строгое"
    )
if conditions_stats["all_except_volatility"] > 0:
    print(
        f"   ✅ Без Volatility условия: {conditions_stats['all_except_volatility']} раз - Volatility условие слишком строгое"
    )
