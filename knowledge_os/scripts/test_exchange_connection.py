...
async def test_exchange_connection() -> TestResult:
    ...
    try:
        ...
        # Создаем адаптер
        keys = {"api_key": api_key, "secret": api_secret, "passphrase": api_passphrase or ""}
        ...
        try:
            ...
            return TestResult(
                ...
                details={"exchange": "bitget", "markets_loaded": len(markets) if markets else 0},
...
async def test_exchange_balance() -> TestResult:
    ...
    try:
        ...
        adapter = ExchangeAdapter(exchange="bitget", keys=keys, sandbox=True, trade_mode="futures")

        if adapter.client is None:
            return TestResult(
                name="Баланс биржи", status=TestStatus.SKIP, message="Клиент биржи не создан"
            )
...
async def run_all_exchange_tests() -> list:
    ...
    # Синхронные тесты
    sync_tests = [test_exchange_adapter_import, test_symbol_format]
    ...
    # Асинхронные тесты
    async_tests = [test_exchange_connection, test_exchange_balance]
