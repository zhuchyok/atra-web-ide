...
# Скрипт для создания простого дашборда
# Создает минимальный дашборд, который точно работает
...
expect << EOF
spawn scp -o StrictHostKeyChecking=no create_simple_dashboard.py $SERVER_USER@$SERVER_IP:$SERVER_PATH/
expect "password:"
send "$SERVER_PASSWORD\r"
expect eof
EOF

# Создаем и запускаем простой дашборд
expect << EOF
...
send "python3 create_simple_dashboard.py\r"
expect "# "
...
echo ""
echo "✅ Простой дашборд создан и запущен!"
