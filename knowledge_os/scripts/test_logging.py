...
from scripts.test_config import DATABASE_PATH, LOG_PATHS
...
def test_log_files_exist() -> TestResult:
    ...
    if len(missing_logs) == len(LOG_PATHS):
        return TestResult(
            ...
            details={"missing": missing_logs, "existing": existing_logs},
            recommendations=[
    ...
    return TestResult(
        ...
        details={"existing": existing_logs, "missing": missing_logs},
        metrics={
...
def test_database_logging() -> TestResult:
    ...
    if conn is None:
        return TestResult(
            name="Логирование в БД", status=TestStatus.FAIL, message="Не удалось подключиться к БД"
        )

    try:
        # Проверяем наличие таблиц для логов
        log_tables = ["signals_log", "order_audit_log", "filter_performance"]
        ...
        if missing_tables:
            return TestResult(
                ...
                recommendations=["Создайте недостающие таблицы", "Проверьте инициализацию БД"],
            )
