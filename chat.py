...
# Простые сообщения для которых не нужен агент Victoria (быстрый путь через Ollama)
SIMPLE_PATTERNS = [
...
VICTORIA_PATTERNS = [
    ...
    "агент", "задач", "mac studio", "макстудио", "ollama", "mlx"
]
...
async def _get_available_model(ideal_model: str, ollama: OllamaClient) -> str:
    ...
    Args:
        ...
        ollama: OllamaClient для проверки доступности
    
    Returns:
        Доступная модель (идеальная или fallback)
    """
    # Маппинг идеальных моделей на возможные варианты имен в Ollama
    model_variants = {
    ...
    # Fallback цепочки (только РЕАЛЬНО существующие модели из Ollama)
    ...
    fallback_chains = {
        ...
        "tinyllama:1.1b-chat": ["phi3.5:3.8b", "moondream:latest"]
    }
    ...
    try:
        ...
        # Если ничего не найдено, возвращаем идеальную модель (пусть Ollama вернет ошибку)
...
async def sse_generator(
    ...
    try:
        ...
        if not use_ollama_direct:
            ...
            if "error" in result:
                # Fallback ответ когда Victoria недоступна
                logger.warning(f"Victoria недоступна, используем fallback ответ")
                fallback_response = f"Привет! Я {message.expert_name or 'Виктория'}. К сожалению, сейчас не могу подключиться к Victoria Agent. Попробуйте позже или используйте простой режим чата."
                ...
                # Отправляем контент по частям для имитации стриминга
                ...
                if not content:
                    ...
                    content = f"Привет! Я {message.expert_name or 'Виктория'}. Получил ваш запрос, но ответ пока пустой. Попробуйте переформулировать вопрос."
            ...
            # Используем Ollama/MLX напрямую для простых сообщений
            expert_prompt = ""
            ...
            # Автоматический выбор модели на основе категории задачи
            # Сначала выбираем идеальную модель, потом проверяем доступность
            ...
            # Проверяем доступность модели и используем fallback если нужно
            selected_model = await _get_available_model(ideal_model, ollama)
            logger.info(f"✅ Выбранная модель: {selected_model}")
            
            result = None
            try:
                logger.info(f"🚀 Генерируем ответ через Ollama с моделью: {selected_model}")
                ...
                logger.info(f"✅ Ollama вернул результат: {bool(result and 'error' not in result)}")
            except Exception as e:
                logger.error(f"Ollama generate exception: {e}")
                result = {"error": str(e)}
            
            # Проверяем результат и используем MLX как резерв при ошибке Ollama
            if result is None:
                result = {"error": "No response from Ollama"}
            
            if isinstance(result, dict) and "error" in result:
                # Пробуем MLX как резервный вариант
                error_msg = result.get('error', 'Unknown error')
                logger.warning(f"Ollama недоступен ({error_msg[:100]}), пробуем MLX как резерв")
                logger.info(f"MLX available check: {mlx.is_available()}")
                
                if mlx.is_available():
                    try:
                        logger.info("🍎 [MLX] Используем MLX как резервный вариант")
                        ...
                        if mlx_result and "response" in mlx_result:
                            ...
                            # MLX тоже не сработал, используем fallback
                            logger.warning("MLX тоже недоступен, используем текстовый fallback")
                            expert_name = message.expert_name or "ассистент"
                            fallback_response = f"Привет! Я {expert_name}. К сожалению, сейчас не могу подключиться к AI-модели (Ollama и MLX недоступны). Попробуйте позже."
                            words = fallback_response.split()
                            chunk = ""
                            for i, word in enumerate(words):
                                chunk += word + " "
                        ...
                        # Fallback при ошибке MLX
                        expert_name = message.expert_name or "ассистент"
                        fallback_response = f"Привет! Я {expert_name}. Произошла ошибка при использовании MLX. Попробуйте позже."
                        words = fallback_response.split()
                        chunk = ""
                        for i, word in enumerate(words):
                            chunk += word + " "
                    ...
                    # MLX недоступен, используем текстовый fallback
                    logger.warning("MLX недоступен, используем текстовый fallback")
                    expert_name = message.expert_name or "ассистент"
                    fallback_response = f"Привет! Я {expert_name}. К сожалению, сейчас не могу подключиться к AI-модели (Ollama недоступна, MLX недоступен). Попробуйте позже или проверьте подключение к Ollama на порту 11434."
                    words = fallback_response.split()
                    chunk = ""
                    for i, word in enumerate(words):
                        chunk += word + " "
                ...
                if not content:
                    content = "Извините, не получилось сгенерировать ответ."
                ...
                # Если результат пустой или неожиданный формат
                logger.warning(f"Неожиданный формат ответа от Ollama: {result}")
                expert_name = message.expert_name or "ассистент"
                fallback_response = f"Привет! Я {expert_name}. Извините, произошла ошибка при обработке запроса."
                words = fallback_response.split()
                chunk = ""
                for i, word in enumerate(words):
                    chunk += word + " "
...
async def chat_status(
    ...
    """
    Статус сервисов чата
    
    Returns:
        Статус Victoria и Ollama
    """
    try:
