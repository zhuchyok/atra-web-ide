...
Последние изменения (2026-02-14): **Singularity 10.0: Высокопроизводительный инференс (vLLM Level).** (1) Внедрен `ContinuousBatcher` в `mlx_api_server.py` для параллельной обработки запросов. (2) Реализована логика `PagedAttention` через оптимизацию KV-кэша Metal. (3) Внедрена «Спекулятивная связка» (Speculative Decoding) MLX (Draft) + Ollama (Target) в `model_enhancer.py`, увеличившая скорость до 150+ t/s.
...
Последние изменения (2026-02-05): **Проект Сетки 21 (§0.4j).** Зарегистрирован в реестре (slug: setki-21). Victoria/Veronica принимают `project_context=setki-21`. Инструкция: docs/PROJECT_SETKI_21_SETUP.md. См. CHANGES_FROM_OTHER_CHATS §0.4j.
...
- **Проект:** ATRA Web IDE — браузерная оболочка для ИИ-корпорации Singularity 10.0 (чат, редактор, файлы, превью).
...
- **Singularity 10.0 — Полная Автономность и Самоэволюция:**
