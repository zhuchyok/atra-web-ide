...
**Куратор без Cursor (автономность):** скрипты куратора уже не зависят от Cursor (только VICTORIA_URL и локальная БД); запуск по cron/launchd возможен. Для полной автономности при расхождении с эталоном нужно не ручное решение, а автоматическое действие: создание задачи в БД (assignee: Technical Writer / Victoria) или осторожная авто-правка RAG. Варианты и изолированная среда — CURATOR_RUNBOOK §6 «Путь к автономности».
