...
### 2.2 Почему после перезагрузки не поднялась Виктория (мозг MLX)

**Victoria Agent** (контейнер) поднимается после перезагрузки, если включён автозапуск Docker и у контейнеров стоит `restart: always`. **MLX API Server** (мозг, порт 11435) работает **на хосте**, не в Docker — поэтому он **не стартует сам**, пока не настроен автозапуск через launchd.

**Что сделать один раз:** выполнить полную настройку автозапуска (включает MLX):

```bash
bash scripts/setup_complete_autostart.sh
```

Либо только MLX при входе в систему:

```bash
bash scripts/setup_mlx_autostart.sh
```

После этого при каждом входе в Mac (или при загрузке, в зависимости от launchd) будет создаваться лаунчер `~/Library/Application Support/Atra/launch_mlx.sh`, который подхватывает `.env` (в т.ч. `VICTORIA_MLX_BRAIN=true`) и запускает `scripts/start_mlx_api_server.sh`. Дополнительно система самовосстановления (`system_auto_recovery.sh`) по расписанию и по webhook (порт 9099) проверяет MLX и при необходимости поднимает его.
