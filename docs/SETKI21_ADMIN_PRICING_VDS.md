...
## База данных Сетки 21 на VDS (единственный источник истины)

| Что                                   | Значение                                                                                             |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **Контейнер PostgreSQL**              | `atra-postgres` (сервис в compose: `postgres`, сеть `atra-net`)                                      |
| **База данных проекта Сетки 21**      | **`moskit`** (отдельная БД в этом же инстансе PostgreSQL)                                            |
| **Пользователь БД**                   | `moskit`                                                                                             |
| **Пароль**                            | переменная окружения `MOSKIT_DB_PASSWORD` (на VDS в `.env`; по умолчанию в скрипте: `moskit_secret`) |
| **Строка подключения для moskit-api** | `postgres://moskit:${MOSKIT_DB_PASSWORD}@postgres:5432/moskit`                                       |

Никакой отдельной «БД Сетки 21» вне этого инстанса нет: **одна БД `moskit`** на **одном PostgreSQL (atra-postgres)**. Knowledge OS и другие сервисы используют другие БД/пользователей на том же или другом хосте; для www.setki21.ru и moskit-api — только `moskit` на atra-postgres. Создание БД и пользователя: `scripts/create_moskit_db_vds.sh`.
