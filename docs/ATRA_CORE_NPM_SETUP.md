...
## Почему http://45.10.43.248:81 открывается, а https — нет

**Панель NPM (порт 81)** по умолчанию слушает **только HTTP**. На порту 81 нет SSL — это штатное поведение Nginx Proxy Manager: админка отдаётся без шифрования. Поэтому:

- **http://45.10.43.248:81** — работает;
- **https://45.10.43.248:81** — не работает (ничего не слушает 81 по HTTPS).

**Как открыть панель по HTTPS:**

1. Завести отдельный домен для админки (например **npm.setki21.ru**), A-запись на **45.10.43.248**.
2. В NPM: **Proxy Hosts → Add Proxy Host**
   - **Domain Names:** `npm.setki21.ru`
   - **Scheme:** `http`
   - **Forward Hostname / IP:** `127.0.0.1` (или `host.docker.internal` — админка в том же контейнере)
   - **Forward Port:** `81`
3. Вкладка **SSL** → **Request a new SSL Certificate** (Let's Encrypt), включить **Force SSL**.
4. Открывать панель по **https://npm.setki21.ru** (трафик идёт на 80/443, NPM проксирует на свой же :81).
5. (Рекомендуется) Закрыть порт 81 снаружи firewall’ом, чтобы входить только через HTTPS-домен.

**Без домена:** по IP сертификат Let's Encrypt не выдаётся. Вариант — самоподписанный сертификат и отдельный reverse proxy на хосте (например nginx на 8443 → 127.0.0.1:81), тогда будет **https://45.10.43.248:8443** с предупреждением браузера.
