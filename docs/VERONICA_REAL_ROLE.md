...
| **Чего НЕ умеет** | Не выбирает экспертов, не планирует стратегию, не синтезирует решения из 86 экспертов (БД, Docker) |
...
- **Почему так плохо:** Veronica не умеет делегировать экспертам и не должна принимать решения за всю корпорацию; у неё нет ролей 86 экспертов (БД, Docker).
